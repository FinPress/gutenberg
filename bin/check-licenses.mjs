...
/*
 * `fin-scripts check-licenses` uses prod and dev dependencies of the package to scan for dependencies. With npm workspaces, workspace packages (the @finpress/* packages) are not listed in the main package json and this approach does not work.
 *
...
const workspacePackages = JSON.parse(
	spawnSync(
		...
		[
			'query',
			'.workspace:attr([finScript]), .workspace:attr([finScriptModuleExports])',
		],
		/*
