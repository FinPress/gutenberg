...
async function runPerformanceTests(branches, options) {
	...
	for (const testSuite of testSuites) {
		...
		for (const [metric, branch] of Object.entries(invertedResult)) {
			...
			for (const [branchName, data] of Object.entries(branch)) {
				printedResult[metric][branchName] = printStats(metric, data);
			}
			...
			if (branches.length === 2) {
				...
				printedResult[metric]['% Change'] = `${percentageChange}%`;
			}
		}
		...
		for (const [metric, resultBranches] of Object.entries(printedResult)) {
			/**
			...
			for (const [branch, value] of Object.entries(resultBranches)) {
				row[branch] = value;
			}
			rows.push(row);
		}
	...
	fs.writeFileSync(path.join(ARTIFACTS_PATH, 'summary.md'), summaryMarkdown);
}
