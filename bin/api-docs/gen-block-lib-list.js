 ...
 * @type {string}
 */
const BLOCK_LIBRARY_DIR = path.resolve(ROOT_DIR, 'packages/block-library/src');
...
function readBlockJSON(filename) {
	...
	if (allowedBlocks?.length > 0) {
		blockInfoList.push(`-	**Allowed Blocks:** ${allowedBlocks.join(', ')}`);
	}
	if (supports) {
		blockInfoList.push(
			`-	**Supports:** ${processObjWithInnerKeys(augmentSupports(supports))
				.sort()
				.join(', ')}`
		);
	}
