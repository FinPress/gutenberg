...
const serializePrimitiveTypes = createSerializer(
	(schema) => schema.type && !['object', 'array'].includes(schema.type),
	(schema) => `\`${schema.type}\``
);
...
const serializeObjectArrayTypes = createSerializer(
	...
	(schema) => `\`[ { ${Object.keys(schema.items.properties).join(', ')} } ]\``
);
...
function generateDocs(themeJson) {
	...
	const settings = [
		...
		...themeJson.properties.settings.allOf[0].allOf.flatMap((subschema) =>
