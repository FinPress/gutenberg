...
const LABEL = process.argv[ 2 ] || 'Backport to FIN Beta/RC';
const BACKPORT_COMPLETED_LABEL = 'Backported to FIN Core';
 ...
 * * Requests the list of PRs to cherry-pick from GitHub API (closed, label=`Backport to FIN Beta/RC`)
...
function reportSummaryNextSteps( successes, failures ) {
	...
	if ( successes.length && ! AUTO_PROPAGATE_RESULTS_TO_GITHUB ) {
		...
		if ( LABEL === 'Backport to FIN Beta/RC' ) {
