...
The Gutenberg block editor uses the `@finpress/i18n` package to provide internationalization support.
...
First, we need to define that our locale is RTL using `@finpress/i18n`.
...
Second, we need to load the RTL CSS stylesheets instead of the LTR ones. For each of the stylesheets that you load from `@finpress` packages, there's an RTL version that you can use instead.

For example, when loading the `@finpress/components` stylesheet, you can load the RTL version by using `@finpress/components/build-style/style-rtl.css` instead of `@finpress/components/build-style/style.css`.
