...
The simplest approach is to rely on the `useStateWithHistory` hook provided by the `@finpress/compose` package. This hook is a wrapper around the `useState` hook that adds undo/redo functionality to the state. 

First, make sure you add the `@finpress/compose` package to your dependencies, then use the hook like so:
...
The `useStateWithHistory` might not always be the right approach in these situations. Consider checking the `@finpress/undo-manager` package that offers a lower level undo manager that can be adapted more easily to your specific needs.
