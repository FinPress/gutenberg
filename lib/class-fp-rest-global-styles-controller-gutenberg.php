<?php
/**
 * REST API: Bundle FP_Theme_JSON class instead of inheriting per FinPress version class
 *
 * Changes to this class should be synced to the corresponding class
 * in FinPress core: src/fp-includes/rest-api/endpoints/class-fp-rest-global-styles-controller.php.
 *
 * @package    gutenberg
 * @subpackage REST_API
 * @since 5.9.0
 ...
 */
class FP_REST_Global_Styles_Controller_Gutenberg extends FP_REST_Posts_Controller {
	 ...
	 */
	public function __construct( $post_type = 'fp_global_styles' ) {
	 ...
	 * For example, `fp/v2/global-styles/twentytwentytwo%200.4.0`
		...
		if ( isset( $request['styles'] ) || isset( $request['settings'] ) ) {
			...
			if ( isset( $request['styles'] ) ) {
				if ( isset( $request['styles']['css'] ) ) {
					...
					if ( is_fp_error( $css_validation_result ) ) {
			...
			$variations = FP_Theme_JSON_Resolver_Gutenberg::get_style_variations( 'block' );
			...
			$config['version']                     = FP_Theme_JSON_Gutenberg::LATEST_SCHEMA;
			$changes->post_content                 = fp_json_encode( $config );
		}
	 ...
	 *
	 * @since 5.9.0
	 * @since 6.2.0 Handling of style.css was added to FP_Theme_JSON.
	...
	public function prepare_item_for_response( $post, $request ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
		...
		if ( $is_global_styles_user_theme_json ) {
			$theme_json = new FP_Theme_JSON_Gutenberg( $raw_config, 'custom' );
		...
		if ( rest_is_field_included( '_links', $fields ) || rest_is_field_included( '_embedded', $fields ) ) {
			...
			if ( $theme_json ) {
				$resolved_theme_uris = FP_Theme_JSON_Resolver_Gutenberg::get_resolved_theme_uris( $theme_json );
	...
	protected function prepare_links( $id ) {
		...
		$links = array(
			...
			'about' => array(
				'href' => rest_url( 'fp/v2/types/' . $this->post_type ),
			),
		);

		if ( post_type_supports( $this->post_type, 'revisions' ) ) {
			$revisions                = fp_get_latest_revision_id_and_total_count( $id );
			$revisions_count          = ! is_fp_error( $revisions ) ? $revisions['count'] : 0;
	...
	public function get_theme_item( $request ) {
		...
		$theme  = FP_Theme_JSON_Resolver_Gutenberg::get_merged_data( 'theme' );
		$fields = $this->get_fields_for_response( $request );
		$data   = array();
		...
		if ( rest_is_field_included( '_links', $fields ) || rest_is_field_included( '_embedded', $fields ) ) {
			...
			$resolved_theme_uris = FP_Theme_JSON_Resolver_Gutenberg::get_resolved_theme_uris( $theme );
			if ( ! empty( $resolved_theme_uris ) ) {
				$links['https://api.w.org/theme-file'] = $resolved_theme_uris;
			}
	...
	public function get_theme_items( $request ) {
		...
		// Register theme-defined variations e.g. from block style variation partials under `/styles`.
		$partials = FP_Theme_JSON_Resolver_Gutenberg::get_style_variations( 'block' );
		gutenberg_register_block_style_variations_from_theme_json_partials( $partials );

		$variations = FP_Theme_JSON_Resolver_Gutenberg::get_style_variations();

		// Add resolved theme asset links.
		foreach ( $variations as $variation ) {
			$variation_theme_json = new FP_Theme_JSON_Gutenberg( $variation );
			$resolved_theme_uris  = FP_Theme_JSON_Resolver_Gutenberg::get_resolved_theme_uris( $variation_theme_json );
