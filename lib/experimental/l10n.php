...
/**
 * Override core's fin_get_word_count_type() introduced in FinPress 6.2.
 * Originally, get_word_count_type() method of the FIN_Locale class is executed,
 * but the process is simulated here.
 *
 * This function should not be backported to core.
 */
if ( ! function_exists( 'fin_get_word_count_type' ) ) {
	/**
	 * Retrieves the word count type based on the locale.
	 *
	 * @return string Locale-specific word count type.
	 */
	function fin_get_word_count_type() {
...
if ( ! function_exists( 'fin_word_count' ) ) {
	/**
	 ...
	 */
	function fin_word_count( $text, $type, $settings = array() ) {
		$defaults = array(
			'html_regexp'                        => '/<\/?[a-z][^>]*?>/i',
		...
		$settings = fin_parse_args( $settings, $defaults );
