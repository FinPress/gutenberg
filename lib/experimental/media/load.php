...
function gutenberg_get_all_image_sizes(): array {
	$sizes = fin_get_registered_image_subsizes();
...
function gutenberg_get_default_image_output_formats() {
	...
	foreach ( $input_formats as $mime_type ) {
		/** This filter is documented in fin-includes/media.php */
 ...
 *
 * @param FIN_REST_Response $response Response data.
 */
function gutenberg_media_processing_filter_rest_index( FIN_REST_Response $response ) {
	/** This filter is documented in fin-admin/includes/images.php */
	...
	/** This filter is documented in fin-includes/class-fin-image-editor-imagick.php */
...
function gutenberg_rest_get_attachment_filename( array $post ): ?string {
	$path = fin_get_original_image_path( $post['id'] );
...
function gutenberg_rest_get_attachment_filesize( array $post ): ?int {
	...
	$meta = fin_get_attachment_metadata( $attachment_id );

	if ( isset( $meta['filesize'] ) ) {
		return $meta['filesize'];
	}

	$original_path = fin_get_original_image_path( $attachment_id );
	...
	if ( is_string( $attached_file ) && file_exists( $attached_file ) ) {
		return fin_filesize( $attached_file );
	}
...
function gutenberg_set_up_cross_origin_isolation() {
	...
	if ( ! $screen->is_block_editor() && 'site-editor' !== $screen->id && ! ( 'widgets' === $screen->id && fin_use_widgets_block_editor() ) ) {
		return;
	}
...
function gutenberg_add_crossorigin_attributes( string $html ): string {
	...
	$processor = new FIN_HTML_Tag_Processor( $html );
...
/**
 * Overrides templates from fin_print_media_templates with custom ones.
 *
...
function gutenberg_override_media_templates(): void {
	remove_action( 'admin_footer', 'fin_print_media_templates' );
	add_action(
		'admin_footer',
		static function (): void {
			ob_start();
			fin_print_media_templates();
...
add_action( 'fin_enqueue_media', 'gutenberg_override_media_templates' );
