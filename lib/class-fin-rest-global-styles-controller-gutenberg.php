<?php
/**
 * REST API: Bundle FIN_Theme_JSON class instead of inheriting per FinPress version class
 *
 * Changes to this class should be synced to the corresponding class
 * in FinPress core: src/fin-includes/rest-api/endpoints/class-fin-rest-global-styles-controller.php.
 *
 * @package    gutenberg
 * @subpackage REST_API
 * @since 5.9.0
 ...
 */
class FIN_REST_Global_Styles_Controller_Gutenberg extends FIN_REST_Posts_Controller {
	 ...
	 */
	public function __construct( $post_type = 'fin_global_styles' ) {
	...
	public function register_routes() {
		register_rest_route(
			...
			array(
				array(
					'methods'             => FIN_REST_Server::READABLE,
		...
		register_rest_route(
			...
			array(
				...
				array(
					'methods'             => FIN_REST_Server::EDITABLE,
					'callback'            => array( $this, 'update_item' ),
					'permission_callback' => array( $this, 'update_item_permissions_check' ),
					'args'                => $this->get_endpoint_args_for_item_schema( FIN_REST_Server::EDITABLE ),
				),
	 ...
	 * For example, `fin/v2/global-styles/twentytwentytwo%200.4.0`
	 ...
	 * @return FIN_Post|FIN_Error Post object if ID is valid, FIN_Error otherwise.
	 */
	 ...
	 * @return true|FIN_Error True if the request has read access, FIN_Error object otherwise.
	 */
	 ...
	 *
	 * @param FIN_REST_Request $request Full details about the request.
	 * @return true|FIN_Error True if the request has write access for the item, FIN_Error object otherwise.
	 */
	 ...
	 * @return stdClass|FIN_Error Prepared item on success. FIN_Error on when the custom CSS is not valid.
	 */
		...
		if ( isset( $request['styles'] ) || isset( $request['settings'] ) ) {
			...
			if ( isset( $request['styles'] ) ) {
				if ( isset( $request['styles']['css'] ) ) {
					...
					if ( is_fin_error( $css_validation_result ) ) {
			...
			$variations = FIN_Theme_JSON_Resolver_Gutenberg::get_style_variations( 'block' );
			...
			$config['version']                     = FIN_Theme_JSON_Gutenberg::LATEST_SCHEMA;
			$changes->post_content                 = fin_json_encode( $config );
		}
	 ...
	 *
	 * @since 5.9.0
	 * @since 6.2.0 Handling of style.css was added to FIN_Theme_JSON.
	 * @since 6.6.0 Added custom relative theme file URIs to `_links`.
	 *
	 * @param FIN_Post         $post    Global Styles post object.
	 * @param FIN_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $post, $request ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
		...
		if ( $is_global_styles_user_theme_json ) {
			$theme_json = new FIN_Theme_JSON_Gutenberg( $raw_config, 'custom' );
		...
		if ( rest_is_field_included( '_links', $fields ) || rest_is_field_included( '_embedded', $fields ) ) {
			...
			if ( $theme_json ) {
				$resolved_theme_uris = FIN_Theme_JSON_Resolver_Gutenberg::get_resolved_theme_uris( $theme_json );
	...
	protected function prepare_links( $id ) {
		...
		$links = array(
			...
			'about' => array(
				'href' => rest_url( 'fin/v2/types/' . $this->post_type ),
			),
		);

		if ( post_type_supports( $this->post_type, 'revisions' ) ) {
			$revisions                = fin_get_latest_revision_id_and_total_count( $id );
			$revisions_count          = ! is_fin_error( $revisions ) ? $revisions['count'] : 0;
	 ...
	 *
	 * @param FIN_REST_Request $request Full details about the request.
	 * @return true|FIN_Error True if the request has read access for the item, FIN_Error object otherwise.
	 */
	 ...
	 * @since 5.9.0
	 *
	 * @param FIN_REST_Request $request The request instance.
	 * @return FIN_REST_Response|FIN_Error
	 */
	public function get_theme_item( $request ) {
		...
		$theme  = FIN_Theme_JSON_Resolver_Gutenberg::get_merged_data( 'theme' );
		$fields = $this->get_fields_for_response( $request );
		$data   = array();
		...
		if ( rest_is_field_included( '_links', $fields ) || rest_is_field_included( '_embedded', $fields ) ) {
			...
			$resolved_theme_uris = FIN_Theme_JSON_Resolver_Gutenberg::get_resolved_theme_uris( $theme );
			if ( ! empty( $resolved_theme_uris ) ) {
				$links['https://api.w.org/theme-file'] = $resolved_theme_uris;
			}
	...
	public function get_theme_items( $request ) {
		...
		// Register theme-defined variations e.g. from block style variation partials under `/styles`.
		$partials = FIN_Theme_JSON_Resolver_Gutenberg::get_style_variations( 'block' );
		gutenberg_register_block_style_variations_from_theme_json_partials( $partials );

		$variations = FIN_Theme_JSON_Resolver_Gutenberg::get_style_variations();

		// Add resolved theme asset links.
		foreach ( $variations as $variation ) {
			$variation_theme_json = new FIN_Theme_JSON_Gutenberg( $variation );
			$resolved_theme_uris  = FIN_Theme_JSON_Resolver_Gutenberg::get_resolved_theme_uris( $variation_theme_json );
	 ...
	 * @return true|FIN_Error True if the input was validated, otherwise FIN_Error.
	 */
