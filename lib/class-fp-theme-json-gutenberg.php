...
class FP_Theme_JSON_Gutenberg {
	...
	const PRESETS_METADATA = array(
		...
		array(
			...
			'use_default_names' => false,
			'value_func'        => null, // CSS Custom Properties for duotone are handled by block supports in class-fp-duotone-gutenberg.php.
			'css_vars'          => null,
			'classes'           => array(),
		...
		array(
			...
			'use_default_names' => false,
			'value_key'         => 'size',
			'css_vars'          => '--fp--preset--border-radius--$slug',
			'classes'           => array(),
	 ...
	 */
	public function __construct( $theme_json = array( 'version' => FP_Theme_JSON_Gutenberg::LATEST_SCHEMA ), $origin = 'theme' ) {
		if ( ! in_array( $origin, static::VALID_ORIGINS, true ) ) {
			$origin = 'theme';
		}

		$this->theme_json    = FP_Theme_JSON_Schema_Gutenberg::migrate( $theme_json, $origin );
		...
		foreach ( $nodes as $node ) {
			foreach ( static::PRESETS_METADATA as $preset_metadata ) {
				...
				$preset = _fp_array_get( $this->theme_json, $path, null );
				if ( null !== $preset ) {
					// If the preset is not already keyed by origin.
					if ( isset( $preset[0] ) || empty( $preset ) ) {
						_fp_array_set( $this->theme_json, $path, array( $origin => $preset ) );
					}
				}
			}
		}
		...
		$spacing_scale = _fp_array_get( $this->theme_json, $scale_path, null );
		if ( null !== $spacing_scale ) {
			...
			if ( empty( array_intersect( array_keys( $spacing_scale ), static::VALID_ORIGINS ) ) ) {
				_fp_array_set( $this->theme_json, $scale_path, array( $origin => $spacing_scale ) );
			}
		}
		...
		if ( isset( $spacing_scale ) ) {
			...
			$spacing_sizes        = _fp_array_get( $this->theme_json, $sizes_path, array() );
			...
			_fp_array_set( $this->theme_json, $sizes_path, $merged_spacing_sizes );
		}
	}
		...
		foreach ( $valid_variations as $block_type => $registered_variations ) {
			foreach ( $registered_variations as $variation_name ) {
				...
				if ( ! empty( $merged_data ) ) {
					_fp_array_set( $new_theme_json, array( 'styles', 'blocks', $block_type, 'variations', $variation_name ), $merged_data );
				}
			}
		}
	...
	protected static function do_opt_in_into_settings( &$context ) {
		foreach ( static::APPEARANCE_TOOLS_OPT_INS as $path ) {
			...
			if ( 'unset prop' === _fp_array_get( $context, $path, 'unset prop' ) ) {
				_fp_array_set( $context, $path, true );
			}
		}
	...
	protected static function get_blocks_metadata() {
		...
		foreach ( $blocks as $block_name => $block_type ) {
			$root_selector = fp_get_block_css_selector( $block_type );
			...
			$duotone_selector = fp_get_block_css_selector( $block_type, 'filter.duotone' );
	...
	public function get_stylesheet( $types = array( 'variables', 'styles', 'presets' ), $origins = null, $options = array() ) {
		...
		if ( in_array( 'custom-css', $types, true ) ) {
			...
			$stylesheet .= _fp_array_get( $this->theme_json, array( 'styles', 'css' ) );
		}
	...
	protected function get_layout_styles( $block_metadata, $types = array() ) {
		...
		$node                     = _fp_array_get( $this->theme_json, $block_metadata['path'], array() );
	...
	protected function get_preset_classes( $setting_nodes, $origins ) {
		...
		foreach ( $setting_nodes as $metadata ) {
			...
			$node          = _fp_array_get( $this->theme_json, $metadata['path'], array() );
	...
	protected static function get_settings_values_by_slug( $settings, $preset_metadata, $origins ) {
		$preset_per_origin = _fp_array_get( $settings, $preset_metadata['path'], array() );
		...
		foreach ( $origins as $origin ) {
			...
			foreach ( $preset_per_origin[ $origin ] as $preset ) {
				$slug = _fp_to_kebab_case( $preset['slug'] );
	...
	protected static function flatten_tree( $tree, $prefix = '', $token = '--' ) {
		...
		foreach ( $tree as $property => $value ) {
			$new_key = $prefix . str_replace(
				'/',
				'-',
				strtolower( _fp_to_kebab_case( $property ) )
			);
	 ...
	 * @since 6.6.0 Passing current theme JSON settings to fp_get_typography_font_size_value(). Using style engine to correctly fetch background CSS values.
	...
	protected static function compute_style_properties( $styles, $settings = array(), $properties = null, $theme_json = null, $selector = null, $use_root_padding = null ) {
		...
		foreach ( $properties as $css_property => $value_path ) {
			...
			if (
				...
				_fp_array_get( $settings, static::PROTECTED_PROPERTIES[ $path_string ], null ) === null
			) {
				continue;
			}
			...
			if ( 'font-size' === $css_property ) {
				/*
				 * fp_get_typography_font_size_value() will check
	...
	protected static function get_property_value( $styles, $path, $theme_json = null ) {
		$value = _fp_array_get( $styles, $path, '' );
		...
		if ( is_array( $value ) && isset( $value['ref'] ) ) {
			...
			$ref_value  = _fp_array_get( $theme_json, $value_path, null );
	...
	protected static function get_style_nodes( $theme_json, $selectors = array(), $options = array() ) {
		 ...
		 */
		return apply_filters( 'fp_theme_json_get_style_nodes', $nodes );
	}
	...
	public function get_styles_for_block( $block_metadata ) {
		...
		if ( ! empty( $block_metadata['variations'] ) ) {
			foreach ( $block_metadata['variations'] as $style_variation ) {
				$style_variation_node           = _fp_array_get( $this->theme_json, $style_variation['path'], array() );
	 ...
	 *
	 * @param FP_Theme_JSON_Gutenberg $incoming Data to merge.
	 */
	public function merge( $incoming ) {
		...
		foreach ( static::VALID_ORIGINS as $origin ) {
			...
			$base_spacing_scale      = _fp_array_get( $this->theme_json, $scale_path, array() );
			$flattened_spacing_scale = array_replace( $flattened_spacing_scale, $base_spacing_scale );

			$spacing_scale = _fp_array_get( $incoming_data, $scale_path, null );
			if ( ! isset( $spacing_scale ) ) {
				continue;
			}
			...
			$sizes_path           = array( 'settings', 'spacing', 'spacingSizes', $origin );
			$spacing_sizes        = _fp_array_get( $incoming_data, $sizes_path, array() );
			$spacing_scale_sizes  = static::compute_spacing_sizes( $flattened_spacing_scale );
			$merged_spacing_sizes = static::merge_spacing_sizes( $spacing_scale_sizes, $spacing_sizes );

			_fp_array_set( $incoming_data, $sizes_path, $merged_spacing_sizes );
		}
		...
		foreach ( $nodes as $node ) {
			...
			$content = _fp_array_get( $incoming_data, $path, null );
			if ( isset( $content ) ) {
				_fp_array_set( $this->theme_json, $path, $content );
			}
			...
			foreach ( static::PRESETS_METADATA as $preset_metadata ) {
				...
				if ( is_array( $prevent_override ) ) {
					$prevent_override = _fp_array_get( $this->theme_json['settings'], $preset_metadata['prevent_override'] );
				}
				...
				foreach ( static::VALID_ORIGINS as $origin ) {
					...
					if ( 'theme' === $origin && $prevent_override ) {
						...
						$preset_global = _fp_array_get( $slugs_global, $preset_metadata['path'], array() );
						$preset_node   = _fp_array_get( $slugs_node, $preset_metadata['path'], array() );
		...
		foreach ( $style_nodes as $style_node ) {
			...
			$content               = _fp_array_get( $incoming_data, $background_image_path, null );
			if ( isset( $content ) ) {
				_fp_array_set( $this->theme_json, $background_image_path, $content );
			}
		}
	}
		...
		foreach ( $setting_nodes as $metadata ) {
			...
			foreach ( $origins as $origin ) {
				...
				foreach ( $duotone_presets[ $origin ] as $duotone_preset ) {
					$filters .= fp_get_duotone_filter_svg( $duotone_preset );
				}
			}
		}
	...
	protected static function should_override_preset( $theme_json, $path, $override ) {
		...
		if ( is_array( $override ) ) {
			$value = _fp_array_get( $theme_json, array_merge( $path, $override ) );
			if ( isset( $value ) ) {
				return ! $value;
			}

			// Search the top-level key if none was found for this node.
			$value = _fp_array_get( $theme_json, array_merge( array( 'settings' ), $override ) );
			if ( isset( $value ) ) {
				return ! $value;
			}
	...
	protected static function get_default_slugs( $data, $node_path ) {
		...
		foreach ( static::PRESETS_METADATA as $metadata ) {
			...
			$preset = _fp_array_get( $data, $path, null );
			if ( ! isset( $preset ) ) {
				continue;
			}
			...
			_fp_array_set( $slugs, $metadata['path'], $slugs_for_preset );
		}
	...
	protected function get_name_from_defaults( $slug, $base_path ) {
		$path            = $base_path;
		$path[]          = 'default';
		$default_content = _fp_array_get( $this->theme_json, $path, null );
		if ( ! $default_content ) {
			return null;
		}
	...
	public static function remove_insecure_properties( $theme_json, $origin = 'theme' ) {
		...
		$theme_json = FP_Theme_JSON_Schema_Gutenberg::migrate( $theme_json, $origin );
		...
		foreach ( $style_nodes as $metadata ) {
			$input = _fp_array_get( $theme_json, $metadata['path'], array() );
			if ( empty( $input ) ) {
				continue;
			}
			...
			if ( ! empty( $output ) ) {
				_fp_array_set( $sanitized, $metadata['path'], $output );
			}

			if ( isset( $metadata['variations'] ) ) {
				foreach ( $metadata['variations'] as $variation ) {
					$variation_input = _fp_array_get( $theme_json, $variation['path'], array() );
					...
					if ( ! empty( $variation_output ) ) {
						_fp_array_set( $sanitized, $variation['path'], $variation_output );
					}
				}
			}
		}
	...
	protected static function remove_insecure_settings( $input ) {
		...
		foreach ( static::PRESETS_METADATA as $preset_metadata ) {
			foreach ( static::VALID_ORIGINS as $origin ) {
				...
				$presets            = _fp_array_get( $input, $path_with_origin, null );
				if ( null === $presets ) {
					continue;
				}
				...
				if ( ! empty( $escaped_preset ) ) {
					_fp_array_set( $output, $path_with_origin, $escaped_preset );
				}
			}
		}
	...
	protected static function remove_insecure_styles( $input ) {
		...
		foreach ( $declarations as $declaration ) {
			if ( static::is_safe_css_declaration( $declaration['name'], $declaration['value'] ) ) {
				...
				$value = _fp_array_get( $input, $path, array() );
				if ( ! is_array( $value ) ) {
					_fp_array_set( $output, $path, $value );
				}
			}
		}
	...
	private static function remove_indirect_properties( $input, &$output ) {
		foreach ( static::INDIRECT_PROPERTIES_METADATA as $property => $paths ) {
			foreach ( $paths as $path ) {
				$value = _fp_array_get( $input, $path );
				if (
					is_string( $value ) &&
	...
	public function get_data() {
		...
		foreach ( $nodes as $node ) {
			foreach ( static::PRESETS_METADATA as $preset_metadata ) {
				...
				$preset = _fp_array_get( $output, $path, null );
				if ( null === $preset ) {
					continue;
				}
				...
				_fp_array_set( $output, $path, $flattened_preset );
			}
		}
		...
		foreach ( $nodes as $node ) {
			...
			foreach ( static::APPEARANCE_TOOLS_OPT_INS as $opt_in_path ) {
				...
				// Use "unset prop" as a marker instead of "null" because
				// "null" can be a valid value for some props (e.g. blockGap).
				$opt_in_value = _fp_array_get( $output, $full_path, 'unset prop' );
			...
			if ( $all_opt_ins_are_set ) {
				...
				_fp_array_set( $output, $node_path_with_appearance_tools, true );
				foreach ( static::APPEARANCE_TOOLS_OPT_INS as $opt_in_path ) {
					$full_path = $node['path'];
					foreach ( $opt_in_path as $opt_in_path_item ) {
						$full_path[] = $opt_in_path_item;
	...
	public function set_spacing_sizes() {
		...
		_fp_array_set( $this->theme_json, array( 'settings', 'spacing', 'spacingSizes', 'default' ), $spacing_sizes );
	}
	...
	protected static function get_block_selectors( $block_type, $root_selector ) {
		...
		foreach ( static::BLOCK_SUPPORT_FEATURE_LEVEL_SELECTORS as $key => $feature ) {
			$feature_selector = fp_get_block_css_selector( $block_type, $key );
	 ...
	 *
	 * @since 6.3.0
	 * @param FP_Theme_JSON_Gutenberg $theme_json The theme json resolver.
	 *
	 * @return FP_Theme_JSON_Gutenberg The $theme_json with resolved variables.
	 */
