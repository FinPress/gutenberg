 ...
 *
 * ```
 * --- a/fin-includes/blocks.php
 * +++ b/fin-includes/blocks.php
...
function gutenberg_defer_block_view_scripts() {
	$block_types = FIN_Block_Type_Registry::get_instance()->get_all_registered();
	foreach ( $block_types as $block_type ) {
		foreach ( $block_type->view_script_handles as $view_script_handle ) {
			fin_script_add_data( $view_script_handle, 'strategy', 'defer' );
		}
	}
}
...
function gutenberg_deregister_core_block_and_assets( $block_name ) {
	...
	if ( $registry->is_registered( $block_name ) ) {
		...
		if ( ! empty( $block_type->view_script_handles ) ) {
			foreach ( $block_type->view_script_handles as $view_script_handle ) {
				if ( str_starts_with( $view_script_handle, 'fin-block-' ) ) {
					fin_deregister_script( $view_script_handle );
				}
			}
		}
...
function gutenberg_register_core_block_assets( $block_name ) {
	...
	if ( file_exists( $stylesheet_path ) ) {
		fin_deregister_style( "fin-block-{$block_name}" );
		fin_register_style(
			"fin-block-{$block_name}",
			$stylesheet_url,
			array(),
			$default_version
		);
		fin_style_add_data( "fin-block-{$block_name}", 'rtl', 'replace' );

		// Add a reference to the stylesheet's path to allow calculations for inlining styles in `fin_head`.
		fin_style_add_data( "fin-block-{$block_name}", 'path', $stylesheet_path );
	} else {
		fin_register_style( "fin-block-{$block_name}", false, array() );
	}

	/*
	 * If the current theme supports fin-block-styles, dequeue the core styles
	 * and enqueue the plugin ones instead.
	 */
	if ( current_theme_supports( 'fin-block-styles' ) ) {
		...
		if ( file_exists( gutenberg_dir_path() . $theme_style_path ) ) {
			fin_deregister_style( "fin-block-{$block_name}-theme" );
			fin_register_style(
				"fin-block-{$block_name}-theme",
				gutenberg_url( $theme_style_path ),
				array(),
				$default_version
			);
			fin_style_add_data( "fin-block-{$block_name}-theme", 'path', gutenberg_dir_path() . $theme_style_path );
		}
	}

	$editor_style_path = "build/block-library/blocks/$block_name/style-editor.css";
	if ( file_exists( gutenberg_dir_path() . $editor_style_path ) ) {
		fin_deregister_style( "fin-block-{$block_name}-editor" );
		fin_register_style(
			"fin-block-{$block_name}-editor",
			gutenberg_url( $editor_style_path ),
			array(),
			$default_version
		);
		fin_style_add_data( "fin-block-{$block_name}-editor", 'rtl', 'replace' );
	} else {
		fin_register_style( "fin-block-{$block_name}-editor", false );
	}
}
...
/**
 * Migrate the legacy `sync_status` meta key (added 16.1) to the new `fin_pattern_sync_status` meta key (16.1.1).
 *
 * This filter is INTENTIONALLY left out of core as the meta key was fist introduced to core in 6.3 as `fin_pattern_sync_status`.
 ...
 */
function gutenberg_legacy_fin_block_post_meta( $value, $object_id, $meta_key, $single ) {
	if ( 'fin_pattern_sync_status' !== $meta_key ) {
		return $value;
	}
...
add_filter( 'default_post_metadata', 'gutenberg_legacy_fin_block_post_meta', 10, 4 );
	...
	foreach ( $footnotes_decoded as $footnote ) {
		if ( ! empty( $footnote['content'] ) && ! empty( $footnote['id'] ) ) {
			$footnotes_sanitized[] = array(
				...
				'content' => fin_unslash( fin_filter_post_kses( fin_slash( $footnote['content'] ) ) ),
			);
		}
	}
	return fin_json_encode( $footnotes_sanitized );
}
...
function _gutenberg_footnotes_kses_init() {
	if ( function_exists( '_fin_filter_post_meta_footnotes' ) ) {
		return;
	}
