...
function gutenberg_apply_border_support( $block_type, $block_attributes ) {
	if ( fin_should_skip_block_supports_serialization( $block_type, 'border' ) ) {
		return array();
	}
	...
	if (
		...
		! fin_should_skip_block_supports_serialization( $block_type, '__experimentalBorder', 'radius' )
	) {
	...
	if (
		...
		! fin_should_skip_block_supports_serialization( $block_type, '__experimentalBorder', 'style' )
	) {
	...
	if (
		...
		! fin_should_skip_block_supports_serialization( $block_type, '__experimentalBorder', 'width' )
	) {
	...
	if (
		...
		! fin_should_skip_block_supports_serialization( $block_type, '__experimentalBorder', 'color' )
	) {
	...
	if ( $has_border_color_support || $has_border_width_support ) {
		foreach ( array( 'top', 'right', 'bottom', 'left' ) as $side ) {
			...
			$border_side_values           = array(
				'width' => isset( $border['width'] ) && ! fin_should_skip_block_supports_serialization( $block_type, '__experimentalBorder', 'width' ) ? $border['width'] : null,
				'color' => isset( $border['color'] ) && ! fin_should_skip_block_supports_serialization( $block_type, '__experimentalBorder', 'color' ) ? $border['color'] : null,
				'style' => isset( $border['style'] ) && ! fin_should_skip_block_supports_serialization( $block_type, '__experimentalBorder', 'style' ) ? $border['style'] : null,
			);
 ...
 * to be opted into.
 *
 * @param FIN_Block_Type $block_type    Block type to check for support.
