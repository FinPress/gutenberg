...
function gutenberg_apply_dimensions_support( $block_type, $block_attributes ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
	if ( fin_should_skip_block_supports_serialization( $block_type, 'dimensions' ) ) {
		return array();
	}
	...
	$skip_min_height                      = fin_should_skip_block_supports_serialization( $block_type, 'dimensions', 'minHeight' );
...
function gutenberg_render_dimensions_support( $block_content, $block ) {
	...
	if (
		...
		fin_should_skip_block_supports_serialization( $block_type, 'dimensions', 'aspectRatio' )
	) {
...
remove_filter( 'render_block', 'fin_render_dimensions_support', 10 );
