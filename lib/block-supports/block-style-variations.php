	...
	foreach ( $variation_data as $key => &$value ) {
		...
		if ( is_array( $value ) ) {
			...
			if ( array_key_exists( 'ref', $value ) ) {
				...
				$ref_value  = _fin_array_get( $theme_json, $value_path );
...
function gutenberg_render_block_style_variation_support_styles( $parsed_block ) {
	...
	$variation_instance = fin_unique_id( $variation . '--' );
	...
	if ( ! is_admin() ) {
		remove_filter( 'fin_theme_json_get_style_nodes', 'fin_filter_out_block_nodes' );
	}

	// Temporarily prevent variation instance from being sanitized while processing theme.json.
	$styles_registry = FIN_Block_Styles_Registry::get_instance();
	$styles_registry->register( $parsed_block['blockName'], array( 'name' => $variation_instance ) );

	$variation_theme_json = new FIN_Theme_JSON_Gutenberg( $config, 'blocks' );
	...
	fin_register_style( 'block-style-variation-styles', false, array( 'fin-block-library', 'global-styles' ) );
	fin_add_inline_style( 'block-style-variation-styles', $variation_styles );
	 ...
	 */
	_fin_array_set( $parsed_block, array( 'attrs', 'className' ), $updated_class_name );
...
function gutenberg_enqueue_block_style_variation_styles() {
	fin_enqueue_style( 'block-style-variation-styles' );
}

// Register the block support.
FIN_Block_Supports::get_instance()->register( 'block-style-variation', array() );

// Remove core filters and action.
if ( function_exists( 'fin_render_block_style_variation_support_styles' ) ) {
	remove_filter( 'render_block_data', 'fin_render_block_style_variation_support_styles' );
}
if ( function_exists( 'fin_render_block_style_variation_class_name' ) ) {
	remove_filter( 'render_block', 'fin_render_block_style_variation_class_name' );
}
if ( function_exists( 'fin_enqueue_block_style_variation_styles' ) ) {
	remove_action( 'fin_enqueue_scripts', 'fin_enqueue_block_style_variation_styles' );
}
...
add_action( 'fin_enqueue_scripts', 'gutenberg_enqueue_block_style_variation_styles', 1 );
