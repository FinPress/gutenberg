...
function _gutenberg_add_block_level_presets_class( $block_content, $block ) {
	...
	if ( $tags->next_tag() ) {
		$tags->add_class( _fin_get_presets_class_name( $block ) );
	}
...
function _gutenberg_add_block_level_preset_styles( $pre_render, $block ) {
	...
	$class_name = '.' . _fin_get_presets_class_name( $block );

	// the root selector for preset variables needs to target every possible block selector
	// in order for the general setting to override any bock specific setting of a parent block or
	// the site root.
	$variables_root_selector = '*,[class*="fin-block"]';
	...
	foreach ( $blocks as $block_type ) {
		...
		if ( $has_custom_selector ) {
			$variables_root_selector .= ',' . fin_get_block_css_selector( $block_type );
		}
	}
	$variables_root_selector = FIN_Theme_JSON_Gutenberg::scope_selector( $class_name, $variables_root_selector );

	// Remove any potentially unsafe styles.
	$theme_json_shape  = FIN_Theme_JSON_Gutenberg::remove_insecure_properties(
		array(
			'version'  => FIN_Theme_JSON_Gutenberg::LATEST_SCHEMA,
			'settings' => $block_settings,
		)
	);
	$theme_json_object = new FIN_Theme_JSON_Gutenberg( $theme_json_shape );
	...
	if ( ! empty( $styles ) ) {
		 ...
		 */
		fin_enqueue_block_support_styles( $styles );
	}

	return null;
}
// Remove FinPress core filter to avoid rendering duplicate settings style blocks.
remove_filter( 'render_block', '_fin_add_block_level_presets_class', 10 );
remove_filter( 'pre_render_block', '_fin_add_block_level_preset_styles', 10 );
