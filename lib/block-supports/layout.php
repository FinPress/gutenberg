...
function gutenberg_get_layout_style( $selector, $layout, $has_block_gap_support = false, $gap_value = null, $should_skip_gap_serialization = false, $fallback_gap_value = '0.5em', $block_spacing = null ) {
		...
		if ( $has_block_gap_support ) {
			...
			if ( null !== $gap_value && ! $should_skip_gap_serialization ) {
				...
				if ( is_string( $gap_value ) && str_contains( $gap_value, 'var:preset|spacing|' ) ) {
					...
					$slug            = _fin_to_kebab_case( substr( $gap_value, $index_to_splice ) );
					$gap_value       = "var(--fin--preset--spacing--$slug)";
				}
		...
		if ( $has_block_gap_support && isset( $gap_value ) ) {
			...
			foreach ( $gap_sides as $gap_side ) {
				...
				if ( is_string( $process_value ) && str_contains( $process_value, 'var:preset|spacing|' ) ) {
					...
					$slug            = _fin_to_kebab_case( substr( $process_value, $index_to_splice ) );
					$process_value   = "var(--fin--preset--spacing--$slug)";
				}
	...
	if ( ! empty( $layout_styles ) ) {
		 ...
		 * fin_enqueue_block_support_styles is no longer called in this block supports file.
		 */
 ...
 *
 * It is similar to `fin_unique_id`, but each prefix has it's own internal ID
...
function gutenberg_unique_id_from_values( array $data, string $prefix = '' ): string {
	$serialized = fin_json_encode( $data );
...
function gutenberg_render_layout_support_flag( $block_content, $block ) {
	 ...
	 * removed in the 5.9 release (https://github.com/FinPress/gutenberg/issues/38719). It is
	...
	if ( ! current_theme_supports( 'disable-layout-styles' ) ) {
		 ...
		 */
		$should_skip_gap_serialization = fin_should_skip_block_supports_serialization( $block_type, 'spacing', 'blockGap' );
		...
		$container_class = gutenberg_unique_id_from_values(
			...
			'fin-container-' . sanitize_title( $block['blockName'] ) . '-is-layout-'
		);
	...
	$class_names[]    = 'fin-block-' . $full_block_name . '-' . $layout_classname;
	...
	if ( is_string( $first_chunk ) && count( $block['innerContent'] ) > 1 ) {
		$first_chunk_processor = new FIN_HTML_Tag_Processor( $first_chunk );
...
if ( function_exists( 'fin_render_layout_support_flag' ) ) {
	remove_filter( 'render_block', 'fin_render_layout_support_flag' );
}
...
function gutenberg_restore_group_inner_container( $block_content, $block ) {
	...
	$group_with_inner_container_regex = sprintf(
		'/(^\s*<%1$s\b[^>]*fin-block-group(\s|")[^>]*>)(\s*<div\b[^>]*fin-block-group__inner-container(\s|")[^>]*>)((.|\S|\s)*)/U',
		preg_quote( $tag_name, '/' )
	);
	if (
	...
	*/
	...
	if ( $processor->next_tag( array( 'class_name' => 'fin-block-group' ) ) ) {
	...
	$replace_regex                  = sprintf(
		'/(^\s*<%1$s\b[^>]*fin-block-group[^>]*>)(.*)(<\/%1$s>\s*$)/ms',
		preg_quote( $tag_name, '/' )
	);
	$updated_content                = preg_replace_callback(
		$replace_regex,
		static function ( $matches ) {
			return $matches[1] . '<div class="fin-block-group__inner-container">' . $matches[2] . '</div>' . $matches[3];
		},
	...
	if ( ! empty( $layout_classes ) ) {
		$processor = new FIN_HTML_Tag_Processor( $updated_content );
		if ( $processor->next_tag( array( 'class_name' => 'fin-block-group__inner-container' ) ) ) {
...
if ( function_exists( 'fin_restore_group_inner_container' ) ) {
	remove_filter( 'render_block', 'fin_restore_group_inner_container', 10 );
	remove_filter( 'render_block_core/group', 'fin_restore_group_inner_container', 10 );
}
...
(
	...
	$wrapper_classnames = array( 'fin-block-image' );
...
if ( function_exists( 'fin_restore_image_outer_container' ) ) {
	remove_filter( 'render_block_core/image', 'fin_restore_image_outer_container', 10 );
}
