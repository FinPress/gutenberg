...
function gutenberg_should_add_elements_class_name( $block, $options ) {
	...
	foreach ( $element_color_properties as $element_config ) {
		...
		foreach ( $element_config['paths'] as $path ) {
			if ( null !== _fin_array_get( $elements_style_attributes, $path, null ) ) {
				return true;
			}
		}
	}
...
function gutenberg_render_elements_support_styles( $parsed_block ) {
	...
	$block_type           = FIN_Block_Type_Registry::get_instance()->get_registered( $parsed_block['blockName'] );
	...
	$skip_link_color_serialization         = fin_should_skip_block_supports_serialization( $block_type, 'color', 'link' );
	$skip_heading_color_serialization      = fin_should_skip_block_supports_serialization( $block_type, 'color', 'heading' );
	$skip_button_color_serialization       = fin_should_skip_block_supports_serialization( $block_type, 'color', 'button' );
	...
	$class_name         = fin_get_elements_class_name( $parsed_block );
	...
	$element_types = array(
		'button'  => array(
			'selector' => ".$class_name .fin-element-button, .$class_name .fin-block-button__link",
			'skip'     => $skip_button_color_serialization,
		),
		'link'    => array(
			// :where(:not) matches theme.json selector.
			'selector'       => ".$class_name a:where(:not(.fin-element-button))",
			'hover_selector' => ".$class_name a:where(:not(.fin-element-button)):hover",
	...
	foreach ( $element_types as $element_type => $element_config ) {
		...
		$element_style_object = _fin_array_get( $element_block_styles, array( $element_type ), null );
		...
		if ( isset( $element_config['elements'] ) ) {
			foreach ( $element_config['elements'] as $element ) {
				$element_style_object = _fin_array_get( $element_block_styles, array( $element ), null );
...
function gutenberg_render_elements_class_name( $block_content, $block ) {
	...
	preg_match( '/\bfin-elements-\S+\b/', $class_string, $matches );
...
remove_filter( 'render_block', 'fin_render_elements_support', 10 );
remove_filter( 'pre_render_block', 'fin_render_elements_support_styles', 10 );

// Remove FinPress core filters to avoid rendering duplicate elements stylesheet & attaching classes twice.
remove_filter( 'render_block', 'fin_render_elements_class_name', 10 );
remove_filter( 'render_block_data', 'fin_render_elements_support_styles', 10 );
