<?php
/**
 * FIN_Theme_JSON_Gutenberg class
 *
 * @package gutenberg
 * @since 5.8.0
 */
...
class FIN_Theme_JSON_Gutenberg {
	 ...
	 *                       Example output: "--fin--preset--duotone--blue: <value>" will generate as many CSS Custom Properties as presets defined
	...
	const PRESETS_METADATA = array(
		array(
			...
			'value_key'         => 'ratio',
			'css_vars'          => '--fin--preset--aspect-ratio--$slug',
			'classes'           => array(),
		...
		array(
			...
			'use_default_names' => false,
			'value_key'         => 'color',
			'css_vars'          => '--fin--preset--color--$slug',
			'classes'           => array(
				'.has-$slug-color'            => 'color',
				'.has-$slug-background-color' => 'background-color',
				'.has-$slug-border-color'     => 'border-color',
		...
		array(
			...
			'use_default_names' => false,
			'value_key'         => 'gradient',
			'css_vars'          => '--fin--preset--gradient--$slug',
		...
		array(
			...
			'use_default_names' => false,
			'value_func'        => null, // CSS Custom Properties for duotone are handled by block supports in class-fin-duotone-gutenberg.php.
			'css_vars'          => null,
			'classes'           => array(),
		...
		array(
			...
			'css_vars'          => '--fin--preset--font-size--$slug',
		...
		array(
			...
			'css_vars'          => '--fin--preset--font-family--$slug',
		...
		array(
			...
			'use_default_names' => true,
			'value_key'         => 'size',
			'css_vars'          => '--fin--preset--spacing--$slug',
			'classes'           => array(),
		...
		array(
			...
			'use_default_names' => false,
			'value_key'         => 'shadow',
			'css_vars'          => '--fin--preset--shadow--$slug',
			'classes'           => array(),
		...
		array(
			...
			'use_default_names' => false,
			'value_key'         => 'size',
			'css_vars'          => '--fin--preset--border-radius--$slug',
			'classes'           => array(),
	 ...
	 *              `letter-spacing`, `margin-*`, `padding-*`, `--fin--style--block-gap`,
	 ...
	 *              `--fin--style--root--padding-*`, and `box-shadow` properties,
	 *              removed the `--fin--style--block-gap` property.
	...
	const PROPERTIES_METADATA = array(
		...
		'--fin--style--root--padding'        => array( 'spacing', 'padding' ),
		'--fin--style--root--padding-top'    => array( 'spacing', 'padding', 'top' ),
		'--fin--style--root--padding-right'  => array( 'spacing', 'padding', 'right' ),
		'--fin--style--root--padding-bottom' => array( 'spacing', 'padding', 'bottom' ),
		'--fin--style--root--padding-left'   => array( 'spacing', 'padding', 'left' ),
	...
	const ELEMENTS = array(
		'link'    => 'a:where(:not(.fin-element-button))', // The `where` is needed to lower the specificity.
		...
		'h5'      => 'h5',
		'h6'      => 'h6',
		// We have the .fin-block-button__link class so that this will target older buttons that have been serialized.
		'button'  => '.fin-element-button, .fin-block-button__link',
		// The block classes are necessary to target older content that won't use the new class names.
		'caption' => '.fin-element-caption, .fin-block-audio figcaption, .fin-block-embed figcaption, .fin-block-gallery figcaption, .fin-block-image figcaption, .fin-block-table figcaption, .fin-block-video figcaption',
		'cite'    => 'cite',
	);
	 ...
	 */
	public function __construct( $theme_json = array( 'version' => FIN_Theme_JSON_Gutenberg::LATEST_SCHEMA ), $origin = 'theme' ) {
		if ( ! in_array( $origin, static::VALID_ORIGINS, true ) ) {
			$origin = 'theme';
		}

		$this->theme_json    = FIN_Theme_JSON_Schema_Gutenberg::migrate( $theme_json, $origin );
		...
		foreach ( $nodes as $node ) {
			foreach ( static::PRESETS_METADATA as $preset_metadata ) {
				...
				$preset = _fin_array_get( $this->theme_json, $path, null );
				if ( null !== $preset ) {
					// If the preset is not already keyed by origin.
					if ( isset( $preset[0] ) || empty( $preset ) ) {
						_fin_array_set( $this->theme_json, $path, array( $origin => $preset ) );
					}
				}
			}
		}
		...
		$spacing_scale = _fin_array_get( $this->theme_json, $scale_path, null );
		if ( null !== $spacing_scale ) {
			...
			if ( empty( array_intersect( array_keys( $spacing_scale ), static::VALID_ORIGINS ) ) ) {
				_fin_array_set( $this->theme_json, $scale_path, array( $origin => $spacing_scale ) );
			}
		}
		...
		if ( isset( $spacing_scale ) ) {
			...
			$spacing_sizes        = _fin_array_get( $this->theme_json, $sizes_path, array() );
			...
			_fin_array_set( $this->theme_json, $sizes_path, $merged_spacing_sizes );
		}
	}
		...
		foreach ( $valid_variations as $block_type => $registered_variations ) {
			foreach ( $registered_variations as $variation_name ) {
				...
				if ( ! empty( $merged_data ) ) {
					_fin_array_set( $new_theme_json, array( 'styles', 'blocks', $block_type, 'variations', $variation_name ), $merged_data );
				}
			}
		}
	...
	protected static function do_opt_in_into_settings( &$context ) {
		foreach ( static::APPEARANCE_TOOLS_OPT_INS as $path ) {
			...
			if ( 'unset prop' === _fin_array_get( $context, $path, 'unset prop' ) ) {
				_fin_array_set( $context, $path, true );
			}
		}
	...
	protected static function get_blocks_metadata() {
		...
		$style_registry = FIN_Block_Styles_Registry::get_instance();

		// Is there metadata for all currently registered blocks?
		$blocks = array_diff_key( $blocks, static::$blocks_metadata );
		if ( empty( $blocks ) ) {
			/*
			 * New block styles may have been registered within FIN_Block_Styles_Registry.
		...
		foreach ( $blocks as $block_name => $block_type ) {
			$root_selector = fin_get_block_css_selector( $block_type );
			...
			$duotone_selector = fin_get_block_css_selector( $block_type, 'filter.duotone' );
			...
			// Block style variations can be registered through the FIN_Block_Styles_Registry as well as block.json.
	...
	public function get_stylesheet( $types = array( 'variables', 'styles', 'presets' ), $origins = null, $options = array() ) {
		...
		if ( in_array( 'styles', $types, true ) ) {
			...
			$columns_selector       = '.fin-block-columns';
			$post_template_selector = '.fin-block-post-template';
			if ( ! empty( $options['scope'] ) ) {
		...
		if ( in_array( 'custom-css', $types, true ) ) {
			...
			$stylesheet .= _fin_array_get( $this->theme_json, array( 'styles', 'css' ) );
		}
	...
	protected function get_layout_styles( $block_metadata, $types = array() ) {
		...
		if ( isset( $block_metadata['name'] ) ) {
			$block_type = FIN_Block_Type_Registry::get_instance()->get_registered( $block_metadata['name'] );
		...
		$node                     = _fin_array_get( $this->theme_json, $block_metadata['path'], array() );
	...
	protected function get_preset_classes( $setting_nodes, $origins ) {
		...
		foreach ( $setting_nodes as $metadata ) {
			...
			$node          = _fin_array_get( $this->theme_json, $metadata['path'], array() );
	 ...
	 *
	 *     block-selector {
	 *       --fin--preset--category--slug: value;
	 *       --fin--custom--variable: value;
	 *     }
	 *
	 * @since 5.8.0
	...
	protected static function get_settings_values_by_slug( $settings, $preset_metadata, $origins ) {
		$preset_per_origin = _fin_array_get( $settings, $preset_metadata['path'], array() );
		...
		foreach ( $origins as $origin ) {
			...
			foreach ( $preset_per_origin[ $origin ] as $preset ) {
				$slug = _fin_to_kebab_case( $preset['slug'] );
	 ...
	 * @return string The CSS Custom Property. Something along the lines of `--fin--preset--color--black`.
	 */
	 ...
	 *
	 * For example, assuming the given prefix is '--fin'
	 ...
	 *
	 *     {
	 *       '--fin--some-property': 'value',
	 *       '--fin--nested-property--sub-property': 'value'
	 *     }
	 *
	 * @since 5.8.0
	 *
	...
	protected static function flatten_tree( $tree, $prefix = '', $token = '--' ) {
		...
		foreach ( $tree as $property => $value ) {
			$new_key = $prefix . str_replace(
				'/',
				'-',
				strtolower( _fin_to_kebab_case( $property ) )
			);
	 ...
	 * @since 6.6.0 Passing current theme JSON settings to fin_get_typography_font_size_value(). Using style engine to correctly fetch background CSS values.
	...
	protected static function compute_style_properties( $styles, $settings = array(), $properties = null, $theme_json = null, $selector = null, $use_root_padding = null ) {
		...
		$root_style_length        = strlen( '--fin--style--root--' );
		...
		foreach ( $properties as $css_property => $value_path ) {
			...
			$is_root_style = str_starts_with( $css_property, '--fin--style--root--' );
			...
			if ( '--fin--style--root--padding' === $css_property && is_string( $value ) ) {
				continue;
			}
			...
			if (
				...
				_fin_array_get( $settings, static::PROTECTED_PROPERTIES[ $path_string ], null ) === null
			) {
				continue;
			}
			...
			if ( 'font-size' === $css_property ) {
				/*
				 * fin_get_typography_font_size_value() will check
	...
	protected static function get_property_value( $styles, $path, $theme_json = null ) {
		$value = _fin_array_get( $styles, $path, '' );
		...
		if ( is_array( $value ) && isset( $value['ref'] ) ) {
			...
			$ref_value  = _fin_array_get( $theme_json, $value_path, null );
	...
	protected static function get_style_nodes( $theme_json, $selectors = array(), $options = array() ) {
		 ...
		 */
		return apply_filters( 'fin_theme_json_get_style_nodes', $nodes );
	}
	...
	public function get_styles_for_block( $block_metadata ) {
		...
		if ( ! empty( $block_metadata['variations'] ) ) {
			foreach ( $block_metadata['variations'] as $style_variation ) {
				$style_variation_node           = _fin_array_get( $this->theme_json, $style_variation['path'], array() );
				...
				foreach ( $variation_declarations as $current_selector => $new_declarations ) {
					 ...
					 * - `.fin-block-list:not(.fin-block-list .fin-block-list)`
					 * - `.fin-block-image img, .fin-block-image.my-class img`
					 */
		 ...
		 * set the min-height to '100%'. Minus `--fin-admin--admin-bar--height` for logged-in view.
		...
		if ( $should_set_root_min_height ) {
			$block_rules .= static::to_ruleset(
				...
				array(
					array(
						'name'  => 'min-height',
						'value' => 'calc(100% - var(--fin-admin--admin-bar--height, 0px))',
					),
				)
			);
		}
		...
		if ( '.fin-block-separator' === $selector ) {
	...
	public function get_root_layout_rules( $selector, $block_metadata ) {
		...
		*/
		if ( isset( $settings['layout']['contentSize'] ) || isset( $settings['layout']['wideSize'] ) ) {
			...
			$css         .= static::ROOT_CSS_PROPERTIES_SELECTOR . ' { --fin--style--global--content-size: ' . $content_size . ';';
			$css         .= '--fin--style--global--wide-size: ' . $wide_size . '; }';
		}
		...
		*/
		...
		if ( $use_root_padding ) {
			...
			$css .= '.fin-site-blocks { padding-top: var(--fin--style--root--padding-top); padding-bottom: var(--fin--style--root--padding-bottom); }';
			// Right and left padding are applied to the first container with `.has-global-padding` class.
			$css .= '.has-global-padding { padding-right: var(--fin--style--root--padding-right); padding-left: var(--fin--style--root--padding-left); }';
			// Alignfull children of the container with left and right padding have negative margins so they can still be full width.
			$css .= '.has-global-padding > .alignfull { margin-right: calc(var(--fin--style--root--padding-right) * -1); margin-left: calc(var(--fin--style--root--padding-left) * -1); }';
			...
			$css .= '.has-global-padding :where(:not(.alignfull.is-layout-flow) > .has-global-padding:not(.fin-block-block, .alignfull)) { padding-right: 0; padding-left: 0; }';
			// Alignfull direct children of the containers that are targeted by the rule above do not need negative margins.
			$css .= '.has-global-padding :where(:not(.alignfull.is-layout-flow) > .has-global-padding:not(.fin-block-block, .alignfull)) > .alignfull { margin-left: 0; margin-right: 0; }';
		}

		$css .= '.fin-site-blocks > .alignleft { float: left; margin-right: 2em; }';
		$css .= '.fin-site-blocks > .alignright { float: right; margin-left: 2em; }';
		$css .= '.fin-site-blocks > .aligncenter { justify-content: center; margin-left: auto; margin-right: auto; }';
		...
		if ( isset( $this->theme_json['settings']['spacing']['blockGap'] ) ) {
			...
			$css            .= ":where(.fin-site-blocks) > * { margin-block-start: $block_gap_value; margin-block-end: 0; }";
			$css            .= ':where(.fin-site-blocks) > :first-child { margin-block-start: 0; }';
			$css            .= ':where(.fin-site-blocks) > :last-child { margin-block-end: 0; }';

			// For backwards compatibility, ensure the legacy block gap CSS variable is still available.
			$css .= static::ROOT_CSS_PROPERTIES_SELECTOR . " { --fin--style--block-gap: $block_gap_value; }";
		}
	...
	protected static function get_metadata_boolean( $data, $path, $default_value = false ) {
		...
		if ( is_array( $path ) ) {
			$value = _fin_array_get( $data, $path );
			if ( null !== $value ) {
				return $value;
			}
		}
	 ...
	 *
	 * @param FIN_Theme_JSON_Gutenberg $incoming Data to merge.
	 */
	public function merge( $incoming ) {
		...
		foreach ( static::VALID_ORIGINS as $origin ) {
			...
			$base_spacing_scale      = _fin_array_get( $this->theme_json, $scale_path, array() );
			$flattened_spacing_scale = array_replace( $flattened_spacing_scale, $base_spacing_scale );

			$spacing_scale = _fin_array_get( $incoming_data, $scale_path, null );
			if ( ! isset( $spacing_scale ) ) {
				continue;
			}
			...
			$sizes_path           = array( 'settings', 'spacing', 'spacingSizes', $origin );
			$spacing_sizes        = _fin_array_get( $incoming_data, $sizes_path, array() );
			$spacing_scale_sizes  = static::compute_spacing_sizes( $flattened_spacing_scale );
			$merged_spacing_sizes = static::merge_spacing_sizes( $spacing_scale_sizes, $spacing_sizes );

			_fin_array_set( $incoming_data, $sizes_path, $merged_spacing_sizes );
		}
		...
		foreach ( $nodes as $node ) {
			...
			$content = _fin_array_get( $incoming_data, $path, null );
			if ( isset( $content ) ) {
				_fin_array_set( $this->theme_json, $path, $content );
			}
			...
			foreach ( static::PRESETS_METADATA as $preset_metadata ) {
				...
				if ( is_array( $prevent_override ) ) {
					$prevent_override = _fin_array_get( $this->theme_json['settings'], $preset_metadata['prevent_override'] );
				}
				...
				foreach ( static::VALID_ORIGINS as $origin ) {
					...
					if ( 'theme' === $origin && $prevent_override ) {
						...
						$preset_global = _fin_array_get( $slugs_global, $preset_metadata['path'], array() );
						$preset_node   = _fin_array_get( $slugs_node, $preset_metadata['path'], array() );
		...
		foreach ( $style_nodes as $style_node ) {
			...
			$content               = _fin_array_get( $incoming_data, $background_image_path, null );
			if ( isset( $content ) ) {
				_fin_array_set( $this->theme_json, $background_image_path, $content );
			}
		}
	}
		...
		foreach ( $setting_nodes as $metadata ) {
			...
			foreach ( $origins as $origin ) {
				...
				foreach ( $duotone_presets[ $origin ] as $duotone_preset ) {
					$filters .= fin_get_duotone_filter_svg( $duotone_preset );
				}
			}
		}
	...
	protected static function should_override_preset( $theme_json, $path, $override ) {
		...
		if ( is_array( $override ) ) {
			$value = _fin_array_get( $theme_json, array_merge( $path, $override ) );
			if ( isset( $value ) ) {
				return ! $value;
			}

			// Search the top-level key if none was found for this node.
			$value = _fin_array_get( $theme_json, array_merge( array( 'settings' ), $override ) );
			if ( isset( $value ) ) {
				return ! $value;
			}
	...
	protected static function get_default_slugs( $data, $node_path ) {
		...
		foreach ( static::PRESETS_METADATA as $metadata ) {
			...
			$preset = _fin_array_get( $data, $path, null );
			if ( ! isset( $preset ) ) {
				continue;
			}
			...
			_fin_array_set( $slugs, $metadata['path'], $slugs_for_preset );
		}
	...
	protected function get_name_from_defaults( $slug, $base_path ) {
		$path            = $base_path;
		$path[]          = 'default';
		$default_content = _fin_array_get( $this->theme_json, $path, null );
		if ( ! $default_content ) {
			return null;
		}
	...
	public static function remove_insecure_properties( $theme_json, $origin = 'theme' ) {
		...
		$theme_json = FIN_Theme_JSON_Schema_Gutenberg::migrate( $theme_json, $origin );
		...
		foreach ( $style_nodes as $metadata ) {
			$input = _fin_array_get( $theme_json, $metadata['path'], array() );
			if ( empty( $input ) ) {
				continue;
			}
			...
			if ( ! empty( $output ) ) {
				_fin_array_set( $sanitized, $metadata['path'], $output );
			}

			if ( isset( $metadata['variations'] ) ) {
				foreach ( $metadata['variations'] as $variation ) {
					$variation_input = _fin_array_get( $theme_json, $variation['path'], array() );
					...
					if ( ! empty( $variation_output ) ) {
						_fin_array_set( $sanitized, $variation['path'], $variation_output );
					}
				}
			}
		}
	...
	protected static function remove_insecure_settings( $input ) {
		...
		foreach ( static::PRESETS_METADATA as $preset_metadata ) {
			foreach ( static::VALID_ORIGINS as $origin ) {
				...
				$presets            = _fin_array_get( $input, $path_with_origin, null );
				if ( null === $presets ) {
					continue;
				}
				...
				if ( ! empty( $escaped_preset ) ) {
					_fin_array_set( $output, $path_with_origin, $escaped_preset );
				}
			}
		}
	...
	protected static function remove_insecure_styles( $input ) {
		...
		foreach ( $declarations as $declaration ) {
			if ( static::is_safe_css_declaration( $declaration['name'], $declaration['value'] ) ) {
				...
				$value = _fin_array_get( $input, $path, array() );
				if ( ! is_array( $value ) ) {
					_fin_array_set( $output, $path, $value );
				}
			}
		}
	...
	private static function remove_indirect_properties( $input, &$output ) {
		foreach ( static::INDIRECT_PROPERTIES_METADATA as $property => $paths ) {
			foreach ( $paths as $path ) {
				$value = _fin_array_get( $input, $path );
				if (
					is_string( $value ) &&
	...
	public function get_data() {
		...
		foreach ( $nodes as $node ) {
			foreach ( static::PRESETS_METADATA as $preset_metadata ) {
				...
				$preset = _fin_array_get( $output, $path, null );
				if ( null === $preset ) {
					continue;
				}
				...
				_fin_array_set( $output, $path, $flattened_preset );
			}
		}
		...
		foreach ( $nodes as $node ) {
			...
			foreach ( static::APPEARANCE_TOOLS_OPT_INS as $opt_in_path ) {
				...
				// Use "unset prop" as a marker instead of "null" because
				// "null" can be a valid value for some props (e.g. blockGap).
				$opt_in_value = _fin_array_get( $output, $full_path, 'unset prop' );
			...
			if ( $all_opt_ins_are_set ) {
				...
				_fin_array_set( $output, $node_path_with_appearance_tools, true );
				foreach ( static::APPEARANCE_TOOLS_OPT_INS as $opt_in_path ) {
					$full_path = $node['path'];
					foreach ( $opt_in_path as $opt_in_path_item ) {
						$full_path[] = $opt_in_path_item;
	...
	public function set_spacing_sizes() {
		...
		_fin_array_set( $this->theme_json, array( 'settings', 'spacing', 'spacingSizes', 'default' ), $spacing_sizes );
	}
	...
	protected static function get_block_selectors( $block_type, $root_selector ) {
		...
		foreach ( static::BLOCK_SUPPORT_FEATURE_LEVEL_SELECTORS as $key => $feature ) {
			$feature_selector = fin_get_block_css_selector( $block_type, $key );
	 ...
	 * For example, `var:preset|color|vivid-green-cyan` becomes `var(--fin--preset--color--vivid-green-cyan)`.
	 *
	 * @since 6.3.0
			...
			if ( 0 <= strpos( $style, 'var(' ) ) {
				...
				if ( $has_matches ) {
					...
					foreach ( $var_parts[1] as $index => $var_part ) {
						...
						$rule_to_replace = $var_parts[0][ $index ]; // the css rule to replace e.g. var(--fin--preset--color--vivid-green-cyan).
	 ...
	 *
	 * @since 6.3.0
	 * @param FIN_Theme_JSON_Gutenberg $theme_json The theme json resolver.
	 *
	 * @return FIN_Theme_JSON_Gutenberg The $theme_json with resolved variables.
	 */
