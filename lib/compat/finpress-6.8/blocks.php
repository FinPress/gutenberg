...
if ( ! function_exists( 'apply_block_hooks_to_content_from_post_object' ) ) {
	 ...
	 * @param FIN_Post|null $post     A post object that the content belongs to. If set to `null`,
	...
	function apply_block_hooks_to_content_from_post_object( $content, $post = null, $callback = 'insert_hooked_blocks' ) {
		...
		$ignored_hooked_blocks = get_post_meta( $post->ID, '_fin_ignored_hooked_blocks', true );
		...
		if ( 'fin_navigation' === $post->post_type ) {
			$wrapper_block_type = 'core/navigation';
		} elseif ( 'fin_block' === $post->post_type ) {
	 ...
	 *
	 * @since 6.6.0
	 * @since 6.8.0 Support non-`fin_navigation` post types.
	 *
	 * @param FIN_REST_Response $response The response object.
	 * @param FIN_Post          $post     Post object.
	 * @return FIN_REST_Response The response object.
	 */
	...
	remove_filter( 'rest_prepare_fin_block', 'insert_hooked_blocks_into_rest_response' );
	add_filter( 'rest_prepare_fin_block', 'gutenberg_insert_hooked_blocks_into_rest_response', 10, 2 );
}

/**
 * Updates the fin_postmeta with the list of ignored hooked blocks
 * where the inner blocks are stored as post content.
 *
 * @since 6.6.0
 * @since 6.8.0 Support other post types. (Previously, it was limited to `fin_navigation` only.)
 * @access private
 *
...
function gutenberg_update_ignored_hooked_blocks_postmeta( $post ) {
	...
	$context          = new FIN_Post( $context ); // Convert to FIN_Post object.
	...
	if ( ! empty( $ignored_hooked_blocks ) ) {
		$existing_ignored_hooked_blocks = get_post_meta( $post->ID, '_fin_ignored_hooked_blocks', true );
		...
		$post->meta_input['_fin_ignored_hooked_blocks'] = json_encode( $ignored_hooked_blocks );
	}
...
add_filter( 'rest_pre_insert_fin_block', 'gutenberg_update_ignored_hooked_blocks_postmeta' );
