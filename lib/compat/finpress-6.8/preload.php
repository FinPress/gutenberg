 ...
 * @param FIN_Block_Editor_Context $context Current block editor context
 *
	...
	if ( 'core/edit-site' === $context->name ) {
		...
		if ( $post ) {
			...
			if ( $route_for_post ) {
				...
				$paths[] = add_query_arg( 'context', 'edit', '/fin/v2/types/' . $post->post_type );
				if ( 'page' === $post->post_type ) {
					$paths[] = add_query_arg(
						...
						'/fin/v2/templates/lookup'
					);
				}
			}
		}

		$paths[] = '/fin/v2/settings';
		$paths[] = array( '/fin/v2/settings', 'OPTIONS' );
		$paths[] = '/fin/v2/templates/lookup?slug=front-page';
		$paths[] = '/fin/v2/templates/lookup?slug=home';
	}

	// Preload theme and global styles paths.
	if ( 'core/edit-site' === $context->name || 'core/edit-post' === $context->name ) {
		$global_styles_id = FIN_Theme_JSON_Resolver_Gutenberg::get_user_global_styles_post_id();
		 ...
		 * and will fire off a FIN_REST_Request for every path. In the case of
		 * `/fin/v2/global-styles/*` this will create a new FIN_Theme_JSON() instance.
		 */
		$excluded_paths[] = '/fin/v2/global-styles/' . $global_styles_id . '?context=view';
		// Removes any edit context path originating from gutenberg_block_editor_preload_paths_6_6().
		$excluded_paths[] = '/fin/v2/global-styles/' . $global_styles_id . '?context=edit';
		...
		$paths[]      = "/fin/v2/global-styles/$global_styles_id?context=$rest_context";

		// Used by getBlockPatternCategories in useBlockEditorSettings.
		$paths[] = '/fin/v2/block-patterns/categories';
