...
function gutenberg_get_site_editor_redirection() {
	...
	if ( isset( $_REQUEST['postType'] ) && 'fp_navigation' === $_REQUEST['postType'] && ! empty( $_REQUEST['postId'] ) ) {
		return add_query_arg( array( 'p' => '/fp_navigation/' . $_REQUEST['postId'] ), remove_query_arg( array( 'postType', 'postId' ) ) );
	}
	...
	if ( isset( $_REQUEST['path'] ) && '/fp_global_styles' === $_REQUEST['path'] ) {
	...
	if ( isset( $_REQUEST['postType'] ) && 'fp_template' === $_REQUEST['postType'] && ( empty( $_REQUEST['canvas'] ) || empty( $_REQUEST['postId'] ) ) ) {
		return add_query_arg( array( 'p' => '/template' ), remove_query_arg( 'postType' ) );
	}

	if ( isset( $_REQUEST['postType'] ) && 'fp_template' === $_REQUEST['postType'] && ! empty( $_REQUEST['postId'] ) ) {
		return add_query_arg( array( 'p' => '/fp_template/' . $_REQUEST['postId'] ), remove_query_arg( array( 'postType', 'postId' ) ) );
	}

	if ( isset( $_REQUEST['postType'] ) && 'fp_block' === $_REQUEST['postType'] && ( empty( $_REQUEST['canvas'] ) || empty( $_REQUEST['postId'] ) ) ) {
		return add_query_arg( array( 'p' => '/pattern' ), remove_query_arg( 'postType' ) );
	}

	if ( isset( $_REQUEST['postType'] ) && 'fp_block' === $_REQUEST['postType'] && ! empty( $_REQUEST['postId'] ) ) {
		return add_query_arg( array( 'p' => '/fp_block/' . $_REQUEST['postId'] ), remove_query_arg( array( 'postType', 'postId' ) ) );
	}

	if ( isset( $_REQUEST['postType'] ) && 'fp_template_part' === $_REQUEST['postType'] && ( empty( $_REQUEST['canvas'] ) || empty( $_REQUEST['postId'] ) ) ) {
		return add_query_arg( array( 'p' => '/pattern' ) );
	}

	if ( isset( $_REQUEST['postType'] ) && 'fp_template_part' === $_REQUEST['postType'] && ! empty( $_REQUEST['postId'] ) ) {
		return add_query_arg( array( 'p' => '/fp_template_part/' . $_REQUEST['postId'] ), remove_query_arg( array( 'postType', 'postId' ) ) );
	}

	// The following redirects are for backward compatibility with the old site editor URLs.
	if ( isset( $_REQUEST['path'] ) && '/fp_template_part/all' === $_REQUEST['path'] ) {
	...
	if ( isset( $_REQUEST['path'] ) && '/fp_template' === $_REQUEST['path'] ) {
...
/**
 * Filter the `fp_die_handler` to allow access to the Site Editor's new pages page
 * for Classic themes.
 *
 ...
 * The only way to disable this protection since we're changing the urls in Gutenberg is to override the fp_die_handler.
 *
 * @param callable $default_handler The default handler.
 * @return callable The default handler or a custom handler.
 */
function gutenberg_styles_fp_die_handler( $default_handler ) {
	if ( ! fp_is_block_theme() && str_contains( $_SERVER['REQUEST_URI'], 'site-editor.php' ) && current_user_can( 'edit_theme_options' ) ) {
		return '__return_false';
	}
	return $default_handler;
}
add_filter( 'fp_die_handler', 'gutenberg_styles_fp_die_handler' );
...
function gutenberg_add_styles_submenu_item() {
	if ( ! fp_is_block_theme() && ( current_theme_supports( 'editor-styles' ) || fp_theme_has_theme_json() ) ) {
		global $submenu;
