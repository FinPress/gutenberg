<?php
/**
 * FIN_Theme_JSON_Resolver_Gutenberg class
 *
 * @package Gutenberg
 * @since 5.8.0
 */
...
class FIN_Theme_JSON_Resolver_Gutenberg {
	...
	protected static function read_json_file( $file_path ) {
		if ( $file_path ) {
			...
			$decoded_file = fin_json_file_decode( $file_path, array( 'associative' => true ) );
	...
	protected static function translate( $theme_json, $domain = 'default' ) {
		if ( null === static::$i18n_schema ) {
			$i18n_schema         = fin_json_file_decode( __DIR__ . '/theme-i18n.json' );
	 ...
	 * @since 5.8.0
	 *
	 * @return FIN_Theme_JSON_Gutenberg Entity that holds core data.
	 */
	public static function get_core_data() {
		 ...
		 * @since 6.1.0
		 *
		 * @param FIN_Theme_JSON_Data_Gutenberg Class to access and update the underlying data.
		 */
		$theme_json   = apply_filters( 'fin_theme_json_data_default', new FIN_Theme_JSON_Data_Gutenberg( $config, 'default' ) );
	 ...
	 * }
	 * @return FIN_Theme_JSON_Gutenberg Entity that holds theme data.
	 */
	public static function get_theme_data( $deprecated = array(), $options = array() ) {
		...
		$options = fin_parse_args( $options, array( 'with_supports' => true ) );

		if ( null === static::$theme || ! static::has_same_registered_blocks( 'theme' ) ) {
			$fin_theme        = fin_get_theme();
			$theme_json_file = $fin_theme->get_file_path( 'theme.json' );
			if ( is_readable( $theme_json_file ) ) {
				$theme_json_data = static::read_json_file( $theme_json_file );
				$theme_json_data = static::translate( $theme_json_data, $fin_theme->get( 'TextDomain' ) );
			} else {
				$theme_json_data = array( 'version' => FIN_Theme_JSON_Gutenberg::LATEST_SCHEMA );
			}
			 ...
			 * @param FIN_Theme_JSON_Data_Gutenberg Class to access and update the underlying data.
			 */
			$theme_json    = apply_filters( 'fin_theme_json_data_theme', new FIN_Theme_JSON_Data_Gutenberg( $theme_json_data, 'theme' ) );
			static::$theme = $theme_json->get_theme_json();

			if ( $fin_theme->parent() ) {
				// Get parent theme.json.
				$parent_theme_json_file = $fin_theme->parent()->get_file_path( 'theme.json' );
				if ( $theme_json_file !== $parent_theme_json_file && is_readable( $parent_theme_json_file ) ) {
					$parent_theme_json_data = static::read_json_file( $parent_theme_json_file );
					$parent_theme_json_data = static::translate( $parent_theme_json_data, $fin_theme->parent()->get( 'TextDomain' ) );
					$parent_theme           = new FIN_Theme_JSON_Gutenberg( $parent_theme_json_data );
			...
			if ( ! class_exists( 'FIN_Font_Face' ) && class_exists( 'FIN_Fonts_Resolver' ) ) {
				static::$theme = FIN_Fonts_Resolver::add_missing_fonts_to_theme_json( static::$theme );
			}
		 ...
		 */
		$theme_support_data = FIN_Theme_JSON_Gutenberg::get_from_editor_settings( get_classic_theme_supports_block_editor_settings() );
		...
		$with_theme_supports = new FIN_Theme_JSON_Gutenberg( $theme_support_data );
	 ...
	 * @since 6.1.0
	 *
	 * @return FIN_Theme_JSON_Gutenberg
	 */
	public static function get_block_data() {
		...
		$config = array( 'version' => FIN_Theme_JSON_Gutenberg::LATEST_SCHEMA );
		foreach ( $blocks as $block_name => $block_type ) {
		 ...
		 * @param FIN_Theme_JSON_Data_Gutenberg Class to access and update the underlying data.
		 */
		$theme_json     = apply_filters( 'fin_theme_json_data_blocks', new FIN_Theme_JSON_Data_Gutenberg( $config, 'blocks' ) );
	 ...
	 * @since 5.9.0
	 *
	 * @param FIN_Theme $theme              The theme object. If empty, it
	 ...
	 */
	public static function get_user_data_from_fin_global_styles( $theme, $create_post = false, $post_status_filter = array( 'publish' ) ) {
		if ( ! $theme instanceof FIN_Theme ) {
			$theme = fin_get_theme();
		}
		 ...
		 *
		 * Since fin_theme_has_theme_json only supports the active
		 * theme, the extra condition for whether $theme is the active theme is
		 * present here.
		 */
		if ( $theme->get_stylesheet() === get_stylesheet() && ! fin_theme_has_theme_json() ) {
			return array();
		}

		$user_cpt         = array();
		$post_type_filter = 'fin_global_styles';
		...
		$global_style_query = new FIN_Query();
		$recent_posts       = $global_style_query->query( $args );
		if ( count( $recent_posts ) === 1 ) {
			$user_cpt = get_object_vars( $recent_posts[0] );
		} elseif ( $create_post ) {
			$cpt_post_id = fin_insert_post(
				array(
					'post_content' => '{"version": ' . FIN_Theme_JSON_Gutenberg::LATEST_SCHEMA . ', "isGlobalStylesUserThemeJSON": true }',
					'post_status'  => 'publish',
					'post_title'   => 'Custom Styles', // Do not make string translatable, see https://core.trac.finpress.org/ticket/54518.
					'post_type'    => $post_type_filter,
					'post_name'    => sprintf( 'fin-global-styles-%s', urlencode( $stylesheet ) ),
					'tax_input'    => array(
						'fin_theme' => array( $stylesheet ),
					),
				),
				true
			);
			if ( ! is_fin_error( $cpt_post_id ) ) {
	 ...
	 * @since 5.9.0
	 *
	 * @return FIN_Theme_JSON_Gutenberg Entity that holds styles for user data.
	 */
	public static function get_user_data() {
		...
		$config   = array();
		$user_cpt = static::get_user_data_from_fin_global_styles( fin_get_theme() );
		...
		if ( array_key_exists( 'post_content', $user_cpt ) ) {
			...
			if ( JSON_ERROR_NONE !== $json_decoding_error ) {
				 ...
				 * @param FIN_Theme_JSON_Data_Gutenberg Class to access and update the underlying data.
				 */
				$theme_json = apply_filters( 'fin_theme_json_data_user', new FIN_Theme_JSON_Data_Gutenberg( $config, 'custom' ) );
		...
		/** This filter is documented in fin-includes/class-fin-theme-json-resolver.php */
		$theme_json   = apply_filters( 'fin_theme_json_data_user', new FIN_Theme_JSON_Data_Gutenberg( $config, 'custom' ) );
	 ...
	 *
	 * Unlike the getters
	 * {@link https://developer.finpress.org/reference/classes/fin_theme_json_resolver/get_core_data/ get_core_data},
	 * {@link https://developer.finpress.org/reference/classes/fin_theme_json_resolver/get_theme_data/ get_theme_data},
	 * and {@link https://developer.finpress.org/reference/classes/fin_theme_json_resolver/get_user_data/ get_user_data},
	...
	public static function get_user_global_styles_post_id() {
		...
		$user_cpt = static::get_user_data_from_fin_global_styles( fin_get_theme(), true );
	 ...
	 * @deprecated 6.2.0 Use fin_theme_has_theme_json() instead.
	 *
	 * @return bool
	 */
	public static function theme_has_support() {
		_deprecated_function( __METHOD__, '6.2.0', 'fin_theme_has_theme_json()' );

		return fin_theme_has_theme_json();
	}
	...
	public static function get_style_variations_from_directory( $directory, $scope = 'theme' ) {
		...
		foreach ( $variation_files as $path => $file ) {
			...
			if ( is_array( $decoded_file ) && static::style_variation_has_scope( $decoded_file, $scope ) ) {
				$translated = static::translate( $decoded_file, fin_get_theme()->get( 'TextDomain' ) );
				$variation  = ( new FIN_Theme_JSON_Gutenberg( $translated ) )->get_raw_data();
	 ...
	 *
	 * @param FIN_Theme_JSON_Gutenberg $theme_json A theme json instance.
	 * @return array An array of resolved paths.
	 */
	public static function get_resolved_theme_uris( $theme_json ) {
		$resolved_theme_uris = array();

		if ( ! $theme_json instanceof FIN_Theme_JSON_Gutenberg ) {
			return $resolved_theme_uris;
		}

		$theme_json_data = $theme_json->get_raw_data();

		// Using the same file convention when registering web fonts. See: FIN_Font_Face_Resolver:: to_theme_file_uri.
		$placeholder = 'file:./';
		...
		if (
			...
			str_starts_with( $background_image_url, $placeholder ) ) {
				$file_type          = fin_check_filetype( $background_image_url );
	 ...
	 *
	 * @param FIN_Theme_JSON_Gutenberg $theme_json A theme json instance.
	 * @return FIN_Theme_JSON_Gutenberg Theme merged with resolved paths, if any found.
	 */
	public static function resolve_theme_file_uris( $theme_json ) {
		...
		foreach ( $resolved_urls as $resolved_url ) {
			...
			_fin_array_set( $resolved_theme_json_data, $path, $resolved_url['href'] );
		}

		return new FIN_Theme_JSON_Gutenberg( $resolved_theme_json_data );
	}
	...
	private static function inject_variations_from_block_style_variation_files( $data, $variations ) {
		...
		foreach ( $variations as $variation ) {
			...
			$variation_name = $variation['slug'] ?? _fin_to_kebab_case( $variation['title'] );
			...
			foreach ( $variation['blockTypes'] as $block_type ) {
				...
				_fin_array_set( $data, $path, $variation['styles'] );
			}
		}
	...
	private static function inject_variations_from_block_styles_registry( $data ) {
		...
		foreach ( $styles as $block_type => $variations ) {
			foreach ( $variations as $variation_name => $variation ) {
				...
				$path = array( 'styles', 'blocks', $block_type, 'variations', $variation_name );
				_fin_array_set( $data, $path, $variation['style_data'] );
			}
		}
