<?php
/**
 * FP_Theme_JSON_Resolver_Gutenberg class
 *
 * @package Gutenberg
 * @since 5.8.0
 */
...
class FP_Theme_JSON_Resolver_Gutenberg {
	...
	protected static function read_json_file( $file_path ) {
		if ( $file_path ) {
			...
			$decoded_file = fp_json_file_decode( $file_path, array( 'associative' => true ) );
	...
	protected static function translate( $theme_json, $domain = 'default' ) {
		if ( null === static::$i18n_schema ) {
			$i18n_schema         = fp_json_file_decode( __DIR__ . '/theme-i18n.json' );
	 ...
	 * @since 5.8.0
	 *
	 * @return FP_Theme_JSON_Gutenberg Entity that holds core data.
	 */
	public static function get_core_data() {
		 ...
		 * @since 6.1.0
		 *
		 * @param FP_Theme_JSON_Data_Gutenberg Class to access and update the underlying data.
		 */
		$theme_json   = apply_filters( 'fp_theme_json_data_default', new FP_Theme_JSON_Data_Gutenberg( $config, 'default' ) );
	 ...
	 * }
	 * @return FP_Theme_JSON_Gutenberg Entity that holds theme data.
	 */
	public static function get_theme_data( $deprecated = array(), $options = array() ) {
		...
		$options = fp_parse_args( $options, array( 'with_supports' => true ) );

		if ( null === static::$theme || ! static::has_same_registered_blocks( 'theme' ) ) {
			$fp_theme        = fp_get_theme();
			$theme_json_file = $fp_theme->get_file_path( 'theme.json' );
			if ( is_readable( $theme_json_file ) ) {
				$theme_json_data = static::read_json_file( $theme_json_file );
				$theme_json_data = static::translate( $theme_json_data, $fp_theme->get( 'TextDomain' ) );
			} else {
				$theme_json_data = array( 'version' => FP_Theme_JSON_Gutenberg::LATEST_SCHEMA );
			}
			 ...
			 * @param FP_Theme_JSON_Data_Gutenberg Class to access and update the underlying data.
			 */
			$theme_json    = apply_filters( 'fp_theme_json_data_theme', new FP_Theme_JSON_Data_Gutenberg( $theme_json_data, 'theme' ) );
			static::$theme = $theme_json->get_theme_json();

			if ( $fp_theme->parent() ) {
				// Get parent theme.json.
				$parent_theme_json_file = $fp_theme->parent()->get_file_path( 'theme.json' );
				if ( $theme_json_file !== $parent_theme_json_file && is_readable( $parent_theme_json_file ) ) {
					$parent_theme_json_data = static::read_json_file( $parent_theme_json_file );
					$parent_theme_json_data = static::translate( $parent_theme_json_data, $fp_theme->parent()->get( 'TextDomain' ) );
					$parent_theme           = new FP_Theme_JSON_Gutenberg( $parent_theme_json_data );
			...
			if ( ! class_exists( 'FP_Font_Face' ) && class_exists( 'FP_Fonts_Resolver' ) ) {
				static::$theme = FP_Fonts_Resolver::add_missing_fonts_to_theme_json( static::$theme );
			}
		 ...
		 */
		$theme_support_data = FP_Theme_JSON_Gutenberg::get_from_editor_settings( get_classic_theme_supports_block_editor_settings() );
		...
		$with_theme_supports = new FP_Theme_JSON_Gutenberg( $theme_support_data );
	 ...
	 * @since 6.1.0
	 *
	 * @return FP_Theme_JSON_Gutenberg
	 */
	public static function get_block_data() {
		...
		$config = array( 'version' => FP_Theme_JSON_Gutenberg::LATEST_SCHEMA );
		foreach ( $blocks as $block_name => $block_type ) {
		 ...
		 * @param FP_Theme_JSON_Data_Gutenberg Class to access and update the underlying data.
		 */
		$theme_json     = apply_filters( 'fp_theme_json_data_blocks', new FP_Theme_JSON_Data_Gutenberg( $config, 'blocks' ) );
	 ...
	 */
	public static function get_user_data_from_fp_global_styles( $theme, $create_post = false, $post_status_filter = array( 'publish' ) ) {
		 ...
		 *
		 * Since fp_theme_has_theme_json only supports the active
		 * theme, the extra condition for whether $theme is the active theme is
		 * present here.
		 */
		if ( $theme->get_stylesheet() === get_stylesheet() && ! fp_theme_has_theme_json() ) {
			return array();
		}

		$user_cpt         = array();
		$post_type_filter = 'fp_global_styles';
		...
		if ( count( $recent_posts ) === 1 ) {
			...
			$cpt_post_id = fp_insert_post(
				array(
					'post_content' => '{"version": ' . FP_Theme_JSON_Gutenberg::LATEST_SCHEMA . ', "isGlobalStylesUserThemeJSON": true }',
					'post_status'  => 'publish',
					...
					'tax_input'    => array(
						'fp_theme' => array( $stylesheet ),
					),
				),
				true
			);
			if ( ! is_fp_error( $cpt_post_id ) ) {
	 ...
	 * @since 5.9.0
	 *
	 * @return FP_Theme_JSON_Gutenberg Entity that holds styles for user data.
	 */
	public static function get_user_data() {
		...
		$config   = array();
		$user_cpt = static::get_user_data_from_fp_global_styles( fp_get_theme() );
		...
		if ( array_key_exists( 'post_content', $user_cpt ) ) {
			...
			if ( JSON_ERROR_NONE !== $json_decoding_error ) {
				 ...
				 * @param FP_Theme_JSON_Data_Gutenberg Class to access and update the underlying data.
				 */
				$theme_json = apply_filters( 'fp_theme_json_data_user', new FP_Theme_JSON_Data_Gutenberg( $config, 'custom' ) );
	 ...
	 *
	 * Unlike the getters
	 * {@link https://developer.finpress.org/reference/classes/fp_theme_json_resolver/get_core_data/ get_core_data},
	 * {@link https://developer.finpress.org/reference/classes/fp_theme_json_resolver/get_theme_data/ get_theme_data},
	 * and {@link https://developer.finpress.org/reference/classes/fp_theme_json_resolver/get_user_data/ get_user_data},
	...
	public static function get_user_global_styles_post_id() {
		...
		$user_cpt = static::get_user_data_from_fp_global_styles( fp_get_theme(), true );
	 ...
	 * @deprecated 6.2.0 Use fp_theme_has_theme_json() instead.
	 *
	 * @return bool
	 */
	public static function theme_has_support() {
		_deprecated_function( __METHOD__, '6.2.0', 'fp_theme_has_theme_json()' );

		return fp_theme_has_theme_json();
	}
	...
	public static function get_style_variations_from_directory( $directory, $scope = 'theme' ) {
		...
		foreach ( $variation_files as $path => $file ) {
			...
			if ( is_array( $decoded_file ) && static::style_variation_has_scope( $decoded_file, $scope ) ) {
				$translated = static::translate( $decoded_file, fp_get_theme()->get( 'TextDomain' ) );
				$variation  = ( new FP_Theme_JSON_Gutenberg( $translated ) )->get_raw_data();
	 ...
	 *
	 * @param FP_Theme_JSON_Gutenberg $theme_json A theme json instance.
	 * @return array An array of resolved paths.
	 */
	public static function get_resolved_theme_uris( $theme_json ) {
		$resolved_theme_uris = array();

		if ( ! $theme_json instanceof FP_Theme_JSON_Gutenberg ) {
			return $resolved_theme_uris;
		}

		$theme_json_data = $theme_json->get_raw_data();

		// Using the same file convention when registering web fonts. See: FP_Font_Face_Resolver:: to_theme_file_uri.
		$placeholder = 'file:./';
		...
		if (
			...
			str_starts_with( $background_image_url, $placeholder ) ) {
				$file_type          = fp_check_filetype( $background_image_url );
	 ...
	 *
	 * @param FP_Theme_JSON_Gutenberg $theme_json A theme json instance.
	 * @return FP_Theme_JSON_Gutenberg Theme merged with resolved paths, if any found.
	 */
	public static function resolve_theme_file_uris( $theme_json ) {
		...
		foreach ( $resolved_urls as $resolved_url ) {
			...
			_fp_array_set( $resolved_theme_json_data, $path, $resolved_url['href'] );
		}

		return new FP_Theme_JSON_Gutenberg( $resolved_theme_json_data );
	}
	...
	private static function inject_variations_from_block_style_variation_files( $data, $variations ) {
		...
		foreach ( $variations as $variation ) {
			...
			foreach ( $variation['blockTypes'] as $block_type ) {
				...
				_fp_array_set( $data, $path, $variation['styles'] );
			}
		}
	...
	private static function inject_variations_from_block_styles_registry( $data ) {
		...
		foreach ( $styles as $block_type => $variations ) {
			foreach ( $variations as $variation_name => $variation ) {
				...
				$path = array( 'styles', 'blocks', $block_type, 'variations', $variation_name );
				_fp_array_set( $data, $path, $variation['style_data'] );
			}
		}
