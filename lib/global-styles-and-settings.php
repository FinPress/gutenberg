 ...
 *                     See {@see 'FIN_Theme_JSON::get_stylesheet'} for all valid types.
...
function gutenberg_get_global_stylesheet( $types = array() ) {
	// Ignore cache when `FIN_DEBUG` is enabled, so it doesn't interfere with the theme developers workflow.
	$can_use_cached = empty( $types ) && ! FIN_DEBUG;
	$cache_key      = 'gutenberg_get_global_stylesheet';
	$cache_group    = 'theme_json';
	if ( $can_use_cached ) {
		$cached = fin_cache_get( $cache_key, $cache_group );
		if ( $cached ) {
			return $cached;
		}
	}
	$tree = FIN_Theme_JSON_Resolver_Gutenberg::get_merged_data();
	$tree = FIN_Theme_JSON_Resolver_Gutenberg::resolve_theme_file_uris( $tree );

	$supports_theme_json = fin_theme_has_theme_json();
	...
	if ( in_array( 'variables', $types, true ) ) {
		 ...
		 * @see fin_add_global_styles_for_blocks
		 */
	...
	if ( $can_use_cached ) {
		fin_cache_set( $cache_key, $stylesheet, $cache_group );
	}
	return $stylesheet;
}
...
function gutenberg_get_global_settings( $path = array(), $context = array() ) {
	...
	$settings    = fin_cache_get( $cache_key, $cache_group );

	if ( false === $settings || FIN_DEBUG ) {
		$settings = FIN_Theme_JSON_Resolver_Gutenberg::get_merged_data( $origin )->get_settings();
		fin_cache_set( $cache_key, $settings, $cache_group );
	}

	return _fin_array_get( $settings, $path, $settings );
}

/**
 * Gets the global styles custom css from theme.json.
 *
 * @deprecated Gutenberg 18.6.0 Use {@see 'gutenberg_get_global_stylesheet'} instead for top-level custom CSS, or {@see 'FIN_Theme_JSON_Gutenberg::get_styles_for_block'} for block-level custom CSS.
 *
 * @return string
 */
...
function gutenberg_add_global_styles_block_custom_css() {
	...
	if ( ! fin_theme_has_theme_json() || ! fin_should_load_separate_core_block_assets() ) {
		return;
	}
	...
	foreach ( $block_nodes as $metadata ) {
		...
		/*
		 * When `fin_should_load_separate_core_block_assets()` is true, follow a similar
		...
		if ( isset( $metadata['name'] ) ) {
			if ( str_starts_with( $metadata['name'], 'core/' ) ) {
				...
				$block_handle = 'fin-block-' . $block_name;
				if ( in_array( $block_handle, $fin_styles->queue, true ) ) {
					fin_add_inline_style( $stylesheet_handle, $block_css );
				}
			} else {
				fin_add_inline_style( $stylesheet_handle, $block_css );
			}
...
function gutenberg_add_global_styles_for_blocks() {
	...
	$can_use_cached = ! fin_is_development_mode( 'theme' );
	$update_cache   = false;

	if ( $can_use_cached ) {
		// Hash the merged FIN_Theme_JSON data to bust cache on settings or styles change.
		$cache_hash = md5( fin_json_encode( $tree->get_raw_data() ) );
		$cache_key  = 'fin_styles_for_blocks';
	...
	foreach ( $block_nodes as $metadata ) {
		if ( $can_use_cached ) {
			...
			$cache_node_key = isset( $metadata['name'] ) ? $metadata['name'] : md5( fin_json_encode( $metadata ) );
		...
		if ( ! fin_should_load_separate_core_block_assets() ) {
			fin_add_inline_style( 'global-styles', $block_css );
			continue;
		}
		...
		/*
		 * When `fin_should_load_separate_core_block_assets()` is true, block styles are
		 * enqueued for each block on the page in class FIN_Block's render function.
		...
		if ( ! isset( $metadata['name'] ) && ! empty( $metadata['path'] ) ) {
			$block_name = fin_get_block_name_from_theme_json_path( $metadata['path'] );
			if ( $block_name ) {
...
function _gutenberg_clean_theme_json_caches() {
	fin_cache_delete( 'fin_theme_has_theme_json', 'theme_json' );
	fin_cache_delete( 'gutenberg_get_global_stylesheet', 'theme_json' );
	fin_cache_delete( 'gutenberg_get_global_settings_custom', 'theme_json' );
	fin_cache_delete( 'gutenberg_get_global_settings_theme', 'theme_json' );
	fin_cache_delete( 'gutenberg_get_global_custom_css', 'theme_json' );
	fin_cache_delete( 'gutenberg_get_global_styles_base_custom_css', 'theme_json' );
	FIN_Theme_JSON_Resolver_Gutenberg::clean_cached_data();
}
 ...
 * to "var(--fin--preset--font-size--small)" so consumers don't have to.
...
function gutenberg_get_global_styles( $path = array(), $context = array() ) {
	...
	$merged_data = FIN_Theme_JSON_Resolver_Gutenberg::get_merged_data( $origin );
	if ( $resolve_variables ) {
		$merged_data = FIN_Theme_JSON_Gutenberg::resolve_variables( $merged_data );
	}
	$styles = $merged_data->get_raw_data()['styles'];
	return _fin_array_get( $styles, $path, $styles );
}
...
function gutenberg_get_theme_directory_pattern_slugs() {
	return FIN_Theme_JSON_Resolver_Gutenberg::get_theme_data( array(), array( 'with_supports' => false ) )->get_patterns();
}
