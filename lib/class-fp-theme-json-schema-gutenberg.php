<?php
/**
 * FP_Theme_JSON_Schema_Gutenberg class
 *
 * @package Gutenberg
 * @since 5.9.0
 */

if ( class_exists( 'FP_Theme_JSON_Schema_Gutenberg' ) ) {
	return;
}
...
class FP_Theme_JSON_Schema_Gutenberg {
	...
	private static function rename_settings( &$settings, $paths_to_rename ) {
		foreach ( $paths_to_rename as $original => $renamed ) {
			...
			$current_value = _fp_array_get( $settings, $original_path, null );

			if ( null !== $current_value ) {
				_fp_array_set( $settings, $renamed_path, $current_value );
