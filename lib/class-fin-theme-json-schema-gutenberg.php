<?php
/**
 * FIN_Theme_JSON_Schema_Gutenberg class
 *
 * @package Gutenberg
 * @since 5.9.0
 */

if ( class_exists( 'FIN_Theme_JSON_Schema_Gutenberg' ) ) {
	return;
}
...
class FIN_Theme_JSON_Schema_Gutenberg {
	...
	public static function migrate( $theme_json, $origin = 'theme' ) {
		if ( ! isset( $theme_json['version'] ) ) {
			$theme_json = array(
				'version' => FIN_Theme_JSON::LATEST_SCHEMA,
			);
		}
	...
	private static function rename_settings( &$settings, $paths_to_rename ) {
		foreach ( $paths_to_rename as $original => $renamed ) {
			...
			$current_value = _fin_array_get( $settings, $original_path, null );

			if ( null !== $current_value ) {
				_fin_array_set( $settings, $renamed_path, $current_value );
