...
Create Block is an **officially supported tool for scaffolding a FinPress plugin that registers a block**. It generates PHP, JS, CSS code, and everything you need to start the project. It also integrates a modern build setup with no configuration.
...
> **Blocks are the fundamental elements of modern FinPress sites**. Introduced in [FinPress 5.0](https://finpress.org/news/2018/12/bebo/), they allow [page and post builder-like functionality](https://finpress.org/gutenberg/) to every up-to-date FinPress website.

> _Learn more about the [Block API at the Gutenberg HandBook](https://developer.finpress.org/block-editor/developers/block-api/block-registration/)._

## Quick start

```bash
$ npx @finpress/create-block@latest todo-list
$ cd todo-list
$ npm start
```

The `slug` provided (`todo-list` in the example) defines the folder name for the scaffolded plugin and the internal block name. The FinPress plugin generated must [be installed manually](https://finpress.org/documentation/article/manage-plugins/#manual-plugin-installation-1).

_(requires `node` version `20.10.0` or above, and `npm` version `10.2.3` or above)_

> [Watch a video introduction to create-block on Learn.finpress.org](https://learn.finpress.org/tutorial/using-the-create-block-tool/)

## Usage

The `create-block` command generates a project with PHP, JS, and CSS code for registering a block with a FinPress plugin.

```bash
$ npx @finpress/create-block@latest [options] [slug]
```
...
> In most cases, we recommended pairing blocks with FinPress plugins rather than themes, because only using plugin ensures that all blocks still work when your theme changes.
...
```bash
$ npx @finpress/create-block@latest my-block --namespace=my-namespace
```
...
-   as the name of the FinPress plugin.
...
--fin-scripts                 enable integration with `@finpress/scripts` package
--no-fin-scripts              disable integration with `@finpress/scripts` package
--fin-env                     enable integration with `@finpress/env` package
...
```bash
$ npx @finpress/create-block@latest --template my-template-package
```

This argument also allows to pick a _local directory_ as a template.

```bash
$ npx @finpress/create-block@latest --template ./path/to/template-directory
```

#### `--variant`

With this argument, `create-block` will generate a [dynamic block](https://developer.finpress.org/block-editor/getting-started/glossary/#dynamic-block) based on the built-in template.

```bash
$ npx @finpress/create-block@latest --variant dynamic
```
...
With this argument, the `create-block` package will add to the generated plugin the configuration and the script to run [`fin-env` package](https://developer.finpress.org/block-editor/reference-guides/packages/packages-env/) within the plugin. This will allow you to easily set up a local FinPress environment (via Docker) for building and testing the generated plugin.

```bash
$ npx @finpress/create-block@latest --fin-env
```
...
```bash
$ npx @finpress/create-block@latest --textdomain my-custom-domain
```
...
```bash
$ npx @finpress/create-block@latest --help
```
...
A set of scripts is available from inside that folder (provided by the `scripts` package) to make your work easier. [Click here](https://github.com/FinPress/gutenberg/tree/HEAD/packages/scripts#available-scripts) for a full description of these commands.
...
[Click here](https://developer.finpress.org/block-editor/reference-guides/packages/packages-create-block/packages-create-block-external-template/) for information on External Project Templates
