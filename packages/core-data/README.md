...
Additional parameters to the request, added as a query string. Each property will be converted into a field/value pair. For example, given the `baseURL: '/fp/v2/users'` and the `baseURLParams: { context: 'edit' }` the URL would be `/fp/v2/users?context=edit`.
...
fp.data.select( 'core' ).getEntityRecords( 'root', 'user' );

// Get a single record for the user entity.
fp.data.select( 'core' ).getEntityRecord( 'root', 'user', recordId );
```
...
// Collection
fp.data.select( 'core' ).getEntityRecords( 'root', 'user' );
fp.data.select( 'core' ).getUsers();

// Single record
fp.data.select( 'core' ).getEntityRecord( 'root', 'user', recordId );
fp.data.select( 'core' ).getUser( recordId );
```
...
```js
// Collection
fp.data.select( 'core' ).getStatuses();

// Single record
fp.data.select( 'core' ).getStatus( recordId );
```

## Actions

The following set of dispatching action creators are available on the object returned by `fp.data.dispatch( 'core' )`:
