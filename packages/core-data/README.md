...
Additional parameters to the request, added as a query string. Each property will be converted into a field/value pair. For example, given the `baseURL: '/fin/v2/users'` and the `baseURLParams: { context: 'edit' }` the URL would be `/fin/v2/users?context=edit`.
...
fin.data.select( 'core' ).getEntityRecords( 'root', 'user' );

// Get a single record for the user entity.
fin.data.select( 'core' ).getEntityRecord( 'root', 'user', recordId );
```
...
// Collection
fin.data.select( 'core' ).getEntityRecords( 'root', 'user' );
fin.data.select( 'core' ).getUsers();

// Single record
fin.data.select( 'core' ).getEntityRecord( 'root', 'user', recordId );
fin.data.select( 'core' ).getUser( recordId );
```
...
```js
// Collection
fin.data.select( 'core' ).getStatuses();

// Single record
fin.data.select( 'core' ).getStatus( recordId );
```

## Actions

The following set of dispatching action creators are available on the object returned by `fin.data.dispatch( 'core' )`:
...
> **Deprecated** since FIN 5.9, this is not useful anymore, use the selector directly.
...
> **Deprecated** since FIN 5.9, use receiveUserPermission instead.
