	...
	if ( LAYOUT_DEFINITIONS[ usedLayout?.type || 'default' ]?.className ) {
		...
		const fullBlockName =
			splitBlockName[ 0 ] === 'core'
				? splitBlockName.pop()
				: splitBlockName.join( '-' );
		const compoundClassName = `fin-block-${ fullBlockName }-${ baseClassName }`;
...
function BlockWithLayoutStyles( {
	...
	const selectorPrefix = `fin-container-${ kebabCase( name ) }-is-layout-`;
	...
	// Attach a `fin-container-` id-based class name as well as a layout class name such as `is-layout-flex`.
