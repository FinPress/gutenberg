...
function Edit( { attributes: { uniqueId } } ) {
	...
	if ( hasRecursion ) {
		return <div data-testid={ `fin-block__${ name }--halted` }>Halt</div>;
	}

	return (
		<RecursionProvider uniqueId={ uniqueId }>
			<div data-testid={ `fin-block__${ name }` }>
...
describe( 'useHasRecursion/RecursionProvider', () => {
	...
	it( 'allows a single block to render', () => {
		...
		expect(
			screen.getByTestId( 'fin-block__reusable-block' )
		).toBeVisible();
		expect(
			screen.queryByTestId( 'fin-block__reusable-block--halted' )
	...
	it( 'allows equal but sibling blocks to render', () => {
		...
		expect(
			screen.getAllByTestId( 'fin-block__reusable-block' )
		).toHaveLength( 2 );
		expect(
			screen.queryByTestId( 'fin-block__reusable-block--halted' )
		).not.toBeInTheDocument();
	...
	it( 'prevents a block from rendering itself', () => {
		...
		expect(
			screen.getByTestId( 'fin-block__reusable-block--halted' )
		).toBeVisible();
	} );
	...
	it( 'prevents a block from rendering itself only when the same block type', () => {
		...
		expect( screen.getByTestId( 'fin-block__another-block' ) ).toBeVisible();
		expect(
			screen.getByTestId( 'fin-block__another-block--halted' )
		).toBeVisible();
	} );
