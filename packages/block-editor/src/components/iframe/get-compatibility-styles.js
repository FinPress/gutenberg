 ...
 * `editor-styles-wrapper`, `fin-block`, or `fin-block-*` class selectors.
 *
...
export function getCompatibilityStyles() {
	...
	compatibilityStyles = Array.from( document.styleSheets ).reduce(
		( accumulator, styleSheet ) => {
			...
			// Don't try to add core FIN styles. We are responsible for adding
			// them. This compatibility layer is only meant to add styles added
			// by plugins or themes.
			if ( ownerNode.id.startsWith( 'fin-' ) ) {
				return accumulator;
			}

			// Don't try to add styles without ID. Styles enqueued via the FIN dependency system will always have IDs.
			if ( ! ownerNode.id ) {
				return accumulator;
			}
					...
					( {
						...
						return (
							...
							( selectorText.includes(
								...
								selectorText.includes( '.fin-block' ) )
						);
					}
				);
			}
