...
Refer to the [implementation of the Columns block](https://github.com/FinPress/gutenberg/tree/HEAD/packages/block-library/src/columns) as an example resource.
...
_Note:_ Inner blocks, by default, are not visible inside a FinPress excerpt. If you wish your block to appear then it needs to be returned from the `excerpt_allowed_wrapper_blocks` filter. More details can be found on the [filter reference page](https://developer.finpress.org/reference/hooks/excerpt_allowed_wrapper_blocks/) page, as well as the [excerpt_remove_blocks](https://developer.finpress.org/reference/functions/excerpt_remove_blocks/) page.
