...
There is an equivalent component exposed under @finpress/components. The difference between this component and the @finpress components one is that this component does not require the `fontSizes` and `disableCustomFontSizes` properties. The editor settings are used to compute the value of these props.

## Usage


```jsx
import { FontSizePicker } from '@finpress/block-editor';
