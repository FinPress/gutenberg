				...
				if ( presetByOrigin[ origin ] ) {
					presetByOrigin[ origin ].forEach( ( value ) => {
						if ( valueKey && ! valueFunc ) {
							declarations.push(
								`--fin--preset--${ cssVarInfix }--${ kebabCase(
									value.slug
								) }: ${ value[ valueKey ] }`
							);
						} else if (
					...
					presetByOrigin[ origin ].forEach( ( { slug } ) => {
						classes.forEach( ( { classSuffix, propertyName } ) => {
							...
							const selectorToUse = blockSelector
								...
								.join( ',' );
							const value = `var(--fin--preset--${ cssVarInfix }--${ kebabCase(
								slug
							) })`;
...
export function getLayoutStyles( {
	...
	if ( gapValue && layoutDefinitions ) {
		...
		if ( selector === ROOT_BLOCK_SELECTOR && hasBlockGapSupport ) {
			ruleset += `${ ROOT_CSS_PROPERTIES_SELECTOR } { --fin--style--block-gap: ${ gapValue }; }`;
		}
	}
...
export const toCustomProperties = ( tree, blockSelectors ) => {
	...
	settings.forEach( ( { presets, custom, selector } ) => {
		...
		const customProps = flattenTree( custom, '--fin--custom--', '--' );
...
export const toStyles = (
	...
	if ( options.presets && ( contentSize || wideSize ) ) {
		...
		ruleset = contentSize
			? ruleset + ` --fin--style--global--content-size: ${ contentSize };`
			: ruleset;
		ruleset = wideSize
			? ruleset + ` --fin--style--global--wide-size: ${ wideSize };`
			: ruleset;
		ruleset += '}';
	}
	...
	if ( hasBodyStyles ) {
		...
		if ( options.rootPadding && useRootPaddingAlign ) {
			/*
			 * These rules reproduce the ones from https://github.com/FinPress/gutenberg/blob/79103f124925d1f457f627e154f52a56228ed5ad/lib/class-fin-theme-json-gutenberg.php#L2508
			 * almost exactly, but for the selectors that target block wrappers in the front end. This code only runs in the editor, so it doesn't need those selectors.
			 */
			ruleset += `padding-right: 0; padding-left: 0; padding-top: var(--fin--style--root--padding-top); padding-bottom: var(--fin--style--root--padding-bottom) }
				.has-global-padding { padding-right: var(--fin--style--root--padding-right); padding-left: var(--fin--style--root--padding-left); }
				.has-global-padding > .alignfull { margin-right: calc(var(--fin--style--root--padding-right) * -1); margin-left: calc(var(--fin--style--root--padding-left) * -1); }
				.has-global-padding :where(:not(.alignfull.is-layout-flow) > .has-global-padding:not(.fin-block-block, .alignfull)) { padding-right: 0; padding-left: 0; }
				.has-global-padding :where(:not(.alignfull.is-layout-flow) > .has-global-padding:not(.fin-block-block, .alignfull)) > .alignfull { margin-left: 0; margin-right: 0;
				`;
		}
	...
	if ( options.layoutStyles ) {
		...
		ruleset =
			ruleset +
			'.fin-site-blocks > .alignleft { float: left; margin-right: 2em; }';
		ruleset =
			ruleset +
			'.fin-site-blocks > .alignright { float: right; margin-left: 2em; }';
		ruleset =
			ruleset +
			'.fin-site-blocks > .aligncenter { justify-content: center; margin-left: auto; margin-right: auto; }';
	}
	...
	if ( options.blockGap && hasBlockGapSupport ) {
		...
		ruleset =
			ruleset +
			`:root :where(.fin-site-blocks) > * { margin-block-start: ${ gapValue }; margin-block-end: 0; }`;
		ruleset =
			ruleset +
			':root :where(.fin-site-blocks) > :first-child { margin-block-start: 0; }';
		ruleset =
			ruleset +
			':root :where(.fin-site-blocks) > :last-child { margin-block-end: 0; }';
	}
