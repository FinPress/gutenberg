...
describe( 'transformStyles', () => {
	...
	describe( 'selector wrap', () => {
		...
		it( 'should ignore keyframes', () => {
			const input = `
			@keyframes __fin-base-styles-fade-in {
				from {
					opacity: 0;
				}
			}`;
		...
		it( 'should not try to wrap items within `:where` selectors', () => {
			const input =
				':where(.fin-element-button:active, .fin-block-button__link:active) { color: blue; }';
			const output = transformStyles(
				[
					{
						css: input,
			...
			const expected =
				'.my-namespace :where(.fin-element-button:active, .fin-block-button__link:active) { color: blue; }';
		...
		it( 'should not try to prefix pseudo elements on `:where` selectors', () => {
			const input =
				':where(.fin-element-button, .fin-block-button__link)::before { color: blue; }';
			const output = transformStyles(
				[
					{
						css: input,
			...
			const expected =
				'.my-namespace :where(.fin-element-button, .fin-block-button__link)::before { color: blue; }';
	...
	it( 'should not break with data urls', () => {
		...
		const output = transformStyles( [
			{
				css: input,
				baseURL: 'http://fin-site.local/themes/gut/css/',
			},
		] );
	...
	describe( 'URL rewrite', () => {
		it( 'should rewrite relative paths', () => {
			...
			const expected =
				'h1 { background: url(http://fin-site.local/themes/gut/css/images/test.png); }';
		...
		it( 'should replace complex relative paths', () => {
			...
			const expected =
				'h1 { background: url(http://fin-site.local/themes/gut/images/test.png); }';
		...
		it( 'should not replace remote paths', () => {
			const input = `h1 { background: url(http://fin.org/images/test.png); }`;
			const output = transformStyles( [
				{
					css: input,
					baseURL: 'http://fin-site.local/themes/gut/css/',
