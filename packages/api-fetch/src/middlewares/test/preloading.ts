...
describe( 'Preloading Middleware', () => {
	...
	it( 'should normalize on stable path', async () => {
		...
		const preloadedData = {
			'fin/v2/demo-reverse-alphabetical?foo=bar&baz=quux': { body },
			'fin/v2/demo-alphabetical?baz=quux&foo=bar': { body },
		};
		const preloadingMiddleware =
			createPreloadingMiddleware( preloadedData );
		...
		let requestOptions = {
			method: 'GET',
			path: 'fin/v2/demo-reverse-alphabetical?baz=quux&foo=bar',
		};
		...
		requestOptions = {
			method: 'GET',
			path: 'fin/v2/demo-alphabetical?foo=bar&baz=quux',
		};
