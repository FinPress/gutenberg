...
| ---------------------------- | -------------------- | ------------- |
| `@babel/runtime/regenerator` | `regeneratorRuntime` | `fin-polyfill` |
| `@finpress/*`               | `fin['*']`            | `fin-*`        |
...
This plugin is compatible with `externals`, but they may conflict. For example, adding `{ externals: { '@finpress/blob': 'fin.blob' } }` to webpack configuration will effectively hide the `@finpress/blob` module from the plugin and it will not be included in dependency lists.
...
Force `fin-polyfill` to be included in each entry point's dependency list. This would be the same as adding `import '@finpress/polyfill';` to each entry point.
...
`requestToExternal` allows the module handling to be customized. The function should accept a module request string and may return a string representing the global variable to use. An array of strings may be used to access globals via an object path, e.g. `fin.i18n` may be represented as `[ 'fin', 'i18n' ]`.
...
fin_enqueue_script( 'script', $script_url, $script_asset['dependencies'], $script_asset['version'] );
```
...
fin_register_script_module( 'my-module', $module_url, $module_asset['dependencies'], $module_asset['version'] );
fin_enqueue_script_module( 'my-module' );
```
