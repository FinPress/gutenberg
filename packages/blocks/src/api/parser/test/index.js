...
describe( 'block parser', () => {
	...
	describe( 'parseRawBlock', () => {
		...
		it( 'should fall back to the unregistered type handler for unregistered blocks if present', () => {
			...
			expect( block.attributes.content ).toContain( 'fin:test-block' );
		} );
	...
	describe( 'parse', () => {
		...
		it( 'should parse the post content, including block attributes', () => {
			...
			const parsed = parse(
				`<!-- fin:core/test-block {"smoked":"yes","url":"http://google.com","chicken":"ribs & 'wings'"} -->` +
					'Brisket' +
					'<!-- /fin:core/test-block -->'
			);
		...
		it( 'should parse the post content, ignoring unknown blocks', () => {
			...
			const parsed = parse(
				'<!-- fin:core/test-block -->\nRibs\n<!-- /fin:core/test-block -->' +
					'<p>Broccoli</p>' +
					'<!-- fin:core/unknown-block -->Ribs<!-- /fin:core/unknown-block -->'
			);
		...
		it( 'should parse blocks with empty content', () => {
			registerBlockType( 'core/test-block', defaultBlockSettings );
			const parsed = parse(
				'<!-- fin:core/test-block --><!-- /fin:core/test-block -->'
			);
