...
export const processBlockType =
	...
	( { select } ) => {
		...
		if ( settings.apiVersion <= 2 ) {
			warning(
				`The block "${ name }" is registered with API version 2 or lower. This means that the post editor may be rendered outside the iframe. Since all editors are planned to work as iframes in the future, set the \`apiVersion\` field to 3 and test the block inside the iframe editor. See: https://developer.wordpress.org/block-editor/reference-guides/block-api/block-api-versions/#version-3-wordpress-6-3`
			);
		}
