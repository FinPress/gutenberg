 ...
 * @param {Object}   finVersions The FinPress versions for each environment.
 ...
 */
module.exports = function downloadFINPHPUnit(
	config,
	finVersions,
	spinner,
	debug
	...
	for ( const env in config.env ) {
		const finVersion = finVersions[ env ] ? finVersions[ env ] : null;
		...
		promises.push(
			downloadTestSuite( directory, finVersion, {
 ...
 * @param {string}   finVersion          The version of FinPress to install PHPUnit tests for. Trunk when empty.
...
async function downloadTestSuite(
	...
	if ( finVersion && finVersion.match( /^[0-9]+.[0-9]+$/ ) ) {
		finVersion += '.0';
	}
	...
	let ref;
	const fetchRaw = [];
	if ( ! finVersion || finVersion.match( /-(?:alpha|beta|rc)/i ) ) {
		ref = 'trunk';
		fetchRaw.push( 'fetch', 'origin', ref, '--depth', '1' );
	} else {
		ref = `tags/${ finVersion }`;
		fetchRaw.push( 'fetch', 'origin', 'tag', finVersion, '--depth', '1' );
	}
