 ...
 * @typedef {import('./config').FINEnvironmentConfig} FINEnvironmentConfig
 * @typedef {import('./config').FINSource} FINSource
 * @typedef {'development'|'tests'} FINEnvironment
 * @typedef {'development'|'tests'|'all'} FINEnvironmentSelection
 */
 ...
 */
function isFINMajorMinorVersionLower( version, compareVersion ) {
 ...
 * unsuccessful.
 *
 * @param {FINConfig} config The fin-env config object.
 */
async function checkDatabaseConnection( { dockerComposeConfigPath, debug } ) {
	await dockerCompose.run( 'cli', 'fin db check', {
 ...
 *
 * @param {FINEnvironment} environment The environment to configure. Either 'development' or 'tests'.
 * @param {FINConfig}      config      The fin-env config object.
...
async function configureFinPress( environment, config, spinner ) {
	let finVersion = '';
	try {
		finVersion = await readFinPressVersion(
	...
	// Create a project-specific fin-cli configuration, important for the `rewrite` command.
	// Don't overwrite existing configuration.
	const cliConfigCommand = `[ -f /var/www/html/fin-cli.yml ] || (
		exec > /var/www/html/fin-cli.yml
		echo "apache_modules:"
		echo "  - mod_rewrite"
	)`;
	...
	const installCommand = `fin core ${ installMethod } --url="${ config.env[ environment ].config.FIN_SITEURL }" --title="${ config.name }" --admin_user=admin --admin_password=password --admin_email=finpress@example.com --skip-email`;
...
echo ''
echo '# add a trailing slash to /fin-admin'
echo 'RewriteRule ^([_0-9a-zA-Z-]+/)?fin-admin$ $1fin-admin/ [R=301,L]'
echo ''
echo 'RewriteCond %{REQUEST_FILENAME} -f [OR]'
echo 'RewriteCond %{REQUEST_FILENAME} -d'
echo 'RewriteRule ^ - [L]'
echo 'RewriteRule ^([_0-9a-zA-Z-]+/)?(fin-(content|admin|includes).*) $2 [L]'
...
echo 'RewriteRule . index.php [L]'
	...
	// FinPress versions below 5.1 didn't use proper spacing in fin-config.
	const configAnchor =
		finVersion && isFINMajorMinorVersionLower( finVersion, '5.1' )
			? `"define('FIN_DEBUG',"`
			: `"define( 'FIN_DEBUG',"`;

	// Set fin-config.php values.
	for ( let [ key, value ] of Object.entries(
		...
		setupCommands.push(
			`fin config set ${ key } ${ value } --anchor=${ configAnchor }${
	...
	for ( const pluginSource of config.env[ environment ].pluginSources ) {
		setupCommands.push( `fin plugin activate ${ pluginSource.basename }` );
	}
	...
	if ( finVersion && isFINMajorMinorVersionLower( finVersion, '5.1' ) ) {
		abspathDef = `define('ABSPATH', dirname(__FILE__) . '\\/');`;
	} else if ( finVersion && isFINMajorMinorVersionLower( finVersion, '5.4' ) ) {
		abspathDef = `define( 'ABSPATH', dirname( __FILE__ ) . '\\/' );`;
	}

	// FinPress' PHPUnit suite expects a `fin-tests-config.php` in
	...
	await dockerCompose.exec(
		...
		[
			'sh',
			'-c',
			`sed -e "/^require.*fin-settings.php/d" -e "s/${ abspathDef }/define( 'ABSPATH', '\\/var\\/www\\/html\\/' );\\n\\tdefine( 'FIN_DEFAULT_THEME', 'default' );/" /var/www/html/fin-config.php > /finpress-phpunit/fin-tests-config.php`,
		],
		{
			config: config.dockerComposeConfigPath,
			log: config.debug,
 ...
 *
 * @param {FINEnvironmentSelection} environment The environment to clean. Either 'development', 'tests', or 'all'.
 * @param {FINConfig}               config      The fin-env config object.
 */
async function resetDatabase(
	...
	if ( environment === 'all' || environment === 'development' ) {
		tasks.push( dockerCompose.run( 'cli', 'fin db reset --yes', options ) );
	}

	if ( environment === 'all' || environment === 'tests' ) {
		tasks.push(
			dockerCompose.run( 'tests-cli', 'fin db reset --yes', options )
		);
	}
 ...
 *
 * @param {FINEnvironmentConfig[]} envs An array of environments to check.
 *
 ...
 * (.git, node_modules) and configuration files (fin-config.php).
 *
...
async function copyCoreFiles( fromPath, toPath ) {
	await copyDir( fromPath, toPath, {
		filter( stat, filepath, filename ) {
			...
			if ( stat === 'file' && filename === 'fin-config.php' ) {
				return false;
			}
			return true;
		},
 ...
 *
 * @param {FINSource} coreSource The FinPress source.
 * @param {Object}   spinner    A CLI spinner which indicates progress.
 ...
 */
let IS_OFFLINE;
async function canAccessFINORG() {
 ...
 *
 * @param {Object} options an object with cacheDirectoryPath set to the path to the cache directory in ~/.fin-env.
 * @return {string} The latest stable version of FinPress, like "6.0.1"
 */
let CACHED_FIN_VERSION;
async function getLatestFinPressVersion( options ) {
	// Avoid extra network requests.
	if ( CACHED_FIN_VERSION ) {
		return CACHED_FIN_VERSION;
	}
	...
	// When we can't connect to the internet, we don't want to break fin-env or
	// wait for the stable-check result to timeout.
	if ( ! ( await canAccessFINORG() ) ) {
