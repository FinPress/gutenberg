 ...
 *
 * @param {string}              workDirectoryPath The working directory for fin-env.
 * @param {FINEnvironmentConfig} config            The service config to get the mounts from.
 * @param {string}              hostUsername      The username of the host running fin-env.
...
function getMounts(
	...
	// Top-level FinPress directory mounts (like fin-content/themes)
	const directoryMounts = Object.entries( config.mappings ).map(
		( [ finDir, source ] ) => `${ source.path }:/var/www/html/${ finDir }`
	);

	const pluginMounts = config.pluginSources.map(
		( source ) =>
			`${ source.path }:/var/www/html/fin-content/plugins/${ source.basename }`
	);

	const themeMounts = config.themeSources.map(
		( source ) =>
			`${ source.path }:/var/www/html/fin-content/themes/${ source.basename }`
	);
 ...
 *
 * @param {FINConfig} config A fin-env config object.
 *
...
module.exports = function buildDockerComposeConfig( config ) {
	...
	// When both tests and development reference the same FIN source, we need to
	...
	//    databases and `fin-content/uploads`.
	//
	...
	//
	// - fin-config.php        <- $tests-finpress/fin-config.php
	// - fin-config-sample.php <- $tests-finpress/fin-config.php
	// - fin-content           <- $tests-finpress/fin-content
	// - *                    <- $finpress/*
	//
	...
	if (
		...
		const finSource = config.env.development.coreSource;
		testsMounts.shift(); // Remove normal core mount.
		testsMounts.unshift(
			...[
				`${ finSource.testsPath }:/var/www/html`,
				...( finSource.type === 'local'
					? fs
							.readdirSync( finSource.path )
							.filter(
								( filename ) =>
									filename !== 'fin-config.php' &&
									filename !== 'fin-config-sample.php' &&
									filename !== 'fin-content'
							)
							.map(
								( filename ) =>
									`${ path.join(
	...
	const developmentPorts = `\${FIN_ENV_PORT:-${ config.env.development.port }}:80`;
	const developmentMysqlPorts = `\${FIN_ENV_MYSQL_PORT:-${
		config.env.development.mysqlPort ?? ''
	}}:3306`;
	const testsPorts = `\${FIN_ENV_TESTS_PORT:-${ config.env.tests.port }}:80`;
	const testsMysqlPorts = `\${FIN_ENV_TESTS_MYSQL_PORT:-${
		config.env.tests.mysqlPort ?? ''
	}}:3306`;

	const developmentPhpmyadminPorts = `\${FIN_ENV_PHPMYADMIN_PORT:-${
		config.env.development.phpmyadminPort ?? ''
	}}:80`;
	const testsPhpmyadminPorts = `\${FIN_ENV_TESTS_PHPMYADMIN_PORT:-${
		...
		services: {
			...
			finpress: {
				...
				environment: {
					...
					...dbEnv.credentials,
					...dbEnv.development,
					FIN_TESTS_DIR: '/finpress-phpunit',
				},
