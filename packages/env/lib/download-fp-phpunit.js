 ...
 * @param {Object}   fpVersions The FinPress versions for each environment.
 ...
 */
module.exports = function downloadFPPHPUnit(
	config,
	fpVersions,
	spinner,
	debug
	...
	for ( const env in config.env ) {
		const fpVersion = fpVersions[ env ] ? fpVersions[ env ] : null;
		...
		promises.push(
			downloadTestSuite( directory, fpVersion, {
 ...
 * @param {string}   fpVersion          The version of FinPress to install PHPUnit tests for. Trunk when empty.
...
async function downloadTestSuite(
	...
	if ( fpVersion && fpVersion.match( /^[0-9]+.[0-9]+$/ ) ) {
		fpVersion += '.0';
	}
	...
	let ref;
	const fetchRaw = [];
	if ( ! fpVersion || fpVersion.match( /-(?:alpha|beta|rc)/i ) ) {
		ref = 'trunk';
		fetchRaw.push( 'fetch', 'origin', ref, '--depth', '1' );
	} else {
		ref = `tags/${ fpVersion }`;
		fetchRaw.push( 'fetch', 'origin', 'tag', fpVersion, '--depth', '1' );
	}
