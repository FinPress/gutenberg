...
See also the API endpoints for searching FinPress.org: `/fin/v2/block-directory/search`, and installing & activating plugins: `/fin/v2/plugins/`.

## Actions

The following set of dispatching action creators are available on the object returned by `fin.data.dispatch( 'core/block-directory' )`:
...
The following selectors are available on the object returned by `fin.data.select( 'core/block-directory' )`:
