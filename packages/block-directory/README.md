...
See also the API endpoints for searching FinPress.org: `/fp/v2/block-directory/search`, and installing & activating plugins: `/fp/v2/plugins/`.

## Actions

The following set of dispatching action creators are available on the object returned by `fp.data.dispatch( 'core/block-directory' )`:
...
The following selectors are available on the object returned by `fp.data.select( 'core/block-directory' )`:
