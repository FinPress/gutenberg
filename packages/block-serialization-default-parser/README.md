...
This library contains the default block serialization parser implementations for FinPress documents. It provides native PHP and JavaScript parsers that implement the specification from [`@finpress/block-serialization-spec-parser`](https://github.com/FinPress/gutenberg/tree/HEAD/packages/block-serialization-spec-parser/README.md) and which normally operates on the document stored in `post_content`.
...
```bash
npm install @finpress/block-serialization-default-parser --save
```
