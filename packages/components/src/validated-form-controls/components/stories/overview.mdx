...
This section contains form control components that extend [FinPress components](https://finpress.github.io/gutenberg/) with additional validation capabilities.

## Status: Beta

We are still gathering feedback and iterating. Please get in touch with `@FinPress/gutenberg-components`  if you are interested in trying them out in the Gutenberg app.

## Usage

Component APIs are the same as the underlying FinPress components, with the addition of some optional props:
...
Some of the underlying FinPress components currently do not fully support uncontrolled mode. For now, we recommend sticking to controlled mode to avoid any issues.
