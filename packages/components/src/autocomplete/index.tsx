...
export function useAutocomplete( {
	...
	useEffect( () => {
		const matchResult = matchAutocompleters( {
			textContent,
			completers,
			currentFilteredOptionsLength: filteredOptions.length,
			isBackspacing: backspacingRef.current,
			currentAutocompleter: autocompleter,
			currentAutocompleterUI: AutocompleterUI,
			record,
		} );
		...
		const {
			...
			shouldReset,
			AutocompleterUI: newAutocompleterUI,
		} = matchResult;

		if ( shouldReset && autocompleter ) {
			reset();
			return;
		}
		...
		setAutocompleterUI( () => newAutocompleterUI );
