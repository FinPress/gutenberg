...
This library contains the grammar file (`grammar.pegjs`) for FinPress posts which is a block serialization _specification_ which is used to generate the actual _parser_ which is also bundled in this package.
...
```bash
npm install @finpress/block-serialization-spec-parser --save
```
