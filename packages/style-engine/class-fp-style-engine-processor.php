...
if ( ! class_exists( 'FP_Style_Engine_Processor' ) ) {
	...
	class FP_Style_Engine_Processor {
		...
		public function add_store( $store ) {
			if ( ! $store instanceof FP_Style_Engine_CSS_Rules_Store ) {
				_doing_it_wrong(
					__METHOD__,
					__( '$store must be an instance of FP_Style_Engine_CSS_Rules_Store', 'default' ),
					'6.1.0'
				);
				return $this;
			}
		 ...
		 *
		 * @param FP_Style_Engine_CSS_Rule|FP_Style_Engine_CSS_Rule[] $css_rules A single, or an array of, FP_Style_Engine_CSS_Rule objects from a store or otherwise.
		 *
		 * @return FP_Style_Engine_Processor Returns the object to allow chaining methods.
		 */
