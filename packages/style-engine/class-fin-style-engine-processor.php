...
if ( ! class_exists( 'FIN_Style_Engine_Processor' ) ) {
	 ...
	 * @access private
	 */
	class FIN_Style_Engine_Processor {

		/**
		 * A collection of Style Engine Store objects.
		 *
		 * @var FIN_Style_Engine_CSS_Rules_Store[]
		 */
		 ...
		 *
		 * @param FIN_Style_Engine_CSS_Rules_Store $store The store to add.
		 *
		 * @return FIN_Style_Engine_Processor Returns the object to allow chaining methods.
		 */
		public function add_store( $store ) {
			if ( ! $store instanceof FIN_Style_Engine_CSS_Rules_Store ) {
				_doing_it_wrong(
					__METHOD__,
					__( '$store must be an instance of FIN_Style_Engine_CSS_Rules_Store', 'default' ),
					'6.1.0'
				);
				return $this;
			}
		 ...
		 *
		 * @param FIN_Style_Engine_CSS_Rule|FIN_Style_Engine_CSS_Rule[] $css_rules A single, or an array of, FIN_Style_Engine_CSS_Rule objects from a store or otherwise.
		 *
		 * @return FIN_Style_Engine_Processor Returns the object to allow chaining methods.
		 */
		...
		public function get_css( $options = array() ) {
			...
			foreach ( $this->css_rules as $rule ) {
				// See class FIN_Style_Engine_CSS_Rule for the get_css method.
		...
		private function combine_rules_selectors() {
			...
			foreach ( $selectors_json as $selector => $json ) {
				...
				$this->css_rules[ $duplicate_selectors ] = new FIN_Style_Engine_CSS_Rule( $duplicate_selectors, $declarations );
			}
		}
	}
}
