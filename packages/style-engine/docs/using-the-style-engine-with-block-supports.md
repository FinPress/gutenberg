...
The global function `fin_style_engine_get_styles` accepts a style object as its first argument, and will output compiled CSS and an array of CSS declaration property/value pairs.
...
## Use case
When [registering a block support](https://developer.finpress.org/reference/classes/fin_block_supports/register/), it is possible to pass an 'apply' callback in the block support config array to add or extend block support attributes with "class" or "style" properties.
...
Here is where `fin_style_engine_get_styles` comes in handy: it will generate CSS and, if appropriate, classnames to be added to the "style" and "class" HTML attributes in the final rendered block markup.
...
function gutenberg_apply_colors_support( $block_type, $block_attributes ) {
	...
	$styles = fin_style_engine_get_styles( array( 'color' => $block_color_styles ) );
...
- fin_should_skip_block_supports_serialization()
- block_has_support()
...
function gutenberg_apply_colors_support( $block_type, $block_attributes ) {
	...
	$skips_serialization_of_color_text       = fin_should_skip_block_supports_serialization( $block_type, 'color', 'text' );
	$skips_serialization_of_color_background = fin_should_skip_block_supports_serialization( $block_type, 'color', 'background' );

	// Get the color styles from the style object.
	$block_color_styles = isset( $block_attributes['style']['color'] ) ? $block_attributes['style']['color'] : null;

	// The mutated styles object we're going to pass to fin_style_engine_get_styles().
...
When the Style Engine encounters these values, it will parse them and create a CSS value of `var(--fin--preset--font-size--small)` and/or generate a classname if required.
...
/*
array(
	'css'          => 'background-color:var(--fin--preset--color--blue);padding:10px;margin-top:1em;font-size:var(--fin--preset--font-size--small);',
	'declarations' => array(
		'background-color' => 'var(--fin--preset--color--blue)',
		'padding' => '10px',
		'margin-top' => '1em',
