...
Persistence utilities for `finpress/preferences`.

Includes a persistence layer that saves data to FinPress user meta via the REST API. If for any reason data cannot be saved to the database, this persistence layer also uses local storage as a fallback.
...
```bash
npm install @finpress/preferences-persistence --save
```
...
Creates a persistence layer that stores data in FinPress user meta via the REST API.
...
-   `Object`: A persistence layer for FinPress user meta.
