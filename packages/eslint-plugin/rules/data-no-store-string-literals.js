...
function getFixes( fixer, context, callNode ) {
	...
	if ( ! storeDefinition && storeName.startsWith( 'core/' ) ) {
		...
		storeDefinition = {
			import: `@finpress/${ storeNameWithoutCore }`,
...
module.exports = {
	meta: {
		...
		schema: [],
		messages: {
			doNotUseStringLiteral: `Do not use string literals ( '{{ argument }}' ) for accessing @finpress/data stores. Pass the store definition instead`,
		},
	},
	create( context ) {
		return {
			ImportDeclaration( node ) {
				if ( node.source.value !== '@finpress/data' ) {
					return;
				}
