...
ruleTester.run( 'react-no-unsafe-timeout', rule, {
	valid: [
		...
		{
			code: `class MyComponent extends fin.element.Component { componentDidMount() { const { setTimeout } = this.props; setTimeout(); } }`,
		},
		{
...
function MyComponent() {
	...
	invalid: [
		...
		{
			code: `class MyComponent extends fin.element.Component { componentDidMount() { setTimeout(); } }`,
			errors: [
				{
					message:
						'setTimeout in a component must be cancelled on unmount',
