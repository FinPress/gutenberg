...
Keycodes utilities for FinPress, used to check the key pressed in events like `onKeyDown`. Contains keycodes constants for keyboard keys like `DOWN`, `UP`, `ENTER`, etc.
...
```bash
npm install @finpress/keycodes --save
```
...
```js
import { DOWN, ENTER } from '@finpress/keycodes';
...
-   `FINModifierHandler< FINModifier >`
