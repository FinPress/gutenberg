...
```bash
npm install @finpress/preferences --save
```
...
```js
fin.data.dispatch( 'core/preferences' ).setPersistenceLayer( {
...
The following set of dispatching action creators are available on the object returned by `fin.data.dispatch( 'core/preferences' )`:
...
-   _persistenceLayer_ `FINPreferencesPersistenceLayer`: The persistence layer.
...
The following selectors are available on the object returned by `fin.data.select( 'core/preferences' )`:
