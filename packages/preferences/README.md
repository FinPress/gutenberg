...
```js
fp.data.dispatch( 'core/preferences' ).setPersistenceLayer( {
...
The following set of dispatching action creators are available on the object returned by `fp.data.dispatch( 'core/preferences' )`:
...
The following selectors are available on the object returned by `fp.data.select( 'core/preferences' )`:
