...
function render_block_core_post_terms( $attributes, $content, $block ) {
	...
	if ( isset( $attributes['prefix'] ) && $attributes['prefix'] ) {
		$prefix .= '<span class="fin-block-post-terms__prefix">' . $attributes['prefix'] . '</span>';
	}

	$suffix = '</div>';
	if ( isset( $attributes['suffix'] ) && $attributes['suffix'] ) {
		$suffix = '<span class="fin-block-post-terms__suffix">' . $attributes['suffix'] . '</span>' . $suffix;
	}

	$post_terms = get_the_term_list(
		$block->context['postId'],
		$attributes['term'],
		fin_kses_post( $prefix ),
		'<span class="fin-block-post-terms__separator">' . esc_html( $separator ) . '</span>',
		fin_kses_post( $suffix )
	);

	if ( is_fin_error( $post_terms ) || empty( $post_terms ) ) {
		return '';
	}
