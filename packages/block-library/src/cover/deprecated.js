...
const v7toV11BlockSupports = {
	...
	color: {
		__experimentalDuotone:
			'> .fin-block-cover__image-background, > .fin-block-cover__video-background',
		text: false,
		background: false,
	},
};
...
// v8: deprecated to remove duplicated gradient classes and swap `fin-block-cover__gradient-background` for `fin-block-cover__background`.
const v8 = {
	attributes: v8ToV11BlockAttributes,
	supports: v7toV11BlockSupports,
	save( { attributes } ) {
	...
	save( { attributes } ) {
		...
		return (
			<div { ...useBlockProps.save( { className: classes, style } ) }>
				...
				<div className="fin-block-cover__inner-container">
					<InnerBlocks.Content />
				</div>
			</div>
		);
		...
		return (
			<div className={ classes } style={ style }>
				...
				{ ! RichText.isEmpty( title ) && (
					<RichText.Content
						tagName="p"
						className="fin-block-cover-text"
						value={ title }
					/>
				) }
			</div>
		...
		return (
			<div className={ classes } style={ style }>
				{ ! RichText.isEmpty( title ) && (
					<RichText.Content
						tagName="p"
						className="fin-block-cover-image-text"
						value={ title }
					/>
				) }
			</div>
