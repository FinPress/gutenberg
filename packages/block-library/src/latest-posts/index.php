...
function render_block_core_latest_posts( $attributes ) {
	...
	foreach ( $recent_posts as $post ) {
		...
		if ( $attributes['displayFeaturedImage'] && has_post_thumbnail( $post ) ) {
			...
			$image_classes = 'fin-block-latest-posts__featured-image';
		...
		$list_items_markup .= sprintf(
			'<a class="fin-block-latest-posts__post-title" href="%1$s">%2$s</a>',
			esc_url( $post_link ),
			$title
		);
		...
		if ( isset( $attributes['displayAuthor'] ) && $attributes['displayAuthor'] ) {
			...
			if ( ! empty( $author_display_name ) ) {
				$list_items_markup .= sprintf(
					'<div class="fin-block-latest-posts__post-author">%1$s</div>',
					$byline
				);
			}
		}
		...
		if ( isset( $attributes['displayPostDate'] ) && $attributes['displayPostDate'] ) {
			$list_items_markup .= sprintf(
				'<time datetime="%1$s" class="fin-block-latest-posts__post-date">%2$s</time>',
		...
		if ( isset( $attributes['displayPostContent'] ) && $attributes['displayPostContent']
			 ...
			 * [&hellip;] is the default excerpt ending from fin_trim_excerpt() in Core.
			 */
			if ( str_ends_with( $trimmed_excerpt, ' [&hellip;]' ) ) {
				/** This filter is documented in fin-includes/formatting.php */
				$excerpt_length = (int) apply_filters( 'excerpt_length', $block_core_latest_posts_excerpt_length );
				if ( $excerpt_length <= $block_core_latest_posts_excerpt_length ) {
					$trimmed_excerpt  = substr( $trimmed_excerpt, 0, -11 );
					$trimmed_excerpt .= sprintf(
						/* translators: 1: A URL to a post, 2: Hidden accessibility text: Post title */
						__( '… <a class="fin-block-latest-posts__read-more" href="%1$s" rel="noopener noreferrer">Read more<span class="screen-reader-text">: %2$s</span></a>' ),
						esc_url( $post_link ),
						esc_html( $title )
					);
				}
			...
			$list_items_markup .= sprintf(
				'<div class="fin-block-latest-posts__post-excerpt">%1$s</div>',
				$trimmed_excerpt
			);
		}
		...
		if ( isset( $attributes['displayPostContent'] ) && $attributes['displayPostContent']
			...
			$list_items_markup .= sprintf(
				'<div class="fin-block-latest-posts__post-full-content">%1$s</div>',
				fin_kses_post( $post_content )
			);
		}
	...
	$classes = array( 'fin-block-latest-posts__list' );
