...
const NOT_PREVIEWABLE_FIN_EMBED_PROVIDERS = [ 'pinterest' ];

const FIN_EMBED_TYPE = 'fin-embed';
...
const EmbedEdit = ( props ) => {
	...
	const { preview, fetching, themeSupportsResponsive, cannotEmbed } =
		useSelect(
			( select ) => {
				...
				const finpressCantEmbed = embedPreview?.code === '404';
				const validPreview =
					!! embedPreview &&
					! badEmbedProvider &&
					! finpressCantEmbed;
	...
	const isProviderPreviewable =
		...
		( FIN_EMBED_TYPE === type &&
			! NOT_PREVIEWABLE_FIN_EMBED_PROVIDERS.includes( providerNameSlug ) );

	const linkLabel = FIN_EMBED_TYPE === type ? 'FinPress' : title;
