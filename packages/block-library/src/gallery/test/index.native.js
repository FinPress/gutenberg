...
describe( 'Gallery block', () => {
	...
	it( 'overrides "Link" setting of gallery items', async () => {
		...
		<figure class="wp-block-image"><img src="${ media[ 0 ].localUrl }" alt="" class="wp-image-${ media[ 0 ].localId }"/><figcaption class="wp-element-caption"></figcaption></figure>
		<!-- /wp:image -->

		<!-- wp:image {"id":${ media[ 1 ].localId },"linkDestination":"attachment"} -->
		<figure class="wp-block-image"><img src="${ media[ 1 ].localUrl }" alt="" class="wp-image-${ media[ 1 ].localId }"/><figcaption class="wp-element-caption"></figcaption></figure>
		<!-- /wp:image --></figure>
		<!-- /wp:gallery -->`,
			numberOfItems: 2,
		} );
