...
function block_core_gallery_render( $attributes, $content ) {
	// Adds a style tag for the --fin--style--unstable-gallery-gap var.
	...
	if ( is_array( $gap ) ) {
		foreach ( $gap as $key => $value ) {
			...
			if ( is_string( $value ) && str_contains( $value, 'var:preset|spacing|' ) ) {
				...
				$slug            = _fin_to_kebab_case( substr( $value, $index_to_splice ) );
				$value           = "var(--fin--preset--spacing--$slug)";
			}
		...
		if ( is_string( $gap ) && str_contains( $gap, 'var:preset|spacing|' ) ) {
			...
			$slug            = _fin_to_kebab_case( substr( $gap, $index_to_splice ) );
			$gap             = "var(--fin--preset--spacing--$slug)";
		}
	}

	$unique_gallery_classname = fin_unique_id( 'fin-block-gallery-' );
	...
	// --gallery-block--gutter-size is deprecated. --fin--style--gallery-gap-default should be used by themes that want to set a default
	// gap on the gallery.
	$fallback_gap = 'var( --fin--style--gallery-gap-default, var( --gallery-block--gutter-size, var( --fin--style--block-gap, 0.5em ) ) )';
	...
	$gallery_styles = array(
		array(
			'selector'     => ".fin-block-gallery.{$unique_gallery_classname}",
			'declarations' => array(
				'--fin--style--unstable-gallery-gap' => $gap_column,
				'gap'                               => $gap_value,
			),
		),
	);
	...
	// The FIN_HTML_Tag_Processor class calls get_updated_html() internally
	 ...
	 *
	 * @see: https://github.com/FinPress/gutenberg/pull/58733
	 */
	if ( empty( $attributes['randomOrder'] ) ) {
		return $updated_content;
	}

	// This pattern matches figure elements with the `fin-block-image` class to
	// avoid the gallery's wrapping `figure` element and extract images only.
	$pattern = '/<figure[^>]*\bfin-block-image\b[^>]*>.*?<\/figure>/s';
