...
Block library for the FinPress editor.
...
```bash
npm install @finpress/block-library --save
```
...
1.  When you only care about registering the block when file gets imported:
    ...
    ```js
    import '@finpress/block-library/build-module/verse/init';
    ```

2.  When you want to use the reference to the block after it gets automatically registered:

    ```js
    import verseBlock from '@finpress/block-library/build-module/verse/init';
    ```

3.  When you need a full control over when the block gets registered:

    ```js
    import { init } from '@finpress/block-library/build-module/verse';
...
1.  Do not forget to register a new core block in the [`index.js`](https://github.com/FinPress/gutenberg/blob/trunk/packages/block-library/src/index.js) file of this package. For example, if you were to add the new core block called `core/blinking-paragraph`, you would have to add something like:
...
2.  Register the block in the `gutenberg_reregister_core_block_types()` function of the [`lib/blocks.php`](https://github.com/FinPress/gutenberg/blob/trunk/lib/blocks.php) file. Add it to the `block_folders` array if it's a [static block](https://developer.finpress.org/block-editor/getting-started/glossary/#static-block) or to the `block_names` array if it's a [dynamic block](https://developer.finpress.org/block-editor/getting-started/glossary/#dynamic-block).
...
4.  If the block exposes a script module on the front end, it must be included in the package's `package.json` file in the `finScriptModules` object. This will include the script module when it's bundled for use in FinPress. See [the packages README for more details.](../README.md):
    ...
    function render_block_core_blinking_paragraph( $attributes, $content ) {
        ...
        if ( $should_load_view_script ) {
    	    fin_enqueue_script_module( '@finpress/block-library/blinking-paragraph' );
        }
...
Unlike in [PHP code in the /lib directory](https://github.com/FinPress/gutenberg/blob/trunk/lib/README.md), you should generally avoid applying plugin-specific prefixes/suffixes such as `gutenberg_` to functions and other code in block PHP files.
...
In such cases, you can use the corresponding Core `fin_` function in the block PHP code, and add its name to [a list of prefixed functions in the Webpack configuration file](https://github.com/FinPress/gutenberg/blob/trunk/tools/webpack/blocks.js#L30).

At build time, Webpack will search for `fin_` functions in that list and replace them with their `gutenberg_` equivalents. This process ensures that the plugin calls the `gutenberg_` functions, but the block will still call the Core `fin_` function when updates are back ported.

Webpack assumes that, prefixes aside, the functions' names are identical: `fin_get_something_useful()` will be replaced with `gutenberg_get_something_useful()`.
