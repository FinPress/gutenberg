...
The following set of dispatching action creators are available on the object returned by `fin.data.dispatch( 'core/upload-media' )`:
...
The following selectors are available on the object returned by `fin.data.select( 'core/upload-media' )`:
