...
The following set of dispatching action creators are available on the object returned by `fp.data.dispatch( 'core/upload-media' )`:
...
The following selectors are available on the object returned by `fp.data.select( 'core/upload-media' )`:
