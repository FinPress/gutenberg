...
Widgets blocks in Customizer Module for FinPress.
...
```bash
npm install @finpress/customize-widgets
```
...
`components/sidebar-block-editor` is the entry point for each widget area's block editor. `component/sidebar-block-editor/sidebar-adapter.js` is an adapter to talk to [the Customize API](https://developer.finpress.org/themes/customize-api/) and transform widget objects into widget instances.
...
For React developers, this can be thought of as a custom reconciler or a custom renderer for the Customizer. But instead of targeting DOM as the render target, we are targeting FinPress widgets using the Customize API.
