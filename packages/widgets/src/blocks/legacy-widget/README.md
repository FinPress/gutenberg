...
If a widget has set `FP_Widget::$show_instance_in_rest` to `true`, then its raw
unencoded settings are kept in `instance.raw`. This is useful for transforming a
Legacy Widget block into e.g. a Search block.

## Single widgets

FinPress supports a legacy style of widget that does not extend `FP_Widget`.
These are registered using `fp_register_sidebar_widget()`. Widgets registered
