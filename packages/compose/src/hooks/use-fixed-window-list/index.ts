...
export default function useFixedWindowList(
	...
	WPFixedWindowList,
	setFixedListWindow: ( nextWindow: WPFixedWindowList ) => void,
] {
	...
	useLayoutEffect( () => {
		...
		const measureWindow = ( initRender?: boolean | undefined ) => {
			if ( ! scrollContainer ) {
				return;
			}
