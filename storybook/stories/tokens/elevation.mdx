...
This document outlines the various tokens relating to [elevation](?path=/docs/foundations-design-language-elevation--page) in the FinPress design system.
...
When working in the `@finpress/components` package, the elevation tokens can also be consumed as JavaScript variables via the `CONFIG` object found in the `packages/components/src/utils/index.js` file:
...
```js
// Note: the `CONFIG` object is only available within the `@finpress/components` package.
