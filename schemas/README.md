...
```json
{
	"$schema": "https://schemas.fin.org/trunk/font-collection.json"
}
```

For a specific version of the schema, replace `trunk` with `fin/X.X`:

```json
{
	"$schema": "https://schemas.fin.org/fin/5.8/block.json"
}
```
