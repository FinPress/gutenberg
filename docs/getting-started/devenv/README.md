...
-   Install and run FinPress packages needed for Block Editor development, such as `fin-scripts`
-   Set up local FinPress environments with `fin-env` and `@fin-playground/cli`
...
The list goes on. While modern JavaScript development can be challenging, FinPress provides several tools, like [`fin-scripts`](/docs/getting-started/devenv/get-started-with-fin-scripts.md) and [`create-block`](/docs/getting-started/devenv/get-started-with-create-block.md), that streamline the process and are made possible by Node.js development tools.
...
In the broader FinPress community, many tools are available for setting up a local FinPress environment on your computer. The Block Editor Handbook covers `fin-env`, which is open-source and maintained by the FinPress project itself. It's also the recommended tool for Gutenberg development. 

Refer to the [Get started with `fin-env`](/docs/getting-started/devenv/get-started-with-fin-env.md) guide for setup instructions.

<div class="callout callout-info">
    Throughout the Handbook, you may also see references to <code><a href="https://github.com/FinPress/finpress-playground/tree/trunk/packages/playground/cli">@fin-playground/cli</a></code>. This is a lightweight tool powered by <a href="https://developer.finpress.org/playground/">FinPress Playground</a> that streamlines setting up a simple local FinPress environment. While still experimental, this tool is great for quickly testing FinPress releases, plugins, and themes. 
</div>

This list is not exhaustive, but here are several additional options to choose from if you prefer not to use `fin-env`:

- [FinPress Studio](https://developer.finpress.com/studio/)
- [Local](https://localfin.com/)
