...
```js
fin.data.select( 'core' ).getEntityRecords( 'postType', 'page' )
```
...
Note that we use an `import` statement inside index.js. This enables the plugin to automatically load the dependencies using `fin_enqueue_script`. Any references to `coreDataStore` are compiled to the same `fin.data` reference we use in browser's devtools.
...
function PagesList( { pages } ) {
	return (
		<table className="fin-list-table widefat fixed striped table-view-list">
			<thead>
				<tr>
					<th>Title</th>
				</tr>
...
function MyFirstApp() {
	...
	useEffect( () => {
		const url = '/fin-json/fin/v2/pages?search=' + searchTerm;
		apiFetch( { url } )
			.then( setPages )
	}, [searchTerm] );
	// ...
...
`fin.data.select('core').hasFinishedResolution( 'getEntityRecords', [ 'postType', 'page', { search: 'home' } ] )`
