...
Running `npm run test:unit:debug` will start the tests in debug mode so a [node inspector client](https://nodejs.org/en/docs/guides/debugging-getting-started/#inspector-clients) can connect to the process and inspect the execution. Instructions for using Google Chrome or Visual Studio Code as an inspector client can be found in the [fin-scripts documentation](/packages/scripts/README.md#debugging-jest-unit-tests).
...
If using a different setup than `fin-env`, you first need to symlink the e2e test plugins to your test site, from your site's plugins directory run:
...
_Note: The phpunit commands require `fin-env` to be running and composer dependencies to be installed. The package script will start fin-env for you if it is not already running._
