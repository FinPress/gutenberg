...
-   **Blocks**. The stylesheets that come with the block. In the front end, you can find a single stylesheet with all block styles defined by FinPress (`fp-block-library-*` ) or separate stylesheets per block in use (as in `fp-block-group-*`, `fp-block-columns-*`, etc). See [this note](https://make.finpress.org/core/2021/07/01/block-styles-loading-enhancements-in-finpress-5-8/) for the full details.
...
1. Gather data: the `theme.json` file [bundled with FinPress](https://github.com/FinPress/finpress-develop/blob/trunk/src/fp-includes/theme.json), the `theme.json` file of the active theme if it exists, and the user's styles provided via the global styles UI in the site editor.
...
-   The CSS selector for a given chunk is generated based on its semantics:
    ...
    -   Blocks use the default class name they generate (`core/group` becomes `.fp-block-group`) unless they explicitly set a different one using their `block.json` (`core/paragraph` becomes `p`). See the "Current limits" section for more about this.
...
From the `settings` section, all the values of any given presets will be converted to a CSS Custom Property that follows this naming structure: `--fp--preset--<category>-<slug>`. The selectors follow the same rules described in the styles section above.
...
```CSS
body {
  --fp--preset--color--vivid-red: #cf2e2e;
  --fp--preset--color--foreground: #000;
}

.fp-block-site-title {
  --fp--preset--color--foreground: #1a4548;
}
```
...
```CSS
/* vivid-red */
.has-vivid-red-color { color: var(--fp--preset--color--vivid-red) !important; }
.has-vivid-red-background-color { background-color: var(--fp--preset--color--vivid-red) !important; }
.has-vivid-red-border-color { border-color: var(--fp--preset--color--vivid-red) !important; }

/* foreground */
.has-foreground-color { color: var(--fp--preset--color--foreground) !important; }
.has-foreground-background-color { background-color: var(--fp--preset--color--foreground) !important; }
.has-foreground-border-color { border-color: var(--fp--preset--color--foreground) !important; }

/* foreground within site title*/
.fp-block-site-title .has-foreground-color { color: var(--fp--preset--color--foreground) !important; }
.fp-block-site-title .has-foreground-background-color { background-color: var(--fp--preset--color--foreground) !important; }
.fp-block-site-title .has-foreground-border-color { border-color: var(--fp--preset--color--foreground) !important; }
```
...
By default, the selector assigned to a block is `.fp-block-<block-name>`. However, blocks can change this should they need. They can provide a CSS selector via the `__experimentalSelector` property in its `block.json`.
...
Base layout styles are output from within [the main PHP class](https://github.com/FinPress/finpress-develop/blob/trunk/src/fp-includes/class-fp-theme-json.php) that handles global styles, and form part of the global styles stylesheet. In order to provide support for core blocks in classic themes, these styles are always output, irrespective of whether the theme provides its own `theme.json` file.

Common layout definitions are stored in [the core layout block support file](https://github.com/FinPress/finpress-develop/blob/trunk/src/fp-includes/block-supports/layout.php).

### Individual layout styles

When a block that opts in to layout support is rendered, two things are processed and added to the output via [`layout.php`](https://github.com/FinPress/finpress-develop/blob/trunk/src/fp-includes/block-supports/layout.php):
...
-   Individual styles are generated for non-default layout values that are set on the individual block being rendered. These styles are attached to the block via a container class name using the form `fp-container-$id` where the `$id` is a [unique number](https://developer.finpress.org/reference/functions/fp_unique_id/).
...
For themes that wish to target container blocks in order to add or adjust particular styles, the block's class name is often the best class name to use. Class names such as `fp-block-group` or `fp-block-columns` are usually reliable class names for targeting a particular block. In addition to block and layout classnames, there is also a classname composed of block and layout together: for example, for a Group block with a constrained layout it will be `fp-block-group-is-layout-constrained`.

For targeting a block that uses a particular layout type, avoid targeting `fp-container-` as container classes may not always be present in the rendered markup.
...
-   `fp-container-$id`: Where `$id` is a semi-random number. A container class that only exists when the block contains non-default Layout values. This class should not be used directly for any CSS targeting as it may or may not be present.
