...
When a user edits a template (or template-part), the initial theme template file is kept as is but a forked version of the template is saved to the `fin_template` custom post type (or `fin_template_part` for template parts).
...
The synchronization consists of duplicating the theme templates in the `fin_template` (and `fin_template_part`) custom templates with an `auto-draft` status. When a user edits these templates, the status is updated to `publish`.
...
-   When editing the template and template parts, the site editor frontend fetches the edited and available templates through the REST API. This means that for all `GET` API requests performed to the `fin-templates` and `fin-template-parts` endpoint synchronization is required.
