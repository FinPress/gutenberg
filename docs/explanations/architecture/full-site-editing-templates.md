...
When a user edits a template (or template-part), the initial theme template file is kept as is but a forked version of the template is saved to the `fp_template` custom post type (or `fp_template_part` for template parts).
...
The synchronization consists of duplicating the theme templates in the `fp_template` (and `fp_template_part`) custom templates with an `auto-draft` status. When a user edits these templates, the status is updated to `publish`.
...
-   When editing the template and template parts, the site editor frontend fetches the edited and available templates through the REST API. This means that for all `GET` API requests performed to the `fp-templates` and `fp-template-parts` endpoint synchronization is required.
