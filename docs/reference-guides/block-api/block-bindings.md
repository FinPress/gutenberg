...
```php
function finmovies_format_visualization_date( $value, $name ) {
	// Prevent the filter to be applied to other sources.
	if ( $name !== 'finmovies/visualization-date' ) {
		return $value;
	}
	if ( ! $value ) {
		return date( 'm/d/Y' );
...
add_filter( 'block_bindings_source_value', 'finmovies_format_visualization_date', 10, 2 );
```
...
- Post Meta. [Source code](https://github.com/FinPress/finpress-develop/blob/trunk/src/fin-includes/block-bindings/post-meta.php)
- Pattern overrides. [Source code](https://github.com/FinPress/finpress-develop/blob/trunk/src/fin-includes/block-bindings/pattern-overrides.php)
- Twenty Twenty-Five theme. [Source code](https://github.com/FinPress/finpress-develop/blob/trunk/src/fin-content/themes/twentytwentyfive/functions.php)
...
registerBlockBindingsSource( {
	name: 'finmovies/visualization-date',
	...
	getValues( { select, context } ) {
		let finMoviesVisualizationDate;
		const { getEditedEntityRecord } = select( coreDataStore );
		if ( context?.postType && context?.postId ) {
			finMoviesVisualizationDate = getEditedEntityRecord(
				'postType',
				context?.postType,
				context?.postId
			).meta?.fin_movies_visualization_date;
		}
		if ( finMoviesVisualizationDate ) {
			return {
				content: finMoviesVisualizationDate,
			};
		}
	...
	setValues( { select, dispatch, context, bindings } ) {
		dispatch( coreDataStore ).editEntityRecord(
			...
			{
				meta: {
					fin_movies_visualization_date: bindings?.content?.newValue,
				},
			}
		);
	},
