...
Returns information for a given media item.
...
_Usage_
    ...
    const mediaItem = useSelect( ( select ) => {
    	return select( coreDataStore ).getMedia( 123 );

    	// Equivalent to:
    	// select( coreDataStore ).getEntityRecord( 'root', 'media', 123 );
    } );

_Parameters_

-   mediaId `number`: The media item ID.

_Returns_

-   `EntityRecord | undefined`: Media record.
