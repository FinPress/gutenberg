...
The following example ensures that List blocks are saved with the canonical generated class name (`fin-block-list`):
...
Generated HTML classes for blocks follow the `fin-block-{name}` nomenclature. This filter allows to provide an alternative class name.
...
const { PanelBody } = fin.components;
...
fin.blocks.getBlockTypes().forEach( function ( blockType ) {
	if ( allowedBlocks.indexOf( blockType.name ) === -1 ) {
		fin.blocks.unregisterBlockType( blockType.name );
	}
} );
```
...
To set an SVG icon for the category shown in the previous example, add the following example JavaScript code to the Editor calling `fin.blocks.updateCategory` e.g:

```js
( function () {
	var el = React.createElement;
	var SVG = fin.primitives.SVG;
	...
	fin.blocks.updateCategory( 'my-category', { icon: svgIcon } );
} )();
```
