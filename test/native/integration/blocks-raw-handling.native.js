...
// Path to the fixtures provided in `gutenberg/test/integration`.
const fixturesPath = `${ __dirname }/../../integration`;
...
describe( 'Blocks raw handling', () => {
	...
	it.skip( 'should break up forced inline content', () => {
		const filtered = pasteHandler( {
	...
	it.skip( 'should not treat single non-inlineable block as inline text', () => {
		const filtered = pasteHandler( {
	...
	it.skip( 'should handle transforms that return an array of blocks', () => {
	...
	describe( 'pasteHandler', () => {
		...
		it( 'should strip windows data', () => {
			const HTML = readFile(
				path.join( fixturesPath, 'fixtures/documents/windows.html' )
			);
			expect( serialize( pasteHandler( { HTML } ) ) ).toMatchSnapshot();
		} );

		it.skip( 'should strip HTML formatting space from inline text', () => {
			const HTML = readFile(
				path.join(
					fixturesPath,
...
describe( 'rawHandler', () => {
	it.skip( 'should convert HTML post to blocks with minimal content changes', () => {
		const HTML = readFile(
			path.join(
				fixturesPath,
				'fixtures/documents/finpress-convert.html'
			)
		);
