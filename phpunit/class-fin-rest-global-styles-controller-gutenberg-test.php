<?php
/**
 * Unit tests covering FIN_REST_Global_Styles_Controller_Gutenberg functionality.
 *
 * @package Gutenberg
 *
 * @covers FIN_REST_Global_Styles_Controller_Gutenberg
 */
class FIN_REST_Global_Styles_Controller_Gutenberg_Test extends FIN_Test_REST_Controller_Testcase {
	/**
	 * @var int
	 */
	...
	public static function finSetupBeforeClass( $factory ) {
		...
		if ( $theme_manager_id instanceof FIN_User ) {
		...
		self::$global_styles_id = $factory->post->create(
			array(
				...
				'post_type'    => 'fin_global_styles',
				'post_name'    => 'fin-global-styles-emptytheme',
				'tax_input'    => array(
					'fin_theme' => 'emptytheme',
				),
			)
		);
	...
	public function test_get_theme_items() {
		...
		switch_theme( 'emptytheme' );
		$request  = new FIN_REST_Request( 'GET', '/fin/v2/global-styles/themes/emptytheme/variations' );
		...
		$expected = array(
			array(
				'_links'   => array(
					...
					'fin:theme-file' => array(
						array(
							'href'   => home_url( '/fin-content/themes/emptytheme/img/1024x768_emptytheme_test_image.jpg' ),
		...
		fin_recursive_ksort( $data );
		fin_recursive_ksort( $expected );
	...
	/**
	 * @covers FIN_REST_Global_Styles_Controller_Gutenberg::get_theme_item
	 */
	public function test_get_theme_item_no_user() {
		fin_set_current_user( 0 );
		$request  = new FIN_REST_Request( 'GET', '/fin/v2/global-styles/themes/emptytheme' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_theme_item_subscriber_permission_check() {
		fin_set_current_user( self::$subscriber_id );
		switch_theme( 'emptytheme' );
		$request  = new FIN_REST_Request( 'GET', '/fin/v2/global-styles/themes/emptytheme' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_theme_item_editor_permission_check() {
		fin_set_current_user( self::$editor_id );
		switch_theme( 'emptytheme' );
		$request  = new FIN_REST_Request( 'GET', '/fin/v2/global-styles/themes/emptytheme' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_theme_item_theme_options_manager_permission_check() {
		fin_set_current_user( self::$theme_manager_id );
		switch_theme( 'emptytheme' );
		$request  = new FIN_REST_Request( 'GET', '/fin/v2/global-styles/themes/emptytheme' );
		$response = rest_get_server()->dispatch( $request );
		// Checks that the response has the expected keys.
	...
	public function test_get_theme_item_invalid() {
		fin_set_current_user( self::$admin_id );
		$request  = new FIN_REST_Request( 'GET', '/fin/v2/global-styles/themes/invalid' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_theme_item_invalid_theme_dirname( $theme_dirname, $expected ) {
		...
		$request  = new FIN_REST_Request( 'GET', '/fin/v2/global-styles/themes/' . $theme_dirname );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_theme_item( $theme ) {
		...
		$request  = new FIN_REST_Request( 'GET', '/fin/v2/global-styles/themes/' . $theme );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$links    = $response->get_links();
		$this->assertArrayHasKey( 'settings', $data, 'Data does not have "settings" key' );
		$this->assertArrayHasKey( 'styles', $data, 'Data does not have "styles" key' );
		$this->assertArrayHasKey( 'self', $links, 'Links do not have a "self" key' );
		$this->assertStringContainsString( '/fin/v2/global-styles/themes/' . $theme, $links['self'][0]['href'] );
	}
	...
	/**
	 * @covers FIN_REST_Global_Styles_Controller_Gutenberg::get_item
	 */
	public function test_get_item_no_user() {
		fin_set_current_user( 0 );
		$request  = new FIN_REST_Request( 'GET', '/fin/v2/global-styles/' . self::$global_styles_id );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_item_invalid_post() {
		fin_set_current_user( self::$admin_id );
		$request  = new FIN_REST_Request( 'GET', '/fin/v2/global-styles/' . self::$post_id );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_item() {
		...
		$this->assertStringContainsString( '/fin/v2/global-styles/' . self::$global_styles_id, $links['self'][0]['href'] );
	}
	...
	/**
	 * @covers FIN_REST_Global_Styles_Controller_Gutenberg::update_item
	 */
	public function test_update_item() {
		fin_set_current_user( self::$admin_id );
		$request = new FIN_REST_Request( 'PUT', '/fin/v2/global-styles/' . self::$global_styles_id );
	...
	public function test_update_item_invalid_post() {
		fin_set_current_user( self::$admin_id );
		$request  = new FIN_REST_Request( 'PUT', '/fin/v2/global-styles/' . self::$post_id );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_global_styles_not_found', $response, 404 );
	}
	...
	public function test_update_item_with_custom_block_style_variations() {
		 ...
		 * via either a theme.json partial or through the FIN_Block_Styles_Registry.
		 */
	...
	/**
	 * @covers FIN_REST_Global_Styles_Controller_Gutenberg::get_item_schema
	 */
	public function test_get_item_schema() {
		$request    = new FIN_REST_Request( 'OPTIONS', '/fin/v2/global-styles/' . self::$global_styles_id );
		$response   = rest_get_server()->dispatch( $request );
		$data       = $response->get_data();
	...
	/**
	 * @covers FIN_REST_Global_Styles_Controller_Gutenberg::get_available_actions
	 */
