<?php
/**
 * Unit tests covering FP_REST_Global_Styles_Controller_Gutenberg functionality.
 *
 * @package Gutenberg
 *
 * @covers FP_REST_Global_Styles_Controller_Gutenberg
 */
class FP_REST_Global_Styles_Controller_Gutenberg_Test extends FP_Test_REST_Controller_Testcase {
	/**
	 * @var int
	 */
	...
	public static function fpSetupBeforeClass( $factory ) {
		...
		if ( $theme_manager_id instanceof FP_User ) {
		...
		self::$global_styles_id = $factory->post->create(
			array(
				...
				'post_type'    => 'fp_global_styles',
				'post_name'    => 'fp-global-styles-emptytheme',
				'tax_input'    => array(
					'fp_theme' => 'emptytheme',
				),
			)
	...
	public function test_get_theme_items() {
		...
		switch_theme( 'emptytheme' );
		$request  = new FP_REST_Request( 'GET', '/fp/v2/global-styles/themes/emptytheme/variations' );
		...
		$expected = array(
			array(
				'_links'   => array(
					...
					'fp:theme-file' => array(
						array(
							'href'   => home_url( '/fp-content/themes/emptytheme/img/1024x768_emptytheme_test_image.jpg' ),
		...
		fp_recursive_ksort( $data );
		fp_recursive_ksort( $expected );
	...
	/**
	 * @covers FP_REST_Global_Styles_Controller_Gutenberg::get_theme_item
	 */
	public function test_get_theme_item_no_user() {
		fp_set_current_user( 0 );
		$request  = new FP_REST_Request( 'GET', '/fp/v2/global-styles/themes/emptytheme' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_theme_item_subscriber_permission_check() {
		fp_set_current_user( self::$subscriber_id );
		switch_theme( 'emptytheme' );
		$request  = new FP_REST_Request( 'GET', '/fp/v2/global-styles/themes/emptytheme' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_theme_item_editor_permission_check() {
		fp_set_current_user( self::$editor_id );
		switch_theme( 'emptytheme' );
		$request  = new FP_REST_Request( 'GET', '/fp/v2/global-styles/themes/emptytheme' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_theme_item_theme_options_manager_permission_check() {
		fp_set_current_user( self::$theme_manager_id );
		switch_theme( 'emptytheme' );
		$request  = new FP_REST_Request( 'GET', '/fp/v2/global-styles/themes/emptytheme' );
		$response = rest_get_server()->dispatch( $request );
		// Checks that the response has the expected keys.
	...
	public function test_get_theme_item_invalid() {
		fp_set_current_user( self::$admin_id );
		$request  = new FP_REST_Request( 'GET', '/fp/v2/global-styles/themes/invalid' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_theme_item_invalid_theme_dirname( $theme_dirname, $expected ) {
		...
		$request  = new FP_REST_Request( 'GET', '/fp/v2/global-styles/themes/' . $theme_dirname );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_theme_item( $theme ) {
		...
		$request  = new FP_REST_Request( 'GET', '/fp/v2/global-styles/themes/' . $theme );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$links    = $response->get_links();
		$this->assertArrayHasKey( 'settings', $data, 'Data does not have "settings" key' );
		$this->assertArrayHasKey( 'styles', $data, 'Data does not have "styles" key' );
		$this->assertArrayHasKey( 'self', $links, 'Links do not have a "self" key' );
		$this->assertStringContainsString( '/fp/v2/global-styles/themes/' . $theme, $links['self'][0]['href'] );
	}
	...
	/**
	 * @covers FP_REST_Global_Styles_Controller_Gutenberg::get_item
	 */
	public function test_get_item_no_user() {
		fp_set_current_user( 0 );
		$request  = new FP_REST_Request( 'GET', '/fp/v2/global-styles/' . self::$global_styles_id );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_item_invalid_post() {
		fp_set_current_user( self::$admin_id );
		$request  = new FP_REST_Request( 'GET', '/fp/v2/global-styles/' . self::$post_id );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_item() {
		...
		$this->assertStringContainsString( '/fp/v2/global-styles/' . self::$global_styles_id, $links['self'][0]['href'] );
	}
	...
	/**
	 * @covers FP_REST_Global_Styles_Controller_Gutenberg::update_item
	 */
	public function test_update_item() {
		fp_set_current_user( self::$admin_id );
		$request = new FP_REST_Request( 'PUT', '/fp/v2/global-styles/' . self::$global_styles_id );
	...
	public function test_update_item_invalid_post() {
		fp_set_current_user( self::$admin_id );
		$request  = new FP_REST_Request( 'PUT', '/fp/v2/global-styles/' . self::$post_id );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_global_styles_not_found', $response, 404 );
	}
	...
	public function test_update_item_with_custom_block_style_variations() {
		 ...
		 * via either a theme.json partial or through the FP_Block_Styles_Registry.
		 */
	...
	/**
	 * @covers FP_REST_Global_Styles_Controller_Gutenberg::get_item_schema
	 */
	public function test_get_item_schema() {
		$request    = new FP_REST_Request( 'OPTIONS', '/fp/v2/global-styles/' . self::$global_styles_id );
		$response   = rest_get_server()->dispatch( $request );
		$data       = $response->get_data();
	...
	/**
	 * @covers FP_REST_Global_Styles_Controller_Gutenberg::get_available_actions
	 */
