...
/**
 * Test FP_Theme_JSON_Resolver_Gutenberg class.
 *
 * @package Gutenberg
 *
 * @since 5.8.0
...
class FP_Theme_JSON_Resolver_Gutenberg_Test extends FP_UnitTestCase {
	...
	public static function set_up_before_class() {
		...
		static::$property_blocks_cache = new ReflectionProperty( FP_Theme_JSON_Resolver_Gutenberg::class, 'blocks_cache' );
		...
		static::$property_core = new ReflectionProperty( FP_Theme_JSON_Resolver_Gutenberg::class, 'core' );
	...
	public function test_translations_are_applied() {
		...
		$theme_data       = FP_Theme_JSON_Resolver_Gutenberg::get_theme_data();
		$style_variations = FP_Theme_JSON_Resolver_Gutenberg::get_style_variations();

		unload_textdomain( 'block-theme' );
		remove_filter( 'locale', array( $this, 'filter_set_locale_to_polish' ) );

		$this->assertSame( 'block-theme', fp_get_theme()->get( 'TextDomain' ) );
	...
	private function get_registered_block_names( $hard_reset = false ) {
		...
		$resolver             = FP_Block_Type_Registry::get_instance();
	...
	/**
	 * Tests when FP_Theme_JSON_Resolver_Gutenberg::$blocks_cache is empty or
	...
	public function test_has_same_registered_blocks_when_all_blocks_not_cached( $origin, array $cache = array() ) {
		$has_same_registered_blocks = new ReflectionMethod( FP_Theme_JSON_Resolver_Gutenberg::class, 'has_same_registered_blocks' );
		...
		$this->assertFalse( $has_same_registered_blocks->invoke( null, $origin ), 'FP_Theme_JSON_Resolver_Gutenberg::has_same_registered_blocks() should return false when same blocks are not cached' );
		$blocks_cache = static::$property_blocks_cache->getValue();
		$this->assertSameSets( $expected_cache, $blocks_cache[ $origin ], 'FP_Theme_JSON_Resolver_Gutenberg::$blocks_cache should contain all expected block names for the given origin' );
	}
	...
	public function test_has_same_registered_blocks_when_all_blocks_are_cached( $origin ) {
		...
		$this->assertTrue( $has_same_registered_blocks->invoke( null, $origin ), 'FP_Theme_JSON_Resolver_Gutenberg::has_same_registered_blocks() should return true when using the cache' );
		$this->assertSameSets( $expected_cache, $blocks_cache[ $origin ], 'FP_Theme_JSON_Resolver_Gutenberg::$blocks_cache should contain all expected block names for the given origin' );
	}
	 ...
	 * @covers FP_Theme_JSON_Resolver_Gutenberg::get_core_data
	 */
	public function test_get_core_data( $should_fire_filter, $core_is_cached, $blocks_are_cached ) {
		...
		if ( $core_is_cached ) {
			FP_Theme_JSON_Resolver_Gutenberg::get_core_data();
		}
		...
		$expected_filter_count = did_filter( 'fp_theme_json_data_default' );
		$actual                = FP_Theme_JSON_Resolver_Gutenberg::get_core_data();
		if ( $should_fire_filter ) {
			++$expected_filter_count;
		}

		$this->assertSame( $expected_filter_count, did_filter( 'fp_theme_json_data_default' ), 'The filter "fp_theme_json_data_default" should fire the given number of times' );
		$this->assertInstanceOf( FP_Theme_JSON_Gutenberg::class, $actual, 'FP_Theme_JSON_Resolver_Gutenberg::get_core_data() should return instance of FP_Theme_JSON_Gutenberg' );
		$this->assertSame( static::$property_core->getValue(), $actual, 'FP_Theme_JSON_Resolver_Gutenberg::$core property should be the same object as returned from FP_Theme_JSON_Resolver_Gutenberg::get_core_data()' );
	}
	...
	public function test_add_theme_supports_are_loaded_for_themes_without_theme_json() {
		...
		$settings = FP_Theme_JSON_Resolver_Gutenberg::get_theme_data()->get_settings();
	...
	public function test_core_default_settings_are_loaded_for_themes_without_theme_json() {
		...
		$settings = FP_Theme_JSON_Resolver_Gutenberg::get_merged_data( 'theme' )->get_settings();
	...
	public function test_merges_child_theme_json_into_parent_theme_json() {
		...
		$actual_settings   = FP_Theme_JSON_Resolver_Gutenberg::get_theme_data()->get_settings();
		...
		fp_recursive_ksort( $actual_settings );
		fp_recursive_ksort( $expected_settings );
		...
		$this->assertSame(
			...
			FP_Theme_JSON_Resolver_Gutenberg::get_theme_data()->get_custom_templates()
		);
	}

	/**
	 * @covers FP_Theme_JSON_Resolver_Gutenberg::get_user_data_from_fp_global_styles
	 */
	public function test_get_user_data_from_fp_global_styles_does_not_use_uncached_queries() {
		// Switch to a theme that does have support.
		switch_theme( 'block-theme' );
		fp_set_current_user( self::$administrator_id );
		$theme = fp_get_theme();
		FP_Theme_JSON_Resolver_Gutenberg::get_user_data_from_fp_global_styles( $theme );
		$global_styles_query_count = 0;
		add_filter(
			'query',
			static function ( $query ) use ( &$global_styles_query_count ) {
				if ( preg_match( '#post_type = \'fp_global_styles\'#', $query ) ) {
		...
		$this->assertSame( 0, $global_styles_query_count, 'Unexpected SQL queries detected for the fp_global_style post type prior to creation.' );

		$user_cpt = FP_Theme_JSON_Resolver_Gutenberg::get_user_data_from_fp_global_styles( $theme );
		$this->assertEmpty( $user_cpt, 'User CPT is expected to be empty.' );

		$user_cpt = FP_Theme_JSON_Resolver_Gutenberg::get_user_data_from_fp_global_styles( $theme, true );
		$this->assertNotEmpty( $user_cpt, 'User CPT is expected not to be empty.' );

		$global_styles_query_count = 0;
		for ( $i = 0; $i < 3; $i++ ) {
			$new_user_cpt = FP_Theme_JSON_Resolver_Gutenberg::get_user_data_from_fp_global_styles( $theme );
			_gutenberg_clean_theme_json_caches();
			$this->assertSameSets( $user_cpt, $new_user_cpt, "User CPTs do not match on run {$i}." );
		}
		$this->assertSame( 1, $global_styles_query_count, 'Unexpected SQL queries detected for the fp_global_style post type after creation.' );
	}

	/**
	 * @covers FP_Theme_JSON_Resolver_Gutenberg::get_user_data_from_fp_global_styles
	 */
	public function test_get_user_data_from_fp_global_styles_does_not_use_uncached_queries_for_logged_out_users() {
		// Switch to a theme that does have support.
		switch_theme( 'block-theme' );
		$theme = fp_get_theme();
		FP_Theme_JSON_Resolver_Gutenberg::get_user_data_from_fp_global_styles( $theme );
		...
		$this->assertSame( 0, $query_count, 'Unexpected SQL queries detected for the fp_global_style post type prior to creation.' );
	 ...
	 * @covers FP_Theme_JSON_Resolver_Gutenberg::get_user_data_from_fp_global_styles
	 */
	public function test_get_user_data_from_fp_global_styles_does_not_run_for_theme_without_support() {
	 ...
	 * @covers FP_Theme_JSON_Resolver_Gutenberg::get_user_data_from_fp_global_styles
	 */
	public function test_get_user_data_from_fp_global_styles_does_exist() {
		// Switch to a theme that does have support.
		switch_theme( 'block-theme' );
		$theme = fp_get_theme();
		$post1 = FP_Theme_JSON_Resolver_Gutenberg::get_user_data_from_fp_global_styles( $theme, true );
		$this->assertIsArray( $post1 );
		$this->assertArrayHasKey( 'ID', $post1 );
		fp_delete_post( $post1['ID'], true );
		$post2 = FP_Theme_JSON_Resolver_Gutenberg::get_user_data_from_fp_global_styles( $theme, true );
	 ...
	 * @covers FP_Theme_JSON_Resolver_Gutenberg::get_user_data_from_fp_global_styles
	 */
	public function test_get_user_data_from_fp_global_styles_create_post() {
		// Switch to a theme that does have support.
		switch_theme( 'block-theme' );
		$theme = fp_get_theme( 'testing' );
		$post1 = FP_Theme_JSON_Resolver_Gutenberg::get_user_data_from_fp_global_styles( $theme );
		$this->assertIsArray( $post1 );
		$this->assertSameSets( array(), $post1 );
		$post2 = FP_Theme_JSON_Resolver_Gutenberg::get_user_data_from_fp_global_styles( $theme );
		$this->assertIsArray( $post2 );
		$this->assertSameSets( array(), $post2 );
		$post3 = FP_Theme_JSON_Resolver_Gutenberg::get_user_data_from_fp_global_styles( $theme, true );
	 ...
	 * @covers FP_Theme_JSON_Resolver_Gutenberg::get_user_data_from_fp_global_styles
	 */
	public function test_get_user_data_from_fp_global_styles_filter_state() {
		// Switch to a theme that does have support.
		switch_theme( 'block-theme' );
		$theme = fp_get_theme( 'foo' );
		$post1 = FP_Theme_JSON_Resolver_Gutenberg::get_user_data_from_fp_global_styles( $theme, true, array( 'publish' ) );
		$this->assertIsArray( $post1 );
		$this->assertArrayHasKey( 'ID', $post1 );
		$post2 = FP_Theme_JSON_Resolver_Gutenberg::get_user_data_from_fp_global_styles( $theme, false, array( 'draft' ) );
		$this->assertIsArray( $post2 );
		$this->assertSameSets( array(), $post2 );
	}

	/**
	 * @covers FP_Theme_JSON_Resolver_Gutenberg::get_theme_data
	 */
	public function test_get_theme_data_theme_supports_overrides_theme_json() {
		switch_theme( 'default' );

		// Test that get_theme_data() returns a FP_Theme_JSON_Gutenberg object.
		$theme_json_resolver = new FP_Theme_JSON_Resolver_Gutenberg();
		$theme_json_resolver->get_merged_data();
		$theme_data = $theme_json_resolver->get_theme_data();
		$this->assertInstanceOf( 'FP_Theme_JSON_Gutenberg', $theme_data, 'Theme data should be an instance of FP_Theme_JSON_Gutenberg.' );

		// Test that fp_theme_json_data_theme filter has been called.
		$this->assertGreaterThan( 0, did_filter( 'fp_theme_json_data_default' ), 'The filter "fp_theme_json_data_default" should fire.' );
	...
	public function test_get_theme_data_does_not_parse_theme_json_if_not_present() {
		...
		$empty_theme_json = array( 'version' => FP_Theme_JSON_Gutenberg::LATEST_SCHEMA );
	 ...
	 *
	 * @covers FP_Theme_JSON_Resolver_Gutenberg::get_merged_data
	 *
	...
	public function test_get_merged_data_returns_origin( $origin, $core_palette, $core_palette_text, $block_styles, $block_styles_text, $theme_palette, $theme_palette_text, $user_palette, $user_palette_text ) {
		...
		fp_set_current_user( self::$administrator_id );
		$user_cpt = FP_Theme_JSON_Resolver_Gutenberg::get_user_data_from_fp_global_styles( fp_get_theme(), true );
		...
		$user_cpt['post_content']                         = fp_json_encode( $config );
		fp_update_post( $user_cpt, true, false );

		$theme_json = FP_Theme_JSON_Resolver_Gutenberg::get_merged_data( $origin );
	...
	public function test_get_merged_data_returns_origin_proper() {
		...
		$theme_json_user  = FP_Theme_JSON_Resolver_Gutenberg::get_merged_data( 'custom' );
		$padding_user     = $theme_json_user->get_raw_data()['styles']['spacing']['padding'];
		$theme_json_theme = FP_Theme_JSON_Resolver_Gutenberg::get_merged_data( 'theme' );
	 ...
	 *
	 * @covers FP_Theme_JSON_Resolver::get_style_variations
	 *
	...
	public function test_get_style_variations( $theme, $scope, $expected_variations ) {
		...
		$actual_variations = FP_Theme_JSON_Resolver_Gutenberg::get_style_variations( $scope );

		fp_recursive_ksort( $actual_variations );
		fp_recursive_ksort( $expected_variations );
	...
	public function test_resolve_theme_file_uris() {
		...
		$expected_data = array(
			...
			'styles'  => array(
				'background' => array(
					'backgroundImage' => array(
						'url' => 'https://example.org/fp-content/themes/example-theme/example/img/image.png',
					),
				),
				'blocks'     => array(
					'core/quote' => array(
						'background' => array(
							'backgroundImage' => array(
								'url' => 'https://example.org/fp-content/themes/example-theme/example/img/quote.png',
							),
						),
					),
					'core/verse' => array(
						'background' => array(
							'backgroundImage' => array(
								'url' => 'https://example.org/fp-content/themes/example-theme/example/img/verse.png',
							),
						),
					),
				),
		...
		$filter_theme_file_uri_callback = function ( $file ) {
			return 'https://example.org/fp-content/themes/example-theme/example/' . explode( 'example/', $file )[1];
		};
		add_filter( 'theme_file_uri', $filter_theme_file_uri_callback );
		$actual = FP_Theme_JSON_Resolver_Gutenberg::resolve_theme_file_uris( $theme_json );
	...
	public function test_get_resolved_theme_uris() {
		...
		$expected_data = array(
			array(
				...
				'href'   => 'https://example.org/fp-content/themes/example-theme/example/img/image.png',
				'target' => 'styles.background.backgroundImage.url',
				'type'   => 'image/png',
			),
			array(
				'name'   => 'file:./example/img/quote.jpg',
				'href'   => 'https://example.org/fp-content/themes/example-theme/example/img/quote.jpg',
				'target' => 'styles.blocks.core/quote.background.backgroundImage.url',
				'type'   => 'image/jpeg',
			),
			array(
				'name'   => 'file:./example/img/verse.gif',
				'href'   => 'https://example.org/fp-content/themes/example-theme/example/img/verse.gif',
		...
		add_filter( 'theme_file_uri', $filter_theme_file_uri_callback );
		$actual = FP_Theme_JSON_Resolver_Gutenberg::get_resolved_theme_uris( $theme_json );
		remove_filter( 'theme_file_uri', $filter_theme_file_uri_callback );
	 ...
	 * - variations from `FP_Block_Styles_Registry`
	 */
	public function test_block_style_variation_merge_order() {
		...
		$data         = FP_Theme_JSON_Resolver_Gutenberg::get_theme_data()->get_raw_data();
