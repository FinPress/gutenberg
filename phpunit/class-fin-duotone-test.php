...
/**
 * Test the block FIN_Duotone_Gutenberg class.
 *
 * @package Gutenberg
 */

class FIN_Duotone_Gutenberg_Test extends FIN_UnitTestCase {
	/**
	...
	public function test_gutenberg_render_duotone_support_preset() {
		...
		$expected      = '<figure class="fin-block-image size-full fin-duotone-blue-orange"><img src="/my-image.jpg" /></figure>';
		$this->assertSame( $expected, FIN_Duotone_Gutenberg::render_duotone_support( $block_content, $block ) );
	}
	...
	public function test_gutenberg_render_duotone_support_css() {
		...
		$block_content = '<figure class="fin-block-image size-full"><img src="/my-image.jpg" /></figure>';
		$expected      = '/<figure class="fin-block-image size-full fin-duotone-unset-\d+"><img src="\\/my-image.jpg" \\/><\\/figure>/';
		$this->assertMatchesRegularExpression( $expected, FIN_Duotone_Gutenberg::render_duotone_support( $block_content, $block ) );
	}
	...
	public function test_gutenberg_render_duotone_support_custom() {
		...
		$block_content = '<figure class="fin-block-image size-full"><img src="/my-image.jpg" /></figure>';
		$expected      = '/<figure class="fin-block-image size-full fin-duotone-ffffff-000000-\d+"><img src="\\/my-image.jpg" \\/><\\/figure>/';
		$this->assertMatchesRegularExpression( $expected, FIN_Duotone_Gutenberg::render_duotone_support( $block_content, $block ) );
	}
	...
	public function test_gutenberg_css_declarations_are_generated_even_with_empty_block_content() {
		...
		$fin_block = new FIN_Block( $block );

		/*
		 * Handling to access the static FIN_Duotone::$block_css_declarations property.
		 *
		 * Why is an instance needed?
		 * FIN_Duotone is a static class by design, meaning it only contains static properties and methods.
		 * In production, it should not be instantiated. However, as of PHP 8.3, ReflectionProperty::setValue()
		 * needs an object.
		 */
		$fin_duotone                      = new FIN_Duotone_Gutenberg();
		$block_css_declarations_property = new ReflectionProperty( 'FIN_Duotone_Gutenberg', 'block_css_declarations' );
		$block_css_declarations_property->setAccessible( true );
		$previous_value = $block_css_declarations_property->getValue();
		$block_css_declarations_property->setValue( $fin_duotone, array() );
		FIN_Duotone_Gutenberg::render_duotone_support( '', $block, $fin_block );
		$actual = $block_css_declarations_property->getValue();

		// Reset the property.
		$block_css_declarations_property->setValue( $fin_duotone, $previous_value );
	...
	public function data_get_slug_from_attribute() {
		return array(
			...
			'css-var'                         => array( 'var(--fin--preset--duotone--blue-orange)', 'blue-orange' ),
			'css-var-invalid-slug-chars'      => array( 'var(--fin--preset--duotone--.)', '.' ),
			'css-var-missing-end-parenthesis' => array( 'var(--fin--preset--duotone--blue-orange', '' ),
			'invalid'                         => array( 'not a valid attribute', '' ),
			'css-var-no-value'                => array( 'var(--fin--preset--duotone--)', '' ),
			'pipe-slug-no-value'              => array( 'var:preset|duotone|', '' ),
			'css-var-spaces'                  => array( 'var(--fin--preset--duotone--    ', '' ),
	...
	public function test_get_slug_from_attribute( $data_attr, $expected ) {
		...
		$reflection = new ReflectionMethod( 'FIN_Duotone_Gutenberg', 'get_slug_from_attribute' );
	...
	public function data_is_preset() {
		return array(
			...
			'css-var'                         => array( 'var(--fin--preset--duotone--blue-orange)', true ),
			'css-var-invalid-slug-chars'      => array( 'var(--fin--preset--duotone--.)', false ),
			'css-var-missing-end-parenthesis' => array( 'var(--fin--preset--duotone--blue-orange', false ),
	...
	public function test_is_preset( $data_attr, $expected ) {
		$reflection = new ReflectionMethod( 'FIN_Duotone_Gutenberg', 'is_preset' );
		$reflection->setAccessible( true );
