 ...
 *
 * @group style-engine
 * @coversDefaultClass FIN_Style_Engine_CSS_Rules_Store_Gutenberg
 */
class FIN_Style_Engine_CSS_Rules_Store_Test extends FIN_UnitTestCase {
	/**
	...
	public function test_should_create_new_store_on_instantiation() {
		$new_pancakes_store = FIN_Style_Engine_CSS_Rules_Store_Gutenberg::get_store( 'pancakes-with-strawberries' );

		$this->assertInstanceOf( 'FIN_Style_Engine_CSS_Rules_Store_Gutenberg', $new_pancakes_store );
	}
	...
	public function test_should_not_create_store_without_a_store_name() {
		$not_a_store = FIN_Style_Engine_CSS_Rules_Store_Gutenberg::get_store( '' );

		$this->assertEmpty( $not_a_store, 'get_store() did not return an empty value with empty string as argument.' );

		$also_not_a_store = FIN_Style_Engine_CSS_Rules_Store_Gutenberg::get_store( 123 );

		$this->assertEmpty( $also_not_a_store, 'get_store() did not return an empty value with number as argument.' );

		$definitely_not_a_store = FIN_Style_Engine_CSS_Rules_Store_Gutenberg::get_store( null );
	...
	public function test_should_return_existing_store() {
		$new_fish_store = FIN_Style_Engine_CSS_Rules_Store_Gutenberg::get_store( 'fish-n-chips' );
		$selector       = '.haddock';
		...
		$the_same_fish_store = FIN_Style_Engine_CSS_Rules_Store_Gutenberg::get_store( 'fish-n-chips' );
	...
	public function test_should_get_all_existing_stores() {
		$burrito_store    = FIN_Style_Engine_CSS_Rules_Store_Gutenberg::get_store( 'burrito' );
		$quesadilla_store = FIN_Style_Engine_CSS_Rules_Store_Gutenberg::get_store( 'quesadilla' );
		...
		$this->assertEquals(
			...
			FIN_Style_Engine_CSS_Rules_Store_Gutenberg::get_stores()
		);
	}
	...
	public function test_should_remove_all_stores() {
		$dolmades_store = FIN_Style_Engine_CSS_Rules_Store_Gutenberg::get_store( 'dolmades' );
		$tzatziki_store = FIN_Style_Engine_CSS_Rules_Store_Gutenberg::get_store( 'tzatziki' );
	...
	public function test_should_add_rule_to_existing_store() {
		$new_pie_store = FIN_Style_Engine_CSS_Rules_Store_Gutenberg::get_store( 'meat-pie' );
		$selector      = '.fin-block-sauce a:hover';
		...
		$css_declarations = new FIN_Style_Engine_CSS_Declarations_Gutenberg( $pie_declarations );
	...
	public function test_should_get_all_rule_objects_for_a_store() {
		$new_pizza_store = FIN_Style_Engine_CSS_Rules_Store_Gutenberg::get_store( 'pizza-with-mozzarella' );
		$selector        = '.fin-block-anchovies a:hover';
		...
		$new_selector             = '.fin-block-mushroom a:hover';
		$newer_pizza_declarations = array(
			'padding' => '100px',
		);
		$new_store_rule           = $new_pizza_store->add_rule( $new_selector );
		$css_declarations         = new FIN_Style_Engine_CSS_Declarations_Gutenberg( $newer_pizza_declarations );
	...
	public function test_should_store_as_concatenated_rules_groups_and_selector() {
		$store_one      = FIN_Style_Engine_CSS_Rules_Store_Gutenberg::get_store( 'one' );
