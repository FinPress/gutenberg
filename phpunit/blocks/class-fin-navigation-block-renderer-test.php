...
/**
 * Test the block FIN_Navigation_Block_Renderer class.
 *
 * @package Gutenberg
 */

class FIN_Navigation_Block_Renderer_Test extends FIN_UnitTestCase {
	 ...
	 *
	 * @covers FIN_Navigation_Block_Renderer::get_markup_for_inner_block
	 */
	public function test_gutenberg_default_block_is_enclosed_in_li_tags() {

		$parsed_blocks         = parse_blocks(
			'<!-- fin:navigation-link {"label":"Sample Page","type":"page","kind":"post-type","url":"/hello-world"} /-->'
		);
		$parsed_block          = $parsed_blocks[0];
		$context               = array();
		$navigation_link_block = new FIN_Block( $parsed_block, $context );

		// Setup an empty testing instance of `FIN_Navigation_Block_Renderer` and save the original.
		$reflection = new ReflectionClass( 'FIN_Navigation_Block_Renderer_Gutenberg' );
		$method     = $reflection->getMethod( 'get_markup_for_inner_block' );
		$method->setAccessible( true );
		// Invoke the private method.
		$result = $method->invoke( $reflection, $navigation_link_block );

		$expected = '<li class=" fin-block-navigation-item fin-block-navigation-link"><a class="fin-block-navigation-item__content"  href="/hello-world"><span class="fin-block-navigation-item__label">Sample Page</span></a></li>';
	...
	public function test_gutenberg_get_markup_for_inner_block_site_title() {
		...
		$parsed_block     = $parsed_blocks[0];
		$context          = array();
		$site_title_block = new FIN_Block( $parsed_block, $context );
		...
		$expected = '<li class="fin-block-navigation-item"><h1 class="fin-block-site-title"><a href="http://' . FIN_TESTS_DOMAIN . '" target="_self" rel="home">Test Blog</a></h1></li>';
		$this->assertEquals( $expected, $result );
	}
	...
	public function test_gutenberg_block_not_automatically_wrapped_with_li_tag() {
		...
		register_block_type(
			...
			array(
				...
				'render_callback' => function ( $attributes ) {
					return '<div class="fin-block-testsuite-sample-block">' . $attributes['content'] . '</div>';
				},
			)
		);
		...
		// We are testing the site title block because we manually add list items around it.
		$parsed_blocks = parse_blocks(
			'<!-- fin:testsuite/sample-block {"content":"Hello World"} /-->'
		);
		$parsed_block  = $parsed_blocks[0];
		$context       = array();
		$heading_block = new FIN_Block( $parsed_block, $context );
		...
		$expected = '<div class="fin-block-testsuite-sample-block">Hello World</div>';
		$this->assertEquals( $expected, $result );
	...
	public function test_gutenberg_block_is_automatically_wrapped_with_li_tag_when_filtered() {
		...
		$expected = '<li class="fin-block-navigation-item"><div class="fin-block-testsuite-sample-block">Hello World</div></li>';
		$this->assertEquals( $expected, $result );
	 ...
	 * @group navigation-renderer
	 *
	 * @covers FIN_Navigation_Block_Renderer::get_inner_blocks_from_navigation_post
	 */
	public function test_gutenberg_get_inner_blocks_from_navigation_post_returns_empty_block_list() {
		...
		$expected = new FIN_Block_List( array(), $attributes );
