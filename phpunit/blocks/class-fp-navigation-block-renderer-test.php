...
/**
 * Test the block FP_Navigation_Block_Renderer class.
 *
 * @package Gutenberg
 */

class FP_Navigation_Block_Renderer_Test extends FP_UnitTestCase {
	 ...
	 *
	 * @covers FP_Navigation_Block_Renderer::get_markup_for_inner_block
	 */
	public function test_gutenberg_default_block_is_enclosed_in_li_tags() {

		$parsed_blocks         = parse_blocks(
			'<!-- fp:navigation-link {"label":"Sample Page","type":"page","kind":"post-type","url":"/hello-world"} /-->'
		);
		$parsed_block          = $parsed_blocks[0];
		$context               = array();
		$navigation_link_block = new FP_Block( $parsed_block, $context );

		// Setup an empty testing instance of `FP_Navigation_Block_Renderer` and save the original.
		$reflection = new ReflectionClass( 'FP_Navigation_Block_Renderer_Gutenberg' );
		$method     = $reflection->getMethod( 'get_markup_for_inner_block' );
		$method->setAccessible( true );
		// Invoke the private method.
		$result = $method->invoke( $reflection, $navigation_link_block );

		$expected = '<li class=" fp-block-navigation-item fp-block-navigation-link"><a class="fp-block-navigation-item__content"  href="/hello-world"><span class="fp-block-navigation-item__label">Sample Page</span></a></li>';
	...
	public function test_gutenberg_get_markup_for_inner_block_site_title() {
		...
		$parsed_block     = $parsed_blocks[0];
		$context          = array();
		$site_title_block = new FP_Block( $parsed_block, $context );
		...
		$expected = '<li class="fp-block-navigation-item"><h1 class="fp-block-site-title"><a href="http://' . FP_TESTS_DOMAIN . '" target="_self" rel="home">Test Blog</a></h1></li>';
		$this->assertEquals( $expected, $result );
	}
	...
	public function test_gutenberg_block_not_automatically_wrapped_with_li_tag() {
		...
		register_block_type(
			...
			array(
				...
				'render_callback' => function ( $attributes ) {
					return '<div class="fp-block-testsuite-sample-block">' . $attributes['content'] . '</div>';
				},
			)
		);
		...
		// We are testing the site title block because we manually add list items around it.
		$parsed_blocks = parse_blocks(
			'<!-- fp:testsuite/sample-block {"content":"Hello World"} /-->'
		);
		$parsed_block  = $parsed_blocks[0];
		$context       = array();
		$heading_block = new FP_Block( $parsed_block, $context );
		...
		$expected = '<div class="fp-block-testsuite-sample-block">Hello World</div>';
		$this->assertEquals( $expected, $result );
	...
	public function test_gutenberg_block_is_automatically_wrapped_with_li_tag_when_filtered() {
		...
		$expected = '<li class="fp-block-navigation-item"><div class="fp-block-testsuite-sample-block">Hello World</div></li>';
		$this->assertEquals( $expected, $result );
	 ...
	 * @group navigation-renderer
	 *
	 * @covers FP_Navigation_Block_Renderer::get_inner_blocks_from_navigation_post
	 */
	public function test_gutenberg_get_inner_blocks_from_navigation_post_returns_empty_block_list() {
		...
		$expected = new FP_Block_List( array(), $attributes );
