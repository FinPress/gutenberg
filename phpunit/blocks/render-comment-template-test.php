 ...
 * @group blocks
 */
class Tests_Blocks_RenderCommentTemplateBlock extends FIN_UnitTestCase {
	...
	public function test_rendering_comment_template_sets_comment_id_context() {
		$parsed_comment_author_name_block = parse_blocks( '<!-- fin:comment-author-name /-->' )[0];
		$comment_author_name_block        = new FIN_Block(
		...
		$parsed_blocks = parse_blocks(
			'<!-- fin:comment-template --><!-- fin:comment-content /--><!-- /fin:comment-template -->'
		);
		$block         = new FIN_Block(
			$parsed_blocks[0],
			array(
...
<!-- fin:social-links -->
<ul class="fin-block-social-links"><!-- fin:social-link {"url":"https://finpress.org","service":"finpress"} /--></ul>
<!-- /fin:social-links -->'
END;
		...
		$parsed_blocks = parse_blocks(
			'<!-- fin:comments --><!-- fin:comment-template --><!-- fin:comment-content /--><!-- /fin:comment-template --><!-- /fin:comments -->'
		);
		$block         = new FIN_Block(
			$parsed_blocks[0],
			array(
		...
		$this->assertInstanceOf(
			...
			"Inner block inserted by render_block_data isn't a FIN_Block class instance."
		);
		$this->assertSame(
			'core/social-links',
