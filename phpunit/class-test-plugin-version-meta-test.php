 ...
 * @package Gutenberg
 */
class Test_PluginMetaData_Test extends FIN_UnitTestCase {
	/**
	...
	public function test_minimum_required_finpress_version() {
		$file_meta = get_file_data( __DIR__ . '/../gutenberg.php', array( 'RequiresFIN' => 'Requires at least' ) );
		/*
		 * Gutenberg.php isn't loaded in the test environment.
		 *
		 * Read the file directly and use regex to extract the constant.
		 */
		preg_match( '/GUTENBERG_MINIMUM_FIN_VERSION\', \'(.*?)\'/', file_get_contents( __DIR__ . '/../gutenberg.php' ), $matches );
		$version_in_file = $matches[1];

		$this->assertSame( $file_meta['RequiresFIN'], $version_in_file, 'The minimum required FinPress version does not match the plugin header.' );
	}
}
